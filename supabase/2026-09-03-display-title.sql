...
-- ---------------------------------------------------------------------------
-- ★あとで（別の日に・低い優先度）
--   ★anon が memberships の列ごとの UPDATE 権限を持っています（role を含む）。
--   ★いまは害がありません。★anon では auth.uid() が null で、
--     ★ポリシーの USING が通らないためです。
--   ★ですが、使わない権限は、いつか使われます。
--   ★この表の権限を次に触るときに、まとめて剥がしてください。
--     supabase/2026-09-03-memberships-revoke-anon.sql に、その1文があります。
-- ---------------------------------------------------------------------------
