...
## ④-3 memberships と、役割の書き換え（2026-09-02・★5件確認ずみ／残りあり）

### 実地で確かめたこと（2026-09-02・最終）

| 場面 | 結果 |
|---|---|
| ★① 責任者が自分を owner に上げる | **止まった** |
| ② 責任者がオーナーの役割を変える | **止まった** |
| ⑥ オーナーが講師を「教室の責任者」にする | **通った** |
| ⑦ オーナーが責任者を降格する | **通った** |
| ⑦´ オーナーが、もう一度責任者にする | **通った** |

いまの状態：**+g4t2 = owner、+g4t3 = admin**。

★①は、2026-09-02 に**一度実際に破られた**場面です。
　`memberships_update_role_management`（手で当てられたもの）の WITH CHECK が
　`(role='owner' AND auth.uid()=user_id) OR (role<>'owner')` で、
　**前の役割を見ていなかった**ため、責任者が自分を owner に書き換えられました。
　★「オーナーが自分の行を触る」と「責任者が自分を上げる」が、
　　同じ条件で通っていた、ということです。

**攻撃の道は塞がり、ふつうの教室運営も動いています。**

### なりすましでの確認（2026-09-02・すべて rollback）

| 試したこと | 結果 |
|---|---|
| 責任者が自分を owner に上げる | **42501 で拒否** |
| 講師が自分を責任者に上げる | **42501 で拒否**（順位の規則） |
| オーナーが、ほかの人の役割を正しく変える | **通った（1行）** |
| 責任者が、自分から降りる | **通った（1行）** |

★上げる方向は塞がり、下げる方向と正当な運営は動いています。
　**権限の昇格は、これで決着です。**
　最終状態：+g4t2 = owner、+g4t3 = admin（変わっていません）。

★`migration_role_rank_no_self_promotion.sql` が効いていることも、
　これで裏が取れました。「講師が自分を責任者に」は、
　順位の規則が入っていなければ★通ってしまう場面です。
...
|---|---|---|
| ③ ★共同オーナーAがBを降格 | 0行 | ★未確認 |
| ④ ★オーナーが**自分で**降りる | **1行** | ★未確認 |
| ⑤ 最初の1人（教室を作る） | 1行 | 確認ずみ（+g4t4 が実際に作成） |
| ⑧ 講師が自分を責任者に | 0行 | 確認ずみ（42501） |
| ★知らない役割を書く | 0行 | ★未確認 |

★③と④は、まだ残っています。今回試したのは
　「責任者が自分から降りる」で、④の「★オーナーが自分で降りる」
　とは別の場面です。オーナーの行には
　`role <> 'owner' or user_id = auth.uid()` という別の条件が掛かるため、
　★責任者で通ったことは、オーナーで通る証拠になりません。
　④が0行なら、★オーナーには降りる手段がありません（譲渡も作っていません）。

★とくに⑤です。INSERT のポリシーは、この件の途中で**2回書き換えています**
　（`migration_fix_memberships_update_policy.sql` と
　 `migration_role_rank_no_self_promotion.sql`）。
　いまある教室が、どちらの版の下で作られたのかは分かりません。
　★次に誰かが教室を作ろうとした時が、そのまま試験になります。
　　失敗すれば、**誰も新しい教室を作れません**。

★⑧は、順位で見る規則（`org_role_rank`）を入れてから一度も試していません。
...
## ④-4 退会の止め方（2026-09-02・★直って、確かめ終わりました）

### 何が起きていたか

+g4t3（**責任者**・その教室を作った人ではない）が退会しようとして、
**止まりませんでした**。ほかに3人いる教室から、
★責任者の membership だけが消えました。

原因は `lib/orgClosure.js` の1行です。

```
.from("memberships")….eq("user_id", userId).eq("role", "owner")
```

★`role='owner'` で絞っていたので、責任者では**0件**が返ります。
`classifyOwnedOrgs` は「止める対象なし」と判断し、退会が進みました。

★`countOtherPeople` は悪くありません。memberships を**役割を問わず**
数えています（先生も責任者も最初から対象）。そもそも今回は
**一度も呼ばれていません**。0件のループに入らないためです。

### ★直し方（同じ関数で兼ねないこと）

`listOwnedOrgs` は `/api/org/close` の**権限の判定**にも使われています。
★ここに責任者を足すと、**責任者が教室を消せる**ようになります。

→ 別の関数を作りました。

| 関数 | 答えている問い | 役割 |
|---|---|---|
| `listOwnedOrgs` | この教室を**閉じてよい**のは誰か | オーナーだけ（変更なし） |
| `listOperatedOrgs` | この人が抜けると**困る**教室はどれか | オーナーと責任者 |
...
| | 直す前 | 直したあと |
|---|---|---|
| 責任者の退会 | ★素通り（membership が消えた） | **止まった** |
| 二択の画面 | 出なかった | **出た**（ほかに3人） |

★`supabase/restore_g4t3_membership.sql` で membership を戻してから
　やり直しました。+g4t3 は二択の画面で止め、どちらも押していません。

### 退会の仕分けは、4つになりました（2026-09-02・確認ずみ）

| 場面 | ふるまい | 実地 |
|---|---|---|
| ほかに誰もいない | 教室ごと消す | — |
| 運営できる人が0になる | ★止める（二択） | **確認ずみ**（+g4t3） |
| ★オーナーが抜けるが運営者は残る | 止めない。契約者の知らせ | **確認ずみ**（+g4t2） |
| 責任者・生徒が抜ける | そのまま抜ける | — |

★「教室を動かせるか」と「誰が払うか」は、別の問いです。
　止めるのは前者が0になるときだけ。後者は知らせるだけ。
　（2026-09-02 に一度、前者だけで止めてしまい、
　　★「契約者がいなくなります」という★事実でない文を出していました）

### ★まだ分かっていないこと

`assignments` の行が1つ残っています（teacher_id = +g4t3）。
`purgeAccount` は assignments を**どちらの一覧にも入れていません**
（`USER_OWNED_TABLES` にも `NULLED_REFERENCES` にも無い）。
`lib/authUserReferences.js:126` は `handledBy: "cascade"` としていますが、
★その注記自身が「CASCADE か SET NULL かは見ていない」と書いています。

→ `pg_constraint` の `confdeltype` を見るまで、直し方を決められません。
★+g4t3 の退会は、いまは**正しく止まる**ので急ぎではありません。
