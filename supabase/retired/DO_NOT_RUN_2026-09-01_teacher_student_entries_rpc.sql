-- 2026-09-01 に廃止。流さないこと。
-- 先生が生徒の健康の記録に届く道は、永久に作らない（Opus判断）。
-- この紙は、なぜ廃止したかを残すためだけに置いてある。
--
-- ============================================================================
-- ★2026-09-14、★ここへ 移しました（★裁定 その51・DISPOSITION ㋒）
--
--   ★★確かめた こと（★本番の 台帳・REST で）
--     ★`rpc/get_student_entries` → **PGRST202**（★関数が ありません）
--     ★匿名で `entries` を 読む → 42501（★anon に 権限が ありません）
--     ★本人以外の `entries` を 読む → **0行**（★RLS が 効いて います）
--
--   ★★この 紙の 中には、★`entries` の 列が **48** 並んで います。
--     ★★ぜんぶ 健康の 記録です。★`voice_quality` 1つの 話では ありません。
--     ★★流すと、★48列 ぜんぶが 先生に 開きます。
--
--   ★★中に、★すでに 永久に 採らないと 決めた ものも あります ──
--     ★`cpps_value` ／ `body_fat_pct` ／ `mental_tags` ／ `performance_quality`
--     ★★この 紙が、★それらの お決めより 前の ものだと 分かります。
--
--   ★★消しません。★なぜ やめたかが 分からなく なる からです。
-- ============================================================================
