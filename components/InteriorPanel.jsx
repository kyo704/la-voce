...
import {
  ...
  currentWindow, interiorOf, interiorItemByKey
...
export default function InteriorPanel({ equipped, onChange }) {
  ...
  // ★★窓は、★いつも1組です（★2026-09-08 夕・坂本さんの決め）。
  //   ★選んでいないほうは、★既定が出ています。★それを、そう書きます。
  const nowWin = currentWindow(interiorOf(equipped));
    ...
    <div className="rounded-2xl p-4 border mb-4" style={{ background: C.card, borderColor: C.line }}>
      ...
      {/* ★★窓は、★いつも1組です（★2026-09-08 夕・坂本さんの決め）。
          ★★もとは「2枚そろわなければ、1枚も出さない」でした。
            ★片方だけ選ぶと、★窓が消え、★壁に穴が空いたように見えました。
          ★★いまは、★選んでいないほうを、★既定で埋めます。
            ★★黙りません。★いま何が出ているかを、書きます。 */}
      {(cat === "window" || cat === "view") && (
        <p className="text-xs" style={{ color: C.inkSoft, margin: "0 0 8px", lineHeight: 1.8 }}>
          いまの窓は「{(interiorItemByKey(nowWin.window) || {}).name}」と
          「{(interiorItemByKey(nowWin.view) || {}).name}」です。
          どちらも、いつも1つずつ出ています。
        </p>
      )}
