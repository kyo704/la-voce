...
// ★★★2026-09-23 ── ★確かめを 1つ 足しました。
//   ★★これまで、★この 紙は **1つも 確かめて いません** でした。
//     ★表を 出して `process.exit(0)`。★何が あっても 緑 です。
//     ★★`components/tests/` に 置いて ある ので、★見張りと 数えられて いました。
//   ★★★足したのは 1つ だけ です ── ★「どこにも 出て こない 鍵が 無い こと」。
//     ★★「札を 出さない ことは 書けない ことでは ない」という この 紙の 言い分は
//       ★そのまま です。★だから「画面に ある こと」は 求めません。
//     ★★★求めるのは「★画面 **か** SQL の どちらかには ある」こと だけ です。
//       ★どちらにも 無い 鍵は、★もう 誰も 使って いません。★消し忘れ です。
const 迷子 = KEYS.filter((k) =>
  usedIn(k).length === 0 && !new RegExp("has_can\\([^)]*'" + k + "'").test(sqlAll));
console.log("");
if (迷子.length) {
  console.log("  NG   ★画面にも SQL にも 出て こない 鍵 …… " + 迷子.join("／"));
  process.exit(1);
}
console.log("  ok   ★どの 鍵も、★画面か SQL の どちらかには 出て きます（" + KEYS.length + "）");
process.exit(0);
