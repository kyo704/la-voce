...
ok("0件のときの文がある", /担当の生徒さんは、見つかりませんでした/.test(RAW));
ok("★「いません」と言い切っていない", !/担当の生徒はいません/.test(RAW));
ok("予定が0件のときの文がある", /教室の予定は、見つかりませんでした/.test(カード));
// ★オーナー・責任者のカードも、同じ言い方にそろえました（2026-09-04）。
//   ★だからファイル全体で見張ります。片方だけ戻ることを防ぎます。
ok("★「予定はありません」と、どこにも書いていない", !/予定はありません。/.test(RAW));
// ★「まだ」は急かす言葉として禁止です（no-nagging-words）。
//   ★言い切らないための言い換えとして使いたくなりますが、使えません。
ok("★0件の文に「まだ」を使っていない",
  !/担当の生徒さんは、まだ/.test(RAW) && !/予定は、まだ/.test(RAW));
