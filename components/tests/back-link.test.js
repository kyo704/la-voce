...
console.log("\n⑧ 戻る 道は、★1つの 画面に 1つ");
// ★★2026-09-16、★通っている ところの 中身を 開くと、
//   ★「‹ もっと ／ 通っているところ」と「‹ 通っている ところ」が **2つ** 並びました。
//   ★★撮って、★はじめて 見えました。★字だけでは 分かりません。
//   ★★どちらを 押せば よいか、★読む 方が 迷います。
{
  const at = vt.indexOf("<Back onClick={() => setMoreSection(null)}>もっと");
  const line = at < 0 ? "" : vt.slice(Math.max(0, at - 200), at);
  t(at > 0, "パンくずが ある");
  t(/!attendingOrgId/.test(line), "★★奥の 1枚を 開いて いる あいだは 出さない");
}
