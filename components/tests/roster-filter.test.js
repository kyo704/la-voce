...
(async () => {
  ...
  // ★★★2026-09-18、★「絞る」1枚（下から 上がる）を 外しました（★坂本さんの お決め 2）。
  //   ★★あれは 2026-09-11 に こちらで 足した ものです。★見本に ありません。
  //   ★★札は 表の 上の 列に 移りました。★1つも 消えて いません。
  //   ★★★だから ここも 移します ── ★「1枚が ある か」では なく
  //     ★★「押した その場で 効く か」と「3つ とも 出せる か」を 見ます。
  ok(/onClick=\{\(\) => setChip\(/.test(ui), "★状態 ── 押した その場で 効く");
  ok(/onClick=\{\(\) => setGrade\(/.test(ui), "★学年 ── 押した その場で 効く");
  ok(/onClick=\{\(\) => setTeacher\(/.test(ui), "★先生 ── 押した その場で 効く");
  ok(!/sheetOpen/.test(ui), "★見本に ない 1枚を 置いて いない");
  // ★★学年（★新しい 見本に あります）。
  ...
  // ★★★字 そのものは `lib/smallGroups.js` が 持ちます（★`TOO_SMALL_NOTE`）。
  //   ★★2026-09-18 まで、★画面に 同じ 字が 直に 書いて ありました（★1枚の 中）。
  //   ★★1枚を 外した ので、★名前で 呼ぶ ほう だけ が 残りました。
  ok(/TOO_SMALL_NOTE/.test(ui), "★5人未満の 断りを、★名前で 呼んで いる");
