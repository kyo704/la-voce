...
(async () => {
  ...
  {
    // ★★★割り算の あまり（`a % b`）を 数えません（★2026-09-18）。
    //   ★★時刻の 足し算に `t % 60` を 書いた ところで 落ちました。
    //   ★★★見たい のは「読み手に 見える ％」です。★式の 記号では ありません。
    //     ★★見える ％は、★数や 字の すぐ 隣に 付きます（`50%`）。
    //     ★★あまりの 記号は、★前後に 空きが あります（`t % 60`）。
    const shown = ui
      .replace(/`[^`]*%`/g, "")
      .replace(/"\d+%"/g, "")
      .replace(/\s%\s/g, " ");
    t(!/%/.test(shown), "★読み手に ％を 見せない");
    // ★★道具の 較正 ── ★わざと 見える ％を 作って、★見つかる こと。
    t(/%/.test('<span>50%</span>'.replace(/\s%\s/g, " ")), "★わざとの 1件を 見つけられる");
    t(!/%/.test("const a = t % 60;".replace(/\s%\s/g, " ")), "★あまりの 記号は 数えない");
  }
