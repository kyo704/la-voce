...
(async () => {
  ...
  console.log("\n=== ③ よそと くらべない ===");
  const raw = readRaw("components", "CountV2.jsx");
  t(/まんなかの値です。くらべる先は、あなた自身です。よその目安は 出しません。/.test(raw),
    "★見本⑭の 但し書きを、1文字も 変えずに 出している");
  {
    ...
    t(ui.includes(DISCLAIMER), "★但し書きそのものは、★消していない");
  }
