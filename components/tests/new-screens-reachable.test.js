...
部品.forEach((n) => {
  ...
  if (呼.length > 0) {
    ...
    // ★★★2026-09-26 …… ★紙の 1行が { why, when } の 形に なりました。
    //   ★★`why` に「いつ つなぐ」の 字、★`when` に **機械が 読む 鍵** が あります。
    //   ★★★鍵の 側を 見ます ── ★字だけ だと、★済んだ かどうかを 数えられません
    //     （★2026-09-25 に MoreBundle を つないだ とき、★11枚が 1日 残りました）。
    //   ★★済んだ かどうかは `components/tests/trigger-met.test.js` が 数えます。
    if (書) {
      const 行 = 待[n];
      const when = (行 && typeof 行 === "object") ? 行.when : null;
      t(!!when, "★" + n + " ── 引き金の 鍵（when）が 書いて ある");
    }
  }
});
