...
// ★★値段だけは、9月6日の版が正です（★9月4日の版は置きかわっています）。
//   ★2026-09-07、★年額を 5,800 → 4,800 に下げたとき、
//   ★古いほうだけ見ていて、★試験が気づきませんでした。
const priceNow = fs.readFileSync(
  path.join(ROOT, "docs", "opus", "lavoce-価格と課金の正（9月6日・確定）.md"), "utf-8");
...
// ★★2026-09-06、★よそおいは月額に入りました（★9月4日の版から逆転）。
//   ★9月4日の版は「入りません」のままなので、★そちらは読みません。
//   ★いまの正（9月6日）が、★何と言っているかを見ます。
ok("★よそおいが、月額に入っている", /毎月 \*\*1点\*\*、自分で選べる/.test(priceNow));
ok("★「月額に3点ついてくる」と書いていない",
  !/毎月のよそおい ── 服・壁・床・家具・景色から★?3点/.test(price));
// ★★2026-09-07、★体験期間を「作る」に引っくり返りました。
//   ★向きは同じです。★「受け取ったものを取り上げない」は、変わっていません。
//   ★まだ作っていないので、★正にそう書かれていることだけを見ます。
ok("★体験期間の反転が、正に記録されている", /14日の体験期間を作ります/.test(priceNow));
...
ok("★月額が、実装と合っている（580）",
  /priceYen: 580/.test(plans) && /¥580/.test(priceNow));
// ★★2026-09-07、★5,800円 → 4,800円。
ok("★年額が、実装と合っている（4800）",
  /priceYen: 4800/.test(plans) && /¥4,800/.test(priceNow));
