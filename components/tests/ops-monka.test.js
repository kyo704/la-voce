...
(async () => {
  ...
  // 【四】★★印
  // -------------------------------------------------------------------------
  console.log("【四】足りない 見込み（★言葉で 出します）");
  // ★★★印では なく 言葉 です（★2026-09-19・Opus）。
  //   ★★印だと「※は 何だったか」を 思い出さねば なりません。
  t(A.SHORT_WORD === "足りない 見込み", "言葉で 出す");
  t(!本文.includes('"※"') && !本文.includes("'※'"), "★画面に 印を 置いて いない");
  t(本文.includes("SHORT_WORD"), "★言葉を lib から もらって いる");
  // ★★★説明の 1行を 置いて いない こと（★言葉に 説明は 要りません）。
  t(!本文.includes("SHORT_NOTE"), "★説明の 1行を 置いて いない");
  ...
  // ★★★説明の 1行は 置きません（★2026-09-19・Opus）。
  //   ★★言葉 そのものが 意味を 持ちます。
  //   ★★「責める ものでは ありません」は、★下の 注に 残して います。
  t(M.NOTES.some((x) => x.includes("責める ものでは ありません")),
    "★責めない、と 注に 残って いる");
  // ★★色で 示して いない こと。
  ...
  // ★★★2026-09-19（★お決め D78）── ★1つに なりました。
  //   ★★「代表を 決める」…… 出来ました（`set_monka_representative`）
  //   ★★「時間割が まだの方」…… 出来ました（`get_timetable_submitted`）
  //   ★★★残るのは「レッスンの 枠」だけ です ── ★しまう ところが ありません。
  //   ★★★数を 覚えません。★0件でない ことと、★中身が 揃って いる ことを 見ます。
  t(M.NOT_YET.length >= 1, "★1つ 以上（★空の 節を 出さない ため）");
  M.NOT_YET.forEach((x) => {
  ...
  //     ★★分けられません。★どちらも 0 です。
  t(M.NOT_YET.every((x) => x.why && x.needs && x.label),
    "★なぜ まだ 出せないかと、★何が 要るかを 書いて いる");
