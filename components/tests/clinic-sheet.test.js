...
(async () => {
  ...
  //
  //   ★★★前は `isOn(clinicPick, "` の 数を 6 と 覚えて いました。
  //     ★2026-09-25、★紙に「氏名は 入れていません」を 出す 1行を 足した とき、
  //     ★7に なって 赤く なりました。★足した ものは 節では なく **文** です。
  //     ★★覚えた 数は、★正しい 仕事を 止めます。
  //   ★★いまは **節の 数を その場で 測ります** ── 節は 出し分けの 札
  //     （`display: isOn(clinicPick, …)` か `&& isOn(clinicPick, …)`）を 持ちます。
  //     ★文は 持ちません。
  const 節 = new Set();
