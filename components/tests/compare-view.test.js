...
(async () => {
  ...
  // ★★★2026-09-21、★見本が「判定」の 語を 外しました（★裁定157 T5）。
  //   ★★実装も 見本の 字を そのまま 写します。★言い換えません。
  //   ★★★この 見張りが 持って いた 字は、★古い 見本の ものでした。
  //     ★★字を ここに 書き写す 限り、★見本が 動くたび に ここも 動きます。
  t(/あとから 書いた日は ○の 白抜き。数えるときに 外します。/.test(raw), "○の 断り（★note）");
  t(/結果は「\{\(LAGS\.find/.test(raw) || /の 記録で 固定<\/b>です。ここは 見るだけ。/.test(raw),
    "★結果は 1つに 固定、の 断り");
  ...
  t(/並べる と さかのぼる は、いつでも 見られます。/.test(lb), "★休まない2つを 案内する");
  t(/並べるを 見る/.test(lb) && /さかのぼるを 見る/.test(lb), "★行き先を 押せる形で 置く");
