...
t(押.length > 0 && 押.every((x) => x >= 44), "★押しどころは 44 以上（" + 押.join("／") + "）");
  ...
  // ---------------------------------------------------------------------------
  // ★2026-09-24 ── ★回が 無い ときに、★白い 画面に しない（★Opus の ご指摘）
  //
  //   ★★前は `return null` でした。★何も 出ません。
  //     ★★「読み込み中」と「無い」と「壊れた」が、★同じ 顔に なります。
  // ---------------------------------------------------------------------------
  t(!/if \(!round\) return null;/.test(画), "★白い ままに して いない");
  t(/MAP_NO_ROUND_HEAD/.test(画) && /MAP_NO_ROUND_SUB/.test(画),
    "★何が 起きて いるかと、★次に する ことを 出して いる");
  {
    // ★★束の 字は、★紙から そのまま 読みます（★写しを 作りません）。
    const 束 = readRaw("lib", "lessonRound.js");
    t(/MAP_NO_ROUND_HEAD = "まだ 回が ありません。"/.test(束), "★責めない 字");
    t(/「回を 始める」から、新しい 学期の 回を 作れます。/.test(束),
      "★次に 押す ところを 書いて いる");
  }
