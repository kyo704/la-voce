...
const FORBIDDEN = [
  ...
  // ★★㋐（画面から 隠す）は、★2026-09-11 夜に 取り消されました。
  //   ★★坂本さん「隠す のでは なく、★見本と 同じ 畳む しくみを 使ってください」
  //   ★★私が「見本に 畳む しくみは ない」と 誤って ご報告して いました。
  //     ★見本は <details> を 使わず、★JavaScript で 畳んでいます。
  //   ★★隠すと、★パッサッジョの通りにくさ・高音の出しやすさ の 入口が
  //     ★1つも 無くなり、★書けなく なります。★だから 畳んで 出します。
  //   ★★題は 見えます。★中身は「詳しく 書く」を 押すまで 出ません。
  //     ★★中身が 閉じている ことは、★下の ③で 見ます。
...
const MUST = [
  ...
  // ★★畳んだ節の 題と 札（★2026-09-11 夜の 訂正）。
  //   ★★入口が 消えて いない ことを 見ます。
  { key: "画面-記録", text: "お仕事に合わせた記録" },
  { key: "画面-記録", text: "詳しく 書く" }
];
MUST.forEach((m) => {
...
console.log("\n③ 畳んだ 節の 中身が、閉じている こと");
// ★★題は 見えて、★中身は 見えない。★これが「畳む」です。
//   ★★書き出しは display:none を 拾いません。★だから 中身は 出ません。
{
  const p = path.join(FRAMES, "画面-記録@390.json");
  if (!fs.existsSync(p)) { t(false, "画面-記録 の 書き出しが ありません"); }
  else {
    const texts = JSON.parse(fs.readFileSync(p, "utf8")).map((d) => d.text).join("\n");
    [
      ["パッサッジョの通りにくさ", "お仕事に合わせた記録 の 中"],
      ["高音の出しやすさ", "同上"]
    ].forEach(([w, why]) => {
      t(!texts.includes(w), "★「" + w + "」は 閉じている（" + why + "）");
    });
  }
}
