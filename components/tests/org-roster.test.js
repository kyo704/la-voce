...
(async () => {
  ...
  console.log("\n=== ④ 段・初期費用・年の 前払い ===");
  // ★★★2026年9月21日、★下限が 1本 から 2本に なりました
  //   （★裁定155 §3 ／ 確定-教室・学校の値段 第3版）──
  //     ★無料 …… 先生1人＋生徒5人まで　　0円
  //     ★教室 …… 6〜30人　　　　　　　月 2,980円（★定額。★段の 式を 当てません）
  //     ★学校 …… 31人〜　　　　　　　下限 月 12,800円
  // ★★★9,800円は 無く なりました。★12,800円に 戻った のでは ありません。
  //   ★★9,800円の ころ、★20人は 9,800円 でした。★いまは 2,980円 です。
  //   ★★同じ「下限」の 言葉で、★中身が 変わって います。★書き残します。
  eq(m.SCHOOL_FLOOR, 12800, "★学校の 下限は 12,800（★2026-09-21・裁定155）");
  eq(m.CLASSROOM_MONTHLY, 2980, "★教室は 月 2,980（★定額）");
  eq(m.monthlyFee(6), 2980, "★6人から 教室");
  eq(m.monthlyFee(20), 2980, "★20人も 教室（★人数で 変わりません）");
  eq(m.monthlyFee(30), 2980, "★30人まで 教室");
  // ★★境目 ── ★31人から 学校。★段の 式（31×400＝12,400）より 下限が 上です。
  eq(m.monthlyFee(31), 12800, "★31人から 学校（★下限が 当たる）");
  eq(m.monthlyFee(32), 12800, "★32人（32×400＝12,800。★下限と 同じ 値）");
  eq(m.monthlyFee(60), 24000, "★60人（60×400）");
  // ★★段の 名も 見ます。★数だけ 合って 段が ちがう、を 起こさせません。
  eq(m.planOf(3), "無料", "★3人は 無料");
  eq(m.planOf(6), "教室", "★6人は 教室");
  eq(m.planOf(31), "学校", "★31人は 学校");
  // ★★教室の 年額は 月額×10 では ありません（★裁定155 §3 の 値）。
  eq(m.yearlyFee(20), 29800, "★教室の 年額は 29,800");
  ...
  //   （★裁定155 Q4 ／ 確定 第3版 §4）。
  eq(m.setupFee(99), 0, "★99人なら 初期費用 なし");
  eq(m.setupFee(100), 100000, "★100人から 100,000円");
