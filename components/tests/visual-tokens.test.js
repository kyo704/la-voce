...
(async () => {
  ...
  // -------------------------------------------------------------------------
  console.log("【四】比を 測る（★床 4.5・★狙い 5.0）");
  // ★★★字が のる かも しれない 地を、★**ぜんぶ** 回ります。
  //   ★★2026-09-18、★`--card` だけ 見て「通った」と しかけました。
  //   ★★地を 1つ しか 見ない 見張りは、★見て いないのと 同じ です。
  //
  // ★★★床では 止めません（★2026-09-18・Opus）。
  //   ★★「WCAG は 床 であって 天井では ない。
  //     ★★4.48 で 通すくらいなら、★もう 1段 暗く する」
  //   ★★だから 字は 5.0 を 求めます。★`--ink4` だけ 4.7 です。
  t(m.CONTRAST_FLOOR === 4.5, "床は 4.5");
  t(m.CONTRAST_AIM === 5.0, "狙いは 5.0（★床の 上に 余白を 置きます）");

  const 地 = m.GROUND_TOKENS;
  t(地.length === 7, "地を 7つ 回る");
  function いちばん低い(T, f) {
    return 地.map((b) => ({ b, r: 比(T[f], T[b]) })).sort((x, y) => x.r - y.r)[0];
  }
  [["明", m.TOKENS_LIGHT], ["暗", m.TOKENS_DARK]].forEach(([名, T]) => {
    m.TEXT_TOKENS.forEach((f) => {
      const 低 = いちばん低い(T, f);
      t(低.r >= m.CONTRAST_AIM,
        `${名} --${f} ★いちばん 低い 地は --${低.b}　${低.r.toFixed(2)}`);
    });
  });

  // ★★`--ink4` ── ★いちばん 薄い 段。★それでも 字 です（★裁定 §1-1「文字 ── 4段」）。
  //   ★★5.0 に すると `--ink3` と 並び、★4段の 下の 段が 消えます。
  //   ★★だから 4.7 を 求め、★かつ `--ink3` より **低い** ことを 見ます。
  t(m.FAINT_TOKEN === "ink4", "薄い 字の 名が 決まって いる");
  [["明", m.TOKENS_LIGHT], ["暗", m.TOKENS_DARK]].forEach(([名, T]) => {
    const 低 = いちばん低い(T, m.FAINT_TOKEN);
    t(低.r >= m.CONTRAST_AIM_FAINT,
      `${名} --${m.FAINT_TOKEN} ★いちばん 低い 地は --${低.b}　${低.r.toFixed(2)}（狙い 4.7）`);
    t(低.r >= m.CONTRAST_FLOOR, `${名} --${m.FAINT_TOKEN} ★床 4.5 を 越えて いる`);
    // ★★4段の 順が 崩れて いない こと。
    const 三 = いちばん低い(T, "ink3").r;
    t(低.r < 三, `${名} --ink4 は --ink3 より 薄い（${低.r.toFixed(2)} < ${三.toFixed(2)}）`);
  });

  // ★★地に 使う えんじ。★白い 字を 載せます。
  t(比("#FFFDF8", m.TOKENS_LIGHT["enji-d"]) >= m.CONTRAST_AIM,
    `明 白字 / --enji-d ＝ ${比("#FFFDF8", m.TOKENS_LIGHT["enji-d"]).toFixed(2)}`);
  // ★★道具の 較正 ── ★わざと 読めない 組を 作って、★見つかる こと。
  t(比("#777777", "#7A7A7A") < m.CONTRAST_FLOOR, "★わざとの 1件（読めない 組）を 見つけられる");
  // ★★★「床は 越えるが 狙いに 足りない」も 見つけられる こと。
  //   ★★これが 今回の 直しの 肝 です（★4.48 を 通さない）。
  const 惜しい = 比("#8C6115", "#F6E4E8");
  t(惜しい >= 4.4 && 惜しい < m.CONTRAST_AIM, `★わざとの 1件（惜しい ${惜しい.toFixed(2)}）を 見つけられる`);

  // ★★動かした 記しに、★役（字か どうか）が 書いて ある こと。
  m.RULING_DEVIATIONS.forEach((d) => {
    t(d.role === "text", `動かし ${d.no} ── 役が 書いて ある（${d.role}）`);
  });
