...
(async () => {
  ...
  // -------------------------------------------------------------------------
  // ★2026-09-24 ── ★見本 `SC['当日の進行を見る']` と 読みくらべて（★段3a A群）
  //
  //   ★★字の 向きが 逆 でした。★見本は 呼ばれて **いる** 行に 置きます。
  //     ★★逆に すると、★出番の 少ない 方ほど「呼ばれていません」が 並びます。
  // -------------------------------------------------------------------------
  {
    const 束 = await import("data:text/javascript;base64,"
      + Buffer.from(fs.readFileSync(path.join(ROOT, "lib", "myKoenDay.js"), "utf8"))
        .toString("base64"));
    t(束.DAY_ALSO_YOU === "あなたも", "★呼ばれて いる 行の 字（見本の まま）");
    // ★★呼ばれて **いる** 行に 出す こと。
    t(/!isStaff && r\.mine \? \(\s*\n?\s*<span style=\{小\}>\{tx\(DAY_ALSO_YOU\)\}/.test(流),
      "★呼ばれて いる 行に 出して いる");
    t(!/!isStaff && !r\.mine \? \(\s*\n?\s*<span style=\{小\}>\{tx\(DAY_NOT_CALLED\)\}/.test(流),
      "★行ごとに「呼ばれていません」を 並べて いない");
    // ★★1つも 無い ときだけ、★1度 だけ。
    t(/!行\.some\(\(r\) => r\.mine\)/.test(流), "★1つも 無い ときの 1行が ある");
    // ★★薄さは 残して いる こと（★流れは 見えた ほうが よい）。
    t(/opacity: isStaff \|\| r\.mine \? 1 : 0\.5/.test(流), "★薄さで 分けて いる");
    // ★★註でも 言って いる こと（★3度 言わない ための 前提）。
    t(束.DAY_NOTE.join("").includes("薄い 行は、あなたが 呼ばれていない ところです。"),
      "★註が わけを 言って いる");
  }
