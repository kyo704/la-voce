...
-- ★auth スキーマの引き金（on_auth_user_created など）。2026-09-23 の土台の書き出しで抜けていた
atg as (select tgname||':'||pg_get_triggerdef(oid) x from pg_trigger
         where not tgisinternal and tgrelid in (select oid from pg_class where relnamespace='auth'::regnamespace)),
-- ★既定の権限（新しいプロジェクトは anon を含む／本番は含まない、という差が出る）
da as (select defaclrole::regrole::text||':'||defaclnamespace::regnamespace::text||':'||defaclacl::text x
         from pg_default_acl),
...
select json_build_object(
 ...
 'auth_triggers',json_build_object('n',(select count(*) from atg),'md5',(select md5(string_agg(x,',' order by x)) from atg)),
 'default_privs',json_build_object('n',(select count(*) from da), 'md5',(select md5(string_agg(x,',' order by x)) from da)),
