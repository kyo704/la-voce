...
ANON_OK={
 ...
 'page_entries_capped':'★公開ページの 節（裁定209 C・2026-09-26）。'
                       '★会員でない 人が 見る ページ なので anon に 要る。'
                       '★★出す 数を 12件で 止めるだけ。★書き込みは しない。'
                       '★★見えるのは 渡された user の 節 ── ★公開の 判定は 呼ぶ 側（get_public_portfolio）',
 'count_referral':'★公開ページの 印から 来た 数（裁定209 C・2026-09-26）。'
                  '★会員でない 人が 通る ところ なので anon に 要る。'
                  '★★実在の 公開 slug からだけ 数える／★1日1ページ 10,000で 頭打ち／'
                  '★★誰が 来たかは 持たない（IP も 端末も）／★読み出しは 誰にも 渡していない',
 'island_residents':'★アイランドの 住人（裁定209 ③・2026-09-26）。'
                    '★会員でなくても 見られる 島。★on_map かつ 公開の 人だけ 出る',
 'island_count':'★アイランドの 人数（同上）。★数だけ。★誰が いるかは island_residents 側',
 'submit_inquiry':'お問い合わせの受け口（裁定129）。書くだけで 読めない',
 'accept_guardian_consent':'保護者の同意（合言葉で受ける・裁定147）。理由を返さない作り',
 'island_residents':'★アイランドの 住人（裁定209）。★公開＋同意＋18歳以上の 3つが 揃った 人だけ。★返すのは 公開ページに 出ている ものだけ（slug・名前・地区・公開日）',
 'island_count':'★アイランドの 人数（裁定209）。★数だけ。★個人は 返さない',
