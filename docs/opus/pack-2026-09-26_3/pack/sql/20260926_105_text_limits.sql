...
-- ★① ★経歴（bio）── ★★800字（★2026-09-26 決定）
--   ★★はじめ 400字と 書きました
--   ★★★Code が 本番を 数えたら ★676字の 方が いました
--   ★★★もし 400字で 当てて いたら:
--     ★★その方は ★★もう 直せません（★書いた ものが 制約に 引っかかります）
--     ★★★制約を 足す 前に ★いまの 中身を 数える ── ★物差しに
...
-- 確かめ（試しの環境で）
-- ★800字 ちょうど → 通る／★801字 → 止まる
-- ★★いまの 676字の 方が ★そのまま 直せる
-- ★★日本語 800字 → ★通る（★byte では ない）
