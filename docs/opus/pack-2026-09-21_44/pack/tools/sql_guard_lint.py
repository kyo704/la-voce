...
# ★逃げ道と認める書き方（2026-09-23 に直した）
#   はじめ 'is not null' を入れていたが、★ふつうの条件文にも出るので
#   「逃げ道がある」と誤って判定し、★E6（消せなくする引き金）を 見逃していた
#   → ★印（current_setting）を使っているものだけを 逃げ道と認める
ESCAPES = ('closing_org', 'closing_koen', 'app.retention', 'current_setting', 'pg_trigger_depth')

def lint(sql, name=''):
    s = re.sub(r'--[^\n]*', '', sql)   # ★注を外した本文（判定用）。注そのものは sql で見る
    out = []
    ...
    for m in re.finditer(r'create\s+or\s+replace\s+function\s+public\.(\w+)\s*\(\s*\)\s*returns\s+trigger([\s\S]*?)\$\$;', s, re.I):
        ...
        if ev and not re.search(r'delete|update', ev.group(2), re.I):
            continue
        # ★わざと逃げ道を置かない場合は、その関数の前に
        #   -- guard-ok: 理由   と1行 書く（★理由を必ず書かせる）
        okmark = re.search(r'--\s*guard-ok:[^\n]*\n(?:[^\n]*\n){0,3}?\s*create\s+or\s+replace\s+function\s+public\.' + re.escape(fn), sql, re.I)
        if okmark: continue
