...
<header>
 <h1>Woolsong ── 作品を 探す</h1>
 <p>__TOTAL__作・__SCENES__場面。★歌詞・楽譜・対訳・あらすじは 入っていません</p>
</header>
<div class="wrap">
 <div class="bar">
  <div class="who">
   <button id="w-singer" aria-pressed="true">歌う 人として 見る</button>
   <button id="w-staff" aria-pressed="false">制作・カンパニーとして 見る</button>
  </div>
...
<dialog id="dlg"><div class="dh">
 ...
 <div class="dhr">
  <div class="swap"><button id="d-singer">歌う 人</button><button id="d-staff">制作</button></div>
...
function setWho(v){ who=v;
 document.getElementById('w-singer').setAttribute('aria-pressed',v==='singer');
 document.getElementById('w-staff').setAttribute('aria-pressed',v==='staff');
 document.getElementById('d-singer').setAttribute('aria-pressed',v==='singer');
 document.getElementById('d-staff').setAttribute('aria-pressed',v==='staff');
 hint.innerHTML = v==='singer'
  ? '<span class="star">★</span>自分の 役を 選ぶと、その 役の 稽古として 記録を はじめられます。'
  : '<span class="star">★</span>作品を 選ぶと 香盤表（場面 × 役）が そのまま 立ちます。表は 書き出せます。';
 if(document.getElementById('dlg').open && cur) open(cur);
}
document.getElementById('w-singer').onclick=()=>setWho('singer');
document.getElementById('w-staff').onclick=()=>setWho('staff');
document.getElementById('d-singer').onclick=()=>setWho('singer');
document.getElementById('d-staff').onclick=()=>setWho('staff');
...
function open(w){ cur=w; if(myRole && !w.R.some(r=>r[0]===myRole)) myRole=null;
 ...
 if(who==='singer'){
  // ── 歌う 人として
  h+='<h3>自分の 役を 選ぶ</h3><div class="pick-role">'+
   w.R.map(r=>'<button data-r="'+esc(r[0])+'"'+(myRole===r[0]?' aria-pressed="true"':'')+'>'+
    esc(r[0])+(r[1]?'（団体）':'')+'</button>').join('')+'</div>';
  if(myRole){
   const mine=w.S.map((s,i)=>[i,s]).filter(([,s])=>s[2].includes(myRole));
   ...
   h+=mine.length?'<table class="mine"><tr><th>'+esc(ki.group||'まとまり')+'</th><th>'+esc(ki.row||'場面')+'</th><th>一緒に 出る 人</th></tr>'+
    ...
    :'<div class="note">この 役が 出る '+esc(ki.row||'場面')+' は 入っていません。</div>';
   h+='<div class="act"><button id="go">この 役の 稽古として 記録を はじめる</button>'+
    '<button class="sub" id="csv1">自分の 出番を 書き出す</button></div>';
   h+='<div class="yakusoku"><b>約束</b><br>'+
    '<span class="star">★</span>体調の 記録は <b>先生・学校・運営・制作・審査員に 見えません</b>。<br>'+
    '<span class="star">★</span>役を 選んだことも <b>ほかの 人には 出ません</b>。<br>'+
    '<span class="star">★</span>Woolsong は 声の 調子を <b>点数に しません</b>。</div>';
  } else {
   h+='<div class="hint" style="margin-top:8px"><span class="star">★</span>役を 選ぶと、'+
    'その 役が 出る '+esc(ki.row||'場面')+'だけが 出ます。</div>';
  }
 }
 // ── 香盤表（★両方の 立場で 出します）
 ...
 if(!w.S.length){ h+='<div class="note">場面が 入っていません。</div>'; }
  ...
  if(who==='staff'){
   ...
   h+='<div class="yakusoku"><b>約束</b><br>'+
    ...
    '<span class="star">★</span>Woolsong は 出演者を <b>点数に しません</b>。</div>';
  }
 }
 // ── 編成
 if(w.I&&w.I.length){
