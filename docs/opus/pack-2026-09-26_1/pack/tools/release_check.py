...
def main(new,old):
    ...
    # ★読込エラーは 0 が 正。★開けないは ★上限（★減るのは よいこと・2026-09-25）
    #   ★前は 6 ちょうど を 求めていて、★5 に 減らしたら 止まりました
    exp={'全画面':(0,6),'iPhone':(0,6),'運営':(0,0),'個人':(0,0)}
    for k,(e0,e1) in exp.items():
        m=re.search(k+r'[^\n]*読込エラー(\d+)[^\n]*開けない(\d+)',o)
        if not m: ng.append(f'allsc {k}: 結果なし'); continue
        if int(m.group(1))!=e0: ng.append(f'allsc {k}: ★読込エラー '+m.group(1))
        if int(m.group(2))>e1: ng.append(f'allsc {k}: ★開けないが 増えた '+m.group(2)+'>'+str(e1))
