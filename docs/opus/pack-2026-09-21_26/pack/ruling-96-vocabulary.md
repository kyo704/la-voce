...
## 7c. ORG_DIVISION_DELETE

```yaml
# 2026-09-19 確定。見本の記述が危険だったため訂正

rejected:
  案: 学部を消すと、その中の学科もいっしょに消える
  proposed_by: Opus（見本に実装していた）
  why_dangerous: |
    在籍者の学科情報が静かに消える。
    本人も事務も気づかない。
    「取り消されても損害が生じない形にする」に反する

adopted:
  rule: 下位が1つでもあれば、上位を消せない
  order: 下から順に手動で消す
  message: 「この学部の中に、まだ N つの学科があります」

use_check:
  その分類を使っている人が1人でもいれば消せない（既存）
  加えて、下位の分類が1つでもあれば消せない（今回）

confirm_dialog:
  消す前に必ず1度たずねる
  ワンクリックで消えない
  表示: 名前 ／ 消せない理由 ／ 取り消せないこと

note: |
  Opus の設計が危険だった。Code が実害を指摘して止めた。
  「連鎖させると楽」より「黙って消えない」を優先する
```
