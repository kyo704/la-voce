...
def main():
    a=asyncio.run(collect(A)); b=asyncio.run(collect(B)); rows1=compare(a,b)
    rows, flaky = rows1, []
    if rows1 and any('文字が違う' in r[0] for r in rows1):
        # ★2回取って、★両方に出たものだけを「食い違い」とする（2026-09-23）
        #   前は 取り直した結果で 置き換えていたため、★回ごとに件数が変わり
        #   release_check が ときどき 配らないと言っていた（★不安定な見張りは 見られなくなる）
        a2=asyncio.run(collect(A)); b2=asyncio.run(collect(B)); rows2=compare(a2,b2)
        rows  = [r for r in rows1 if r in rows2]
        flaky = [r for r in rows1 if r not in rows2] + [r for r in rows2 if r not in rows1]
        a, b = a2, b2
    print(f'MOBILE_PARITY 食い違い {len(rows)}件')
    for r in rows: print('  ',' | '.join(r))
    if flaky:
        print(f'  ★回ごとに 出たり出なかったり: {len(flaky)}件（★止めません。中身が 時刻や順番で 変わる画面です）')
        for r in flaky[:6]: print('    ',' | '.join(r))
    bad=[]
