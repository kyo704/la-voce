...
# ★見本の ある場所を 自分で 探す（2026-09-24）
#   ★私の 束では tools/ の 親に 見本が あります
#   ★リポジトリでは docs/design/pack-final/ などに あります
#   → ★「見本が 実際に ある場所」を 探して そこを PACK に します
def _find_pack():
    """★見本の ある場所を 探す。★tools/ から 上に たどります
       （2026-09-24: ★全体を 歩くと 過去の束や よその場所を 拾いました）"""
    MOCK='00-動く見本-iPhoneで開く用.html'
    SKIP=re.compile(r'pack[-_]\d|/旧版/|/_old-|/reports?/|/docs/opus/')
    cand=os.path.dirname(HERE)
    if os.path.exists(os.path.join(cand, MOCK)): return cand      # ★束の 形
    # ★リポジトリの 形: tools/ の 親から 下を 探す（★過去の束は 飛ばす）
    best=None
    for d,dirs,fs in os.walk(cand):
        dirs[:] = [x for x in dirs if not SKIP.search((d+'/'+x+'/').replace(os.sep,'/'))]
        if MOCK in fs:
            p=(d+'/').replace(os.sep,'/')
            if SKIP.search(p): continue
            # ★pack-final のような「いまの置き場」を 優先する
            if best is None or 'final' in p: best=d
    return best or cand
PACK=_find_pack()
...
def check_stale(files=None,P=P,baseline=None):
    ...
    ★★2026-09-26: ★★裁定の 文（ruling-*.md）は ★見ません
      ★理由: ★★裁定は「★この 数字を 使わない」と ★書く ところ です
            ★★禁止を 書いた ことで ★止まるのは ★おかしい
      ★★私は 一度 ★裁定 3本を 書き換えて しまいました（★戻しました）
    """
