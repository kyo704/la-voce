...
def check(root):
    ...
    sids = collections.defaultdict(list)   # ★検査I
    n = sc = 0
    for f in sorted(glob.glob(os.path.join(root, '*.json'))):
        ...
        for w in (d if isinstance(d, list) else [d]):
            ...
            sids[w.get('source_id') or os.path.splitext(b)[0]].append((b, w.get('title')))
    ...
    # ★★検査I ── ★同じ source_id は ★取り込みで ★あとの 1つが 前を 消します。
    #   ★2026-09-26:『イタリア歌曲集』と『イタリア歌曲集（ヴォルフ）』が 同じ sid で、
    #   ★★2,051作 入れたら 2,050作に なりました（★試しの Postgres で 気づきました）。
    #   ★★検査D は 題名が 違うので 捕まえません
...
MSG = {
 ...
 'H': '★★ミュージカルに 版（edition）が 無い ── ★版で 曲が 変わります',
    'I': '★★同じ source_id ── ★取り込みで ★あとの 1つが 前を 消します',
}

def check_source_id(works):
    """★I ★★同じ source_id が 2つ以上 ── ★取り込みで ★あとの 1つが 前を 消します。

    ★★2026-09-26 に 起きた こと:
      ★『イタリア歌曲集』（既にある・曲名20曲入り）と
      ★『イタリア歌曲集（ヴォルフ）』（私が 作った・曲数だけ）が
      ★★同じ source_id 'wolf-italienisches' でした。
      ★2,051作を 取り込んだら ★2,050作に なりました。
    ★★検査D（同じ題名・同じ作曲家）は ★題名が 違うので 捕まえられません。
    """
    import collections, os
    c = collections.defaultdict(list)
    for f, w in works:
        sid = w.get('source_id') or os.path.splitext(os.path.basename(f))[0]
        c[sid].append((os.path.basename(f), w.get('title')))
    return [(k, v) for k, v in c.items() if len(v) > 1]

HARD = {'A', 'B', 'C', 'D', 'G', 'H', 'I'}   # ★これは 直す
SOFT = {'E', 'F'}                  # ★これは 知らせるだけ
...
def selftest():
    ...
    # ★★検査I ── ★同じ source_id・★違う 題名（★検査D では 捕まらない 形）
    #   ★2026-09-26 に 実際に 起きた 形を そのまま 仕込みます
    d2 = tempfile.mkdtemp()
    J.dump(dict(ok, title="イタリア歌曲集", source_id="wolf-italienisches"),
           open(os.path.join(d2, 'x.json'), 'w'))
    J.dump(dict(ok, title="イタリア歌曲集（ヴォルフ）", source_id="wolf-italienisches"),
           open(os.path.join(d2, 'y.json'), 'w'))
    n, sc, o = check(d2)
    assert o.get('I'), f'★★同じ source_id を 見逃した: {dict(o)}'
    assert not o.get('D'), '★題名が 違うので D では 捕まらない はず'
    # ★★ぶつかって いない ときに 騒がない ことも 見ます
    d3 = tempfile.mkdtemp()
    J.dump(dict(ok, source_id="a1"), open(os.path.join(d3, 'p.json'), 'w'))
    J.dump(dict(ok, title="べつ", source_id="a2"), open(os.path.join(d3, 'q.json'), 'w'))
    n, sc, o = check(d3)
    assert not o.get('I'), f'★ぶつかって いないのに 騒いだ: {o.get("I")}'
