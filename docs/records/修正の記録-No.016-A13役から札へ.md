...
## ★★訂正 ── 「`is_org_owner_or_admin` は 0本」は、★**決まりの 層 だけ** でした

★2026-09-14。★Opus が 台帳の 関数を 直に 読んで 見つけました。

★★A13 が 移したのは **決まり（policy）の 層 だけ** です。
　★★**関数の 層は、★一度も 触って いません。**

| いまも 呼んで いる 関数 | 出どころ |
|---|---|
| `create_org_event(p_org_id, …)` | ★紙に あり（`supabase/2026-09-04-rpc-functions.sql:160`） |
| `can_view_ops(viewer_id, p_org_id, p_student_id)` | ★★紙に **ありません**。★台帳だけ |

★★正しい 言い方 ── 「★**決まりでは 0本。★関数に 2本 残存**」。

★★`can_view_ops` は、★倉庫の 紙を いくら 読んでも 出て きません。
　★SQLエディタで 直に 作られた もの です。
　★★2026-09-14 の 数え ── 倉庫 24件／別名 18件 に 対し、★台帳 **66件**。
　★★その 差の 中に、★これが 居ました。

★★片づけは ㉑（A2）に 入れました ── `docs/records/やること-9月15日のあと.md`。
★★いきさつは `docs/records/修正の記録-No.020-台帳の関数を読んで見つかった4件.md`。
