...
全436行 / 末尾は「★★そうすれば、★私の 側でも 同じ ことを 確かめられます。」

★見つけた日 2026-09-14（★Opus が 台帳を 直に 読んで）
★調べた日 2026-09-14（★私が 倉庫の 紙で 裏を 取りました）
★直した日 2026-09-14（★②）／ 2026-09-15（★②の 実機確認・③の 片づけ）
...
### ★★実機で 通りました（★2026-09-15）

★本物の ブラウザで、★2つの 入れ物を 使って 通しました
　（★`tools/invitation_flow_e2e.py`・★再実行 できます）。

| | |
|---|---|
| 先生 | kyo0703opera+teachertest@gmail.com（adcab9c5…） |
| 生徒 | kyo0703opera+forcode@gmail.com（f7520dc1…） |
| コード | `PNJE7MJZ` |
| 絵 | `docs/design/compare/invitation/` 9枚 |
| コンソールの 誤り | ★0件 |

★★画面の 字だけでは 信じません。★台帳で 裏を 取りました ──

```
teacher_student_links   teacher adcab9c5… / student f7520dc1… / active
                        accepted_at 2026-09-15T00:42:22.331756+00:00
teacher_invitations     PNJE7MJZ
                        used_at     2026-09-15T00:42:22.331756+00:00
                        used_by_student_id f7520dc1…
```

★★★`accepted_at` と `used_at` が **同じ 時刻** です。
　★★`UPDATE … RETURNING` の **1文**で 取って 読んで いる ことの しるし です。
　★★前の 形なら、★`insert` の あとに `update` が 走る ので ずれます。

★同じ コードの 2度目 ── ★**400 `INVITATION_NOT_USABLE`**。★使い回せません。

### ★片づけ（★2026-09-15・坂本さん お裁き「片づけて よい」）

★紙 `supabase/drop_accept_invitation.sql`（★私は 流せません）。

★★落とす 前に、★その 場で もう一度 呼び手を 数えます（★第1部）──
　★関数の 中 ／ 決まりの 中 ／ 引き金。★1つでも 出たら 落としません。
★★落とした あと、★**生きて いる ほう**が 無事か 見ます（★第3部 ③-2）。
　★★消して しまって いないかを、★必ず その場で 確かめる ため です。
