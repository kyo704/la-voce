...
★特定商取引法に基づく表記（`app/legal/tokushoho/`）は、2026-09-03 に削除しました。
無償で提供しているあいだ、表示義務は生じないためです（通信販売は「有償の役務の提供」が要件）。
★空欄（月額［金額］円）と、事実と違う記載が公開されている状態でした。
★有料化する日に、完全な形で作り直してください。★そのときは、
利用規約の料金の条・Stripe の設定と、★同じ1回の作業として行ってください。

いずれも `［ ］` の部分を実情に合わせて埋めてください。これらは草案であり法的助言ではないため、
実際の公開前に一度、専門家（弁護士・行政書士等）に確認することをおすすめします。

★新しい版は `docs/legal/` にあります（`terms-ja-2026-09-v1.md` /
`privacy-ja-2026-09-v1.md`）。★どちらもまだ公開していません。
★撤回ボタンができるまで、公開できません。
