...
export const BACK_TO = tx("自分の 予定");

/** ★升目を 選んで いない ときの 1枚（★見本の 前半）。 */
export const NOT_PICKED_TITLE = tx("予定を 選んでください");
export const NOT_PICKED_NOTE = tx("一覧から、見たい 予定を 押してください。");

/** ★どちらの 予定か（★見本の `pills`）。 */
export const KIND_LABEL = tx("どちらの 予定ですか");
...
export const KINDS = Object.freeze([
  Object.freeze({ key: "school", label: tx("学校の 予定") }),
  Object.freeze({ key: "self", label: tx("自分の 予定") })
]);
...
export const KIND_NOTE_SELF =
  tx("事務から「動かしてください」と 言われません。理由も 聞かれません。");
export const KIND_NOTE_SCHOOL =
  tx("学校の 用です。行事と ぶつかったとき、事務が 相談に 来ます。");
export function kindNoteOf(row) {
  return isSelf(row) ? KIND_NOTE_SELF : KIND_NOTE_SCHOOL;
}

/** ★名前の 欄（★見本の `input`）。 */
export const NAME_LABEL = tx("何が ありますか");
export const NAME_OPTIONAL = tx("（空でも かまいません）");
export const NAME_PLACEHOLDER = tx("れい：西洋音楽史（講義）");
...
export const NAMES_SCHOOL = Object.freeze([
  tx("予定"), tx("講義"), tx("会議"), tx("非常勤"), tx("入試"), tx("行事"), tx("移動")
]);
export const NAMES_SELF = Object.freeze([
  tx("予定"), tx("用事"), tx("通院"), tx("お休み"), tx("送りむかえ"), tx("移動"), tx("伴奏合わせ"), tx("練習")
]);
...
/** ★札 2つ。 */
export const BTN_DELETE = tx("この 予定を 消す");
export const BTN_OK = tx("これで いい");

/** ★下の 断り（★約束。★消さないこと）。 */
export const NOTES = Object.freeze([
  tx("ここには レッスンを 入れられません。空きに 戻すと、入れられるように なります。"),
  tx("この 名前が 見えるのは あなただけです。事務にも 生徒にも 出ません。")
]);
