...
/**
 * ★★ホーム画面の案内を、どの形で出すか（2026-09-05）
 *
 *   ★★パソコンの方には、★出しません。★1つも出しません。
 *     ★パソコンには、★そもそも「ホーム画面」がありません。
 *     ★★そして、★出る幕もありません。
 *       ★パソコンは、★ふつうのウェブサイトとして使っていただきます。
 *       ★リンクを押しても、★出ていく先がありません。
 *
 *   ★★2026-09-05、★ここが漏れていました。
 *     ★画面の側で「beforeinstallprompt が来たら Android」と決めていました。
 *     ★★beforeinstallprompt は、★パソコンの Chrome と Edge にも来ます。
 *       ★だから、★パソコンの方に Android 向けの手順が出ていました。
 *         「画面の右上にある ⋮ のボタンを押す」
 *     ★★「案内が出せるか」と「どの機械か」は、★別の問いです。
 *       ★前者から後者を決めると、★こうなります。
 *
 *   ★この判断は、★ここ1か所にあります。★画面の側で組み立てないこと。
 */
export function installGuidePlatform({ os, standalone = false, isIosSafari = false } = {}) {
  // ★もうホーム画面版で開いておられるなら、★案内は要りません。
  if (standalone) return "other";
  if (os === OS.IOS || isIosSafari) return "ios";
  if (os === OS.ANDROID) return "android";
  // ★★パソコン・その他は、★"other" です。★案内は出ません。
  return "other";
}
