...
// ★★選び直した 2色目も、★覚え書きの 鍵に 入れます（★2026-09-08 夜）。
//   ★★入れないと、★2色目を 変えても 前の絵が そのまま 出ます。
export function cacheKey(itemKey, colorKey, second) {
  return itemKey + ":" + colorKey + (second ? ":" + second : "");
}

/** ★もう塗ってあるか。★あれば、その在りかを返します（★待ちません）。 */
export function paintedUrl(itemKey, colorKey, second) {
  const k = cacheKey(itemKey, colorKey, second);
 ...
 */
export async function paintCloth(itemKey, colorKey, second) {
  if (!isColorable(itemKey) || !colorKey) return null;
  const k = cacheKey(itemKey, colorKey, second);
  const done = paintedUrl(itemKey, colorKey, second);
  if (done) return done;
  ...
  const job = (async () => {
    ...
    if (mask) {
      ...
      c2 = secondColor(itemKey, colorKey, hexToRgb, paintHex, second);
    }
