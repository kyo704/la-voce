...
// ★★★2026-09-26 ── ★400 → **800**（★坂本さんの お決め）。
//   ★★台帳を 数えたら、★400字を 超える 行が 1つ ありました（★676字）。
//     ★★書いた ものを 切らない ので、★上限を 上げました。
//   ★★★台帳の 縛りと 同じ 数 です（★`sql/105` の `portfolios_bio_len`）。
//     ★★片方だけ 変えると、★画面は 通して 台帳が 弾きます（★23514）。
 ...
 */
// ============================================================================
// ★★★節の 一覧は **この 紙に ありません**（★2026-09-26・裁定207・Opus の お指し）
//
//   ★きょうまで ここに `KINDS_BY_FIELD` を 手で 書いて いました。
//   ★★台帳（`portfolio_entries.kind` 16種）と 見本（music 9／voice 11／stage 11）に
//     ★対して、★手書きは music 9／voice 7／stage 8 でした。★節の 名も 古い まま。
//   ★★★2か所に 書いた から ずれました。★だから 書くのを やめました。
//
//   ★出どころは `public.my_page_fields()` **だけ** です（★`sql/96`・本番 132本目）。
//   ★読み方は `lib/pageFields.js` が 持ちます。★倒れ先の 一覧も 置きません。
//
//   ★★`enabledAt`（★出す 日）も 消しました ── ★台帳の 関数が 出す ものが 正 です。
//     ★★38名の 画面は 変わりません ── ★経歴の 1枚は 門の 中だけ に あります。
// ============================================================================
