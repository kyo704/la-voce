...
export function sheepItemSrc(item, side) {
  if (!item) return null;
  // ★★WebP を 先に 出します（★2026-09-09）。★読めなければ PNG に 戻ります。
  //   ★決めは lib/imageFormat.js が 持ちます。★ここで「.webp」と 書きません。
  if (item.files) {
    const k = side === "L" ? "left" : (side === "C" ? "both" : "right");
    return webp(SHEEP_ASSET_BASE + (item.files[k] || item.files.right));
  }
  return webp(SHEEP_ASSET_BASE + item.file);
}
