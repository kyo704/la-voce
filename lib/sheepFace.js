...
export function preloadList() {
  return FIRST_FACES.map((k) => FACE_BASE + FACES[k]);
}

/** ★あとから 読む 残り。★手が空いたときに 読みます。 */
export function laterPreloadList() {
  return Object.keys(FACES)
    .filter((k) => !FIRST_FACES.includes(k))
    .map((k) => FACE_BASE + FACES[k]);
}
