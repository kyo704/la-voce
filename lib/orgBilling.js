...
/**
 * ★領収書の 断り（★裁定 その74 追補・2026-09-18／★2026-09-19 に 直しました）。
 *
 ...
 *
 *   ★★★2026-09-19 ── ★登録の ご予定が ある と 伺いました。
 *     ★★「いま 無い」ことは 変わりません。★そこは そのまま 書きます。
 *     ★★★日づけを 書きません。★決まって いない ことを 約束に しません。
 *       ★★「月曜 以降 できるだけ 早く」と 伺って います。
 *       ★★登録番号が 入れば、★この 断りは 自動で 消えます（`hasInvoiceNo`）。
 */
export const NOT_QUALIFIED_INVOICE =
  "いまは 適格請求書では ありません。登録を 予定して います。";
...
// ---------------------------------------------------------------------------
// ★支払い方法（★見本 `P_pay`・お決め D13・2026-09-19）
// ---------------------------------------------------------------------------
//   ★★★いま 選べるのは「請求書（銀行振込）」だけ です。
//     ★★カードと 口座振替は、★カード会社・金融機関の 画面が 前提 です。
//     ★★その 道が、★この 蔵に まだ ありません（★Stripe は 休んで います）。
//     ★★★押せる 札に しません（★§8⑤）。★「まだ」と 名ざしで お伝えします。
//
//   ★★★カード番号・口座番号は、★どの 道でも Woolsong に 入りません。
//     ★★入れる 欄が ありません。★台帳に 列も ありません。

export const METHOD_HEAD = "支払い方法";

/**
 * ★選べる 道と、★まだ の 道。
 *
 *   `value` … ★台帳に 入れる 字（★`org_billing.method`）
 *   `ready` … ★いま 選べるか
 *   `needs` … ★まだ の とき、★何が 足りないか
 */
export const METHOD_CHOICES = Object.freeze([
  { key: "invoice", value: "invoice", label: "銀行振込（請求書）",
    note: "年払い・自治体・学校法人 向け。請求書を お送りします。", ready: true },
  { key: "card", value: "card", label: "クレジットカード",
    note: "カード会社の 画面で 入れていただきます。",
    ready: false, needs: "カード会社へ つなぐ 道（いまは 休んで います）" },
  { key: "bank", value: null, label: "口座振替",
    note: "金融機関の 画面で 手続きします。",
    ready: false, needs: "金融機関へ つなぐ 道と、台帳の 字（いまは card と invoice だけ）" }
]);

export function readyMethods() {
  return METHOD_CHOICES.filter((x) => x.ready);
}
export function notYetMethods() {
  return METHOD_CHOICES.filter((x) => !x.ready);
}

/**
 * ★銀行振込は **年払い だけ** です（★見本の warn）。
 *
 *   ★★毎月の 振込は、★双方の 手間が 大きすぎます。
 *   ★★だから「お支払いの まとまり」を 選ばせません。★決まって います。
 */
export const INVOICE_YEARLY_ONLY =
  "銀行振込は 年払いのみ お受けします。毎月の 振込は、双方の 手間が 大きすぎます。";

/** ★年の ご請求（★2か月ぶん 引きます）。★数は lib/orgRoster.js が 出します。 */
export const YEARLY_LINE = "1年 まとめて。2か月ぶん 引きます。";

export const METHOD_NOTES = Object.freeze([
  "カード番号・口座番号は、Woolsong に 1つも 入りません。入れる 欄が ありません。",
  "変えても、いまの 月の ご請求は 変わりません。次の ご請求から 新しい 方法に なります。",
  "変えた ことは 記録に 残ります（誰が・いつ・何を）。消せません。",
  "止める とき（解約）は、この 画面では できません。お問い合わせから 承ります。"
]);

/** ★記録に 残す 字。 */
export function logWordForMethod(label) {
  return "支払い方法を " + (label || "") + " に しました";
}
