 ...
 */
/**
 * 指定した日が、直近の開始日から何日目か。★開始日からの導出のみ。
 *
 * ★以前は entries.cycle_start（1日1行の記録の中の真偽値）から数えていました。
 *   ホームの新しいボタンは cycle_periods に書くため、同じ「周期◯日目」が
 *   2つの別々の置き場所から計算される状態になっていました。
 *   片方に入れた記録が、もう片方からは見えません。置き場所は1つにします。
 */
// ---------------------------------------------------------------------------
// ★「この人に周期の機能を出すか」を決める、唯一の場所。
//
//   これが3か所に分かれていました。
//     設定のスイッチ … sex !== "男性" && track_cycle
//     ホームの1行    … track_cycle だけ
//     記録画面の欄   … track_cycle だけ
//   その結果、性別を「男性」にした人の画面に、ホームと記録画面だけ
//   周期の欄が出続けました。★しかも設定のスイッチは隠れているので、
//   本人には消す手段がありませんでした。出ているのに消せない、が
//   いちばん質の悪い出方です。
//
//   sex を "男性" にすること自体が、この機能に該当しないという意思表示です。
//   未設定の人には出します（該当するかどうか分からないため、選べるように）。
// ---------------------------------------------------------------------------
export function cycleFeatureApplies(profile) {
  if (!profile) return false;
  if (profile.sex === "男性") return false;
  return true;
}

/** 記録する設定になっているか（機能が該当し、かつ本人が有効にしている） */
export function cycleTrackingOn(profile) {
  return cycleFeatureApplies(profile) && !!(profile && profile.track_cycle);
}

/** ホームに1行を出すか（§4-3 の3段階の②） */
export function cycleShowsOnHome(profile) {
  return cycleTrackingOn(profile) && (profile.cycle_show_on_home !== false);
}

export function cycleDayForDate(dateISO, periods) {
  const starts = sortPeriods(periods)
    .map((p) => p.start_date)
    .filter((d) => d && d <= dateISO);
  if (starts.length === 0) return null;
  return diffDays(starts[0], dateISO) + 1;   // sortPeriods は新しい順
}

/** その日が、記録された開始日そのものか（記録画面の表示に使う） */
export function isCycleStartDate(dateISO, periods) {
  return (periods || []).some((p) => p && p.start_date === dateISO);
}
