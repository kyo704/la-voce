 ...
 */
/**
 * ★学校を 選ぶ（★裁定 その140・2026-09-21）。
 *
 *   ★★★1校 だけ の 方には、★選ぶ ところを 出しません（★裁定 その73）。
 *     ★★持って いない ものを 出しません。
 *   ★★★2校 以上の ときだけ 並べます。★混ぜません。
 *     ★★混ぜると「どの 学校の 方か」が 見る人に 分かりません。
 *     ★★応募した とき、★相手に 見える 自分の 所属が あいまいに なります。
 *   ★★★覚えるのは 端末だけ です。★台帳に 持ちません。
 *     ★★「最後に 見た 学校」は、★残す 値打ちが ありません。
 */
export const SCHOOL_HEAD = tx("どの 学校で さがしますか");
/** ★端末に 覚える ときの 名。 */
export const SCHOOL_KEY = "la-voce-matching-org";

/**
 * ★どの 学校を 選ぶか。
 *
 *   ★★① 前に 選んだ 学校（★いまも 在籍して いれば）
 *   ★★② 無ければ、★いちばん 早く 入った 学校（★並びは 台帳が 決めます）
 */
export function pickSchool(schools, remembered) {
  const 並 = Array.isArray(schools) ? schools : [];
  if (並.length === 0) return null;
  const 覚 = 並.find((x) => x && x.org_id === remembered);
  return (覚 || 並[0]).org_id;
}

/** ★選ぶ ところを 出すか（★1校 なら 出しません）。 */
export function showSchoolPicker(schools) {
  return Array.isArray(schools) && schools.length >= 2;
}
