...
export const NOTES = Object.freeze([
  ...
  "「足りない 見込み」は、残り 全部 出ても 届かない ときに 出ます。",
  "責める ものでは ありません。声を かける ための 目安です。",
...
export const NOT_YET = Object.freeze([
  {
    key: "timetable",
    // ★★★2026-09-19 ──「出して いるか どうか」は **出来ました**（★お決め D78）。
    //   ★★`get_timetable_submitted` が 真偽 1つ だけ を 返します。
    //   ★★残るのは「レッスンの 枠」だけ です ── ★しまう ところが ありません。
    label: "レッスンの 枠",
    ...
    why: "どこに 入れたか を しまう ところが ない",
    needs: "レッスンの 枠の しまい場所（見本 `P_okeru` ／ `lesson_slots`）"
  }
  // ★★★2026-09-19 ──「時間割が まだの方」も 出来ました（★お決め D78）。
  //   ★読み道 `get_timetable_submitted`（★真偽 だけ）
  //   ★書く道 `nudge_timetable`（★1回だけ。★2度目は 台帳が 弾きます）
  //   ★★門下の 帯に「まだ 出せません」は 1つも 残って いません。
]);


/** ★代表を 決める へ 行く 札（★見本 `P_daihyo`・2026-09-19）。 */
/** ★門下に 招く へ 行く 札（★見本 `P_monkaInvite`・裁定 その108・2026-09-20）。 */
export const GO_INVITE_LABEL = "門下に 招く";
export const GO_INVITE_SUB = "名簿に いる 方を、名指しで 招けます。";

/** ★時間割が まだの方 へ 行く 札（★見本 `P_mada`・2026-09-19・お決め D78）。 */
export const GO_MADA_LABEL = "時間割が まだの方";
export const GO_MADA_SUB = "出したか どうかだけ 見えます。1回だけ 知らせられます。";

export const GO_DAIHYO_LABEL = "代表を 決める";
export const GO_DAIHYO_SUB =
  "1〜2名まで。新しい 役職では ありません（門下の 中の 印です）。";

/** ★日程を 組む へ 行く 札（★見本 `P_monka` の 1つ目）。 */
export const GO_KUMU_LABEL = "レッスンの 日程を 組む";
export const GO_KUMU_SUB = "空いて いる コマから 組めます。中身は 見えません。";
