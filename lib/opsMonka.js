...
export const NOT_YET = Object.freeze([
  ...
  {
    key: "kumu",
    // ★★★2026-09-19 ──「レッスンの 日程を 組む」は 出来ました（★裁定 その98 ①）。
    //   ★★読み道（`get_student_free_slots`）が 出来たので、★詰まりが 外れました。
    //   ★★残りは 2枚 です。
    // ★★★2026-09-19 ──「代表を 決める」は 出来ました（★見本 `P_daihyo`）。
    //   ★★書く道は `set_monka_representative`（★台帳が 2人までを 守ります）。
    //   ★★残りは 1枚 です。
    label: "時間割が まだの方",
    why: "行き先の 画面が ありません（P_mada）",
    needs: "その 1枚を 作る こと"
  }
]);
