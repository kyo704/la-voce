 ...
 *     ★★`components/NotesV2.jsx` が、★打つ たびに 時計を かけ直し、
 *       ★`AUTOSAVE_MS` で ひとりでに 送って います。
 *     ★★下の 2つは「やめる」と「しまう」── ★どちらも 出口 です。
 *       ★★「保存」の 札は ありません。★押し忘れが 起きません。
 *
 *   ★②「先生には 送られません。コメント欄も ありません。」
 *     ★★`notes` の 決まりは 4つ とも `auth.uid() = user_id` です。
 *       ★先生の 決まりも、★`security definer` の 道も ありません。
 *     ★★この 画面に コメントの 欄は 1つも ありません。
 *       ★★`entry_comments` は 別の もの です（★きょうの 記録の ほう）。
 *
 *   ★③「みた曲を 選ぶと、レパートリーの その曲 にも 同じメモが 出ます。」
 *     ★★`notesForRepertoire` が 出して います（★曲の 台帳の「この曲の メモ」）。
 *     ★★★写しを 作って いません。★同じ 1行を、★2つの ところから 見て います。
 */
export const PRACTICE_NOTES = Object.freeze([
  "打った そばから 残ります（この2つは 出口だけ）。",
  "先生には 送られません。コメント欄も ありません。",
  "みた曲を 選ぶと、レパートリーの その曲にも 同じメモが 出ます。"
]);
