...
export const AUTH_USER_REFERENCES = [
  ...
  // ---- ★まだ実物を確かめていないもの ----
  //   ★「たぶん CASCADE だろう」で cascade と書かないこと。
  //     それが③（events）の原因そのものです。
  //     確かめていないものは pending と書き、下の一覧に理由を残します。
  { table: "assignments", column: "teacher_id", handledBy: "pending", verified: false }
];
...
export const PENDING_VERIFICATION = [
  {
    table: "assignments",
    column: "teacher_id",
    note: "先生が退会したときの assignments の扱い。行ごと消すのか、null にするのか、"
        + "外部キーの CASCADE に任せるのかを、実物の制約を見てから決めます。"
        + "★student_id と org_id が CASCADE なのは確認ずみですが、teacher_id は未確認です。"
  }
];
