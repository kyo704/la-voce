...
export const TIMING = Object.freeze({
  ...
  fadeMs: 300,
  riseMs: 180,
  // ★★ひとりごとの 間（★2026-09-09・実機「もっと ひんぱんに」）。
  //   ★もとは 45〜90秒に1回。★25〜55秒に1回に します。
  //   ★★同時に 2つ 出さない決めは、★そのままです。
  soloMinMs: 25000,
  soloMaxMs: 55000
});
...
/**
 * ★吹き出しの 大きさ（★2026-09-08 夜・実機「ひとりごとが 読みにくい」）。
 *
 *   ★★2行までに 収めます。★3行に なると、★読みにくくなります。
 *
 *   ★★もとは ひとりごと 幅132・字16 でした。
 *     ★「むこうの へやが、しずかです。」が 3行に なっていました。
 *   ★★あいづちも、★幅168・字20 で、★8文が 3行に なっていました。
 *     ★ご指摘は ひとりごとだけでしたが、★同じ算です。★合わせて 直します。
 *
 *   ★★字を 少し小さくし、★幅を 少し広げました。
 *     ★ひとりごと 幅168・字14　★あいづち 幅200・字17
 *   ★★これで、★70文すべてが 2行に 収まります（★見張りで 数えます）。
 *
 *   ★★部屋から はみ出さない幅に しています。
 *     ★羊の箱は 部屋の 26％、★部屋は せまくても 360px ほどです。
 *     ★200px なら、★羊が 右端にいても 部屋の中に 収まります。
 */
export const BUBBLE = Object.freeze({
  solo: Object.freeze({ maxWidthPx: 168, fontPx: 14, padXPx: 9 }),
  reply: Object.freeze({ maxWidthPx: 200, fontPx: 17, padXPx: 11 }),
  borderPx: 2,
  maxLines: 2
});
 ...
 * ★その文が、★何行に なるか（★見積り）。
 *
 *   ★★全角は 1文字ぶん、★半角（空白・英数）は 半分で 数えます。
 *     ★見積りなので、★少し 多めに 出ます。★そのほうが 安全です。
 *   ★★見張りで 使います。★新しい文が 3行に なったら、★そこで 落ちます。
 */
export function bubbleLines(text, spec) {
  const sp = spec || BUBBLE.solo;
  const inner = sp.maxWidthPx - sp.padXPx * 2 - BUBBLE.borderPx * 2;
  let w = 0;
  for (const c of String(text || "")) {
    w += c.charCodeAt(0) > 0x2000 ? sp.fontPx : sp.fontPx / 2;
  }
  return Math.max(1, Math.ceil(w / inner));
}
