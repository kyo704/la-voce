 ...
 */
export function seatPos(placed, positions) {
  return firstPlaced(SEAT_KEYS, placed, positions);
}

/** ★置いている中から、★寝られるものを 1つ。 */
export function bedPos(placed, positions) {
  return firstPlaced(BED_KEYS, placed, positions);
}
...
function firstPlaced(keys, placed, positions) {
  ...
  for (const k of keys) {
    ...
    return {
      left: 50,
      top: (it && typeof it.floorY === "number") ? it.floorY : FLOOR_BAND[1]
    };
  }
  return null;
}
