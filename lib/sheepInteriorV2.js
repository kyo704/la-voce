...
// ============================================================================
// ★見本 A07 の 部屋（★design.zip ／ 2026-09-10）
//
//   ★出どころ docs/design/pack/screens/A07-ひつじながめる既定.html
//     ★枠　　 height:452px（★本文の 列 330px の 中）
//     ★床　　 height:120px（★枠の 下）
//     ★羊　　 width:230px（★絵は 420×558 なので、★高さ 305px）
//
//   ★★これまでの 部屋は 4：3 で、★高さ 247px 相当でした。
//     ★羊は 幅の 26%（★87px）── ★見本の 3分の1 ほどでした。
//   ★★実機で「羊が 大きく 表示されていない」と ご指摘を いただきました。
//     ★そのとおりでした。★A07 の 寸法を 入れていませんでした。
//
//   ★★門の中（名簿の方）だけです。★38人の 部屋は 4：3 の ままです。
// ============================================================================

/** ★枠の 横：縦（★見本 A07）。 */
export const ROOM_ASPECT_V2 = "330 / 452";

/** ★床の 高さ（★枠の 高さに 対する ％）。 */
export const FLOOR_PCT_V2 = 120 / 452 * 100;

/** ★羊の 高さ（★枠の 高さに 対する ％）。 */
export const SHEEP_HEIGHT_PCT_V2 = 305 / 452 * 100;

/**
 * ★見本 A07 の 羊の 大きさ（★画素）。
 *
 *   ★★SheepDressed は 正方形の 入れ物です。★高さが そのまま 大きさに なります。
 *   ★★枠の 高さから 出します。★幅から 出すと、★枠の 縦横比を 変えたとき ずれます。
 */
export function sheepSizePxV2(roomHeightPx) {
  const h = (typeof roomHeightPx === "number" && roomHeightPx > 0) ? roomHeightPx : 0;
  if (!h) return 0;
  return h * SHEEP_HEIGHT_PCT_V2 / 100;
}
