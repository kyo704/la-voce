import { webp } from "@/lib/imageFormat";
import index from "@/docs/assets/sheep-interior-index.json";

// ============================================================================
// おうちの内装 120点（第2〜5便・2026-09-08）
//
//   ★出どころ assets/wardrobe-v3 の zip の中の acnh/interior/
//            （★中身の説明は、その zip の README.md が正です）
//
//   ★★いまの101点（lib/character.js の SHOP_ITEMS）とは、★別のものです。
//     ★鍵が、★1つも重なりません（★2026-09-08 に数えました）。
//     ★★だから「差し替え」ではなく、★足す形になります。
//     ★101点をどうするかは、まだ決まっていません
//       （docs/reports/2026-09-08-おうちの道具79点の行く先.md）。
//
//   ★★大きさが、★5種類あります。★1024 の1種類ではありません。
//     家具・庭・壁掛け・昭和   320×320  ★床に接する位置は y300
//     窓枠                     384×384  ★中は抜いてある（景色が透ける）
//     扉                       320×512
//     窓の外の景色             480×320
//     壁・床のタイル           256×256  ★敷き詰め用
//
//   ★★窓は「枠」と「景色」の★2枚です。
//     ★13枠 × 13景色 ＝ ★169通り。
//     ★重ね順は ★景色（view）→ 枠（window）。★枠が上です。
//
//   ★見張り components/tests/sheep-interior-v2.test.js
// ============================================================================

export const INTERIOR_INDEX = index;
export const INTERIOR_ITEMS = index.items;
export const INTERIOR_BASE = "/sheep/";

/** ★分類。★並べる順です（★探す順であって、描く順ではありません）。 */
export const INTERIOR_CATEGORIES = Object.freeze([
  { key: "furniture", label: "家具" },
  { key: "showa", label: "昭和の家具" },
  { key: "window", label: "窓枠" },
  { key: "view", label: "窓の外" },
  { key: "door", label: "扉" },
  { key: "wallart", label: "壁かけ" },
  { key: "garden", label: "庭の置物" },
  { key: "tile", label: "壁と床" }
]);

export function categoryLabel(key) {
  const c = INTERIOR_CATEGORIES.find((x) => x.key === key);
  return c ? c.label : key;
}

export function interiorItemByKey(key) {
  return INTERIOR_ITEMS.find((i) => i.key === key) || null;
}

export function interiorSrc(item) {
  // ★★WebP を 先に（★2026-09-09）。★読めなければ PNG に 戻ります。
  return item ? webp(INTERIOR_BASE + item.file) : null;
}

export function itemsByCategory(category) {
  return INTERIOR_ITEMS.filter((i) => i.category === category);
}

/**
 * ★床に接する位置（★zip の README より）。
 *
 *   ★「家具は y300 が床の線です。320の画像の下20pxは余白なので、
 *     ★床のラインに y300 を合わせて置いてください。全点そろえてあります。」
 *
 *   ★★数を、ここ1か所で持ちます。★画面で書かないこと。
 */
export const FLOOR_LINE_Y = 300;
export const FURNITURE_CANVAS = 320;

/**
 * ★その絵の、床に着く線（★実測）。
 *
 *   ★★2026-09-08、★どの絵も y300 だとして置いていました。
 *     ★README に「全点そろえてあります」と書かれていたためです。
 *   ★★実際に測ったところ、★そろっていませんでした。
 *     家具 247 / 301 / 309　　庭 289〜311　　昭和 181〜309
 *     ★★扉は 503（★512の絵）。★300 を当てると、★2割ちかく浮きます。
 *     ★実機で「扉が壁に接していない」とご指摘をいただきました。
 *   ★★だから、★1点ずつの実測値（floorY）を使います。
 *     ★名簿に入っています。★無いときだけ FLOOR_LINE_Y に戻します。
 */
export function floorLineOf(item) {
  if (item && typeof item.floorY === "number") return item.floorY;
  return FLOOR_LINE_Y;
}

/** ★その絵の、下端から床の線までの割合（★置くときに使います）。 */
export function floorOffsetRatio(item) {
  const h = item && item.size ? item.size[1] : FURNITURE_CANVAS;
  return (h - floorLineOf(item)) / h;
}

/**
 * ★大きさの単位（★2026-09-08・坂本さんのご指摘）。
 *
 *   ★★「窓枠・庭・景色の大きさが合わない」とご指摘をいただきました。
 *     ★そのとおりでした。★分類ごとに、ばらばらの％を決め打ちしていました。
 *       家具22％　昭和22％　庭15％　壁かけ12％　扉16％　窓26％
 *     ★★どれも同じ 320 の絵なのに、★庭だけ小さく出していました。
 *   ★★だから、★1画素あたりの大きさを、★1つに決めます。
 *     ★絵の幅 × この単位 ＝ 部屋に対する％。
 *     ★320 の絵は 22％、★384 の窓枠は 26.4％、★512 の扉も 320幅なので 22％。
 *   ★★これで、★同じ大きさで描かれたものは、★同じ大きさに出ます。
 */
export const WIDTH_PER_CANVAS_PX = 22 / 320;

/**
 * ★置き場所の種類（★2026-09-08・Opus の placement-120.json）。
 *
 *   ★★9種類あります。★こちらでは決めません。★名簿が正です。
 *     ceiling　 天井から下げる　　7点
 *     wall　　　壁に掛ける　　　 13点
 *     tabletop　台の上に置く　　　3点
 *     floor　　 床に置く　　　　 35点
 *     structure 建物の一部　　　　1点
 *     outside　 外（★いちばん奥）14点
 *     opening　 窓枠・扉　　　　 25点
 *     view　　　窓の外の景色　　 13点
 *     surface　 壁材・床材　　　　9点
 *
 *   ★★2026-09-08 まで、★天井のものも 壁のものも、★床に置いていました。
 *     ★名簿に、その分けが 無かったためです。
 *     ★実機で「電球が床に置かれる」とご指摘をいただきました。
 */
export const PLACEMENTS = Object.freeze([
  "outside", "surface", "view", "opening", "wall", "structure",
  "floor", "tabletop", "ceiling"
]);

/**
 * ★基準点（★どこを合わせて置くか）。
 *
 *   ★★Opus の決め
 *     floor / tabletop / outside … bottom-center（★足もと）
 *     ceiling　　　　　　　　　　… top-center（★上端）
 *     wall / opening / view　　　… center（★まん中）
 *     surface　　　　　　　　　　… tile（★敷き詰め）
 */
export const ANCHORS = Object.freeze({
  floor: "bottom-center", tabletop: "bottom-center", outside: "bottom-center",
  ceiling: "top-center",
  wall: "center", opening: "center", view: "center",
  structure: "bottom-center", surface: "tile"
});

export function placementOf(item) {
  return (item && item.placement) || "floor";
}

export function anchorOf(item) {
  if (item && item.anchor) return item.anchor;
  return ANCHORS[placementOf(item)] || "bottom-center";
}

/**
 * ★重ね順（★小さい順に描く・Opus の決め）。
 *
 *   ★★羊は floor と同じ帯（70）に入れ、★足もとの y で 前後を決めます。
 *     ★だから、★羊の z は 決め打ちに しません（★13番）。
 */
export const SHEEP_Z_BAND = 70;

export function zOf(item) {
  return (item && typeof item.z === "number") ? item.z : 70;
}

/**
 * ★実際に使う重ね順。
 *
 *   ★★Opus の決め ── 羊は floor と同じ帯（70）に入れ、
 *     ★★足もとの y で 前後を決めます。
 *
 *   ★★だから、★帯の中に「足もとのぶん」の幅が要ります。
 *     ★z を100倍し、★足もとの％（0〜100）を足します。
 *       outside 500 ／ 壁材 1000 ／ view 2000 ／ opening 3000 ／ wall 4000
 *       structure 6000 ／ ★floor・tabletop・羊 7000＋足もと ／ ceiling 9000
 *   ★★これで、★帯の順は そのまま、★床の中だけで 前後が付きます。
 *
 *   ★★2026-09-08 まで、★羊は z=6 の決め打ちでした。
 *     ★窓が z=30 なので、★羊が 窓の後ろに隠れていました。
 *     ★実機でご報告をいただきました。★そのとおりです。
 *
 *   @param band    帯（★zOf の値）
 *   @param feetPct 足もとの高さ（★部屋の上からの％。★無ければ 0）
 */
export function zIndexOf(band, feetPct) {
  const b = typeof band === "number" ? band : 70;
  const f = typeof feetPct === "number" && Number.isFinite(feetPct)
    ? Math.max(0, Math.min(100, feetPct)) : 0;
  return b * 100 + Math.round(f);
}

/**
 * ★場面の いちばん上（★2026-09-08 夜）。
 *
 *   ★★場面の上に乗せる操作ボタン（★「うごかす」など）は、
 *     ★★場面のどの品物よりも 前に出ていなければ なりません。
 *
 *   ★★もとは CharacterHome に UI_CHROME_Z = 10 と 書いてありました。
 *     ★そこには「場面の要素は front(6) を超えてはいけない」と ありました。
 *     ★★内装を入れたときに、★その約束が こわれました。
 *       ★内装の帯は 500〜9100、★羊は 7000台です。
 *     ★★だから「うごかす」が、★床材や壁材の 後ろに 隠れていました。
 *       ★押せてはいましたが（★内装は pointerEvents:none）、★見えません。
 *
 *   ★★数を、決め打ちしません。★荷物の帯から 数えます。
 *     ★新しい帯が届いても、★ここが 勝手に 追いつきます。
 */
const MAX_BAND = INTERIOR_ITEMS.reduce(
  (m, i) => Math.max(m, zOf(i)), SHEEP_Z_BAND
);
export const SCENE_MAX_Z = zIndexOf(MAX_BAND, 100);
export const UI_CHROME_Z = SCENE_MAX_Z + 100;

/**
 * ★羊が歩く範囲（★2026-09-08・坂本さんのご指摘）。
 *
 *   ★★もとは 左右 50±18（★32〜68）でした。★画面の まん中3分の1だけです。
 *     ★実機で「まん中しか歩かない」とご指摘をいただきました。★そのとおりです。
 *
 *   ★★端まで歩けるようにします。★ただし、★壁にめり込ませないこと。
 *     ★羊は 部屋の幅の およそ19％。★まん中で置くので、半分の 9.5％が はみ出ます。
 *     ★だから、★左右に 12％ ずつ 残します。
 *
 *   ★★縦は、★床の帯（67〜97）の中で 動かします。
 *     ★★これで、★手前と奥が 生まれます。★重ね順が、足もとの y で 決まるためです。
 */
/**
 * ★羊の大きさ（★2026-09-08 夕・坂本さんのご要望）。
 *
 *   ★★「小さすぎる。★あつ森のように、もっと大きく」とのことでした。
 *   ★★92 → 140。★部屋の幅の およそ29％です。
 *     ★★これ以上 大きくすると、★歩ける幅が 狭くなります。
 *       ★羊が広いほど、★壁に めり込まない範囲が 減るためです。
 *   ★歩ける範囲（下）と、★合わせて 決めてあります。
 */
export const SHEEP_SIZE = 140;

/**
 * ★羊の 大きさ（★2026-09-08 夜・坂本さんの お決め「案A」）。
 *
 *   ★★もとは 140px の 決め打ちでした。★2つ、まずいことが ありました。
 *
 *   ★① 家具は ％、★羊だけ 画素でした。
 *     ★だから 釣り合いが、★機種ごとに 変わっていました。
 *       広い画面 20.0％（家具より細い）／狭い画面 38.9％（家具の1.8倍）。
 *     ★家具は 22.0％ です。★羊が 29.2％ では、★ソファより 羊が 太い、
 *     ★という 並びに なっていました。
 *
 *   ★② 背の高い かぶりものの 先が、★切れていました。
 *     ★羊の箱は 140×1.15＝161px。★部屋が 短い画面では、
 *     ★★箱の上が 部屋の外に出て、★#room-anchor の overflow:hidden が
 *     ★切っていました（★政宗の兜は 絵の いちばん上まで 中身があります）。
 *
 *   ★★％に すると、★どちらも 消えます。
 *     ★箱の高さは「部屋の幅の◯％」＝ 部屋の高さの ◯×4/3 ％。
 *     ★26％なら 34.7％。★足もと 70％ から 引いても 35.3％ 残ります。
 *     ★★どの機種でも、★上に はみ出しません。
 */
export const SHEEP_WIDTH_PCT = 26;

// ★★見本 A07 の 寸法（★枠 330：452 ／ 羊 230px）を、★ここに 置いていました。
//   ★★2026-09-10、★坂本さんの お決めで 外しました ──
//     「★ひつじの 画面の 羊が 3倍の 大きさに なっています。★元に 戻してください」
//   ★★見本の 取りちがえでは ありませんでした。★A07 は たしかに 230px です。
//     ★私が、★列の 幅では なく 枠の 高さに 掛けていました（★331px）。
//   ★★そして A07 の 絵は「羊を ながめる」1枚です。
//     ★アプリの 部屋は、★羊が 歩き、★家具が 置いてあります。
//     ★70％の 羊は、★歩く 場所を 残しません。
//   ★見本の 数が また 要るときは、★A07 の HTML から 取ってください。

/** ★部屋の幅（画素）から、★羊の 大きさ（画素）を 出します。 */
export function sheepSizePx(roomWidthPx) {
  const w = (typeof roomWidthPx === "number" && roomWidthPx > 0) ? roomWidthPx : 0;
  if (!w) return 0;
  return w * SHEEP_WIDTH_PCT / 100;
}

/**
 * ★座れる 内装（★2026-09-08 夜・実機「羊が 歩くだけ」）。
 *
 *   ★★羊は もともと、★椅子に すわり、★寝台で 眠ります。
 *     ★けれど その判定が、★古い家具の鍵（furniture_chair / furniture_bed）を
 *     ★★見ていました。★門の中の方には、★古い79点を 隠しています。
 *     ★だから 椅子も 寝台も「置いていない」ことに なり、
 *     ★★すわることも 眠ることも、★1度も 起きませんでした。
 *   ★★新しい内装の 鍵に つなぎ直します。
 *
 *   ★★寝台は、★新しい内装に ありません（★数えました）。
 *     ★だから 眠るのは、★寝台が 届くまで お預けです。
 *     ★★無いものを 指して「眠ります」と 書かないこと。
 */
export const SEAT_KEYS = Object.freeze([
  "furniture_04",  // ★革の大きなソファ
  "furniture_07",  // ★直線の黒いソファ
  "furniture_10",  // ★曲線の木脚チェア
  "furniture_13",  // ★ラタンのソファ
  "furniture_19",  // ★花柄のソファ
  "furniture_22",  // ★低いラウンジチェア
  "furniture_25",  // ★座椅子
  "showa_12"       // ★座卓と座布団
]);

/** ★寝られる 内装。★いまは 1点も ありません（★届いたら 足します）。 */
export const BED_KEYS = Object.freeze([]);

/**
 * ★置いている中から、★座れるものを 1つ。
 *
 *   ★★いくつ 置いていても、★1つに 決めます。★並びの はじめのものです。
 *     ★運を 使いません。★同じ部屋なら 同じ椅子です。
 *
 *   @param placed    置いている鍵の 並び
 *   @param positions {鍵: {left, top}}（★動かした ぶん）
 */
export function seatPos(equipped, positions) {
  // ★★入れ物ではなく、★平らにした 並びで 見ます（★2026-09-08 夜の直し）。
  //   ★★new Set(入れ物) は 落ちます。★「is not iterable」です。
  return firstPlaced(SEAT_KEYS, placedKeys(equipped), positions);
}

/** ★置いている中から、★寝られるものを 1つ。 */
export function bedPos(equipped, positions) {
  return firstPlaced(BED_KEYS, placedKeys(equipped), positions);
}

function firstPlaced(keys, placed, positions) {
  const on = new Set(placed || []);
  for (const k of keys) {
    if (!on.has(k)) continue;
    const p = (positions || {})[k];
    if (p && typeof p.left === "number") {
      return { left: p.left, top: typeof p.top === "number" ? p.top : FLOOR_BAND[1] };
    }
    // ★★動かしていなければ、★既定の場所です。
    //   ★★floorY は 絵の中の 画素です（★0〜1024）。★％では ありません。
    //     ★そのまま 渡すと、★羊が 部屋の外へ 行きます（★301％ など）。
    //   ★★％で 受け取る所へ 渡すので、★％に 直します。
    const it = interiorItemByKey(k);
    const raw = it && typeof it.floorY === "number" ? it.floorY : null;
    const pct = raw === null ? FLOOR_BAND[1]
      : (raw > 100 ? Math.round(raw / 1024 * 100) : raw);
    return {
      left: 50,
      // ★★床の帯から 出さないこと。
      top: Math.min(FLOOR_BAND[1], Math.max(FLOOR_BAND[0], pct))
    };
  }
  return null;
}

/**
 * ★歩く 速さと 間（★2026-09-09・坂本さんの ご承認）。
 *
 *   ★★「速い」の 正体は、★歩く速さより「★休まないこと」でした。
 *     ★もとは 2.2秒 歩いて、★1秒ほどで また 歩き出していました。
 *     ★★歩いている割合が 61％。★ほとんど 歩いています。
 *   ★★いまは 3.2秒 歩いて、★4〜9秒 休みます。★割合は 33％です。
 *     ★止まっているあいだに、★しぐさ と ひとりごとが 入ります。
 *
 *   ★★CSS の 移りの 秒数と、★かならず 同じ数に すること。
 *     ★ずれると、★着く前に 止まったり、★着いてから 動いたりします。
 */
export const WALK_MS = 3200;
export const WALK_REST_MIN_MS = 4000;
export const WALK_REST_MAX_MS = 9000;

/** ★つぎに 歩き出すまでの 間（ミリ秒）。 */
/**
 * ★椅子へ 行く 頻度（★2026-09-09・実機「座りに行きすぎ」）。
 *
 *   ★★もとは 20〜40秒に1回でした。★歩くのを 3.2秒に 遅くしたので、
 *     ★座っている 時間の 割合が 増えて 見えます。
 *   ★★60〜150秒に1回に します。
 */
export const SIT_MIN_MS = 60000;
export const SIT_MAX_MS = 150000;

/**
 * ★前後が 入れ替わる 頃合い（★2026-09-09・実機「唐突に 出てくる」）。
 *
 *   ★★重ね順は、★足もとの y から 出しています（★sheepZIndex）。
 *     ★★ところが y は、★動き出す 瞬間に 変わります。
 *       ★体は そのあと 3.2秒 かけて 進みます。
 *     ★★だから、★まだ 動いていないうちに 前後が 入れ替わっていました。
 *       ★置物の 後ろから 前へ、★急に 出てくるように 見えます。
 *
 *   ★★歩きの 半ばで 入れ替えます。
 *     ★ぶつかる あたりで 入れ替わるので、★自然に 見えます。
 *   ★★重ね順は「前か 後ろか」の 2つに 1つです。★間が ありません。
 *     ★だから「なめらかに」は、★入れ替える 時を 選ぶことです。
 */
export const Z_SWITCH_AT = 0.5;

/** ★入れ替えまでの 間（ミリ秒）。 */
export function zSwitchDelayMs() {
  return Math.round(WALK_MS * Z_SWITCH_AT);
}

export function nextSitMs(random) {
  const r = typeof random === "function" ? random() : Math.random();
  return SIT_MIN_MS + r * (SIT_MAX_MS - SIT_MIN_MS);
}

export function nextWalkRestMs(random) {
  const r = typeof random === "function" ? random() : Math.random();
  return WALK_REST_MIN_MS + r * (WALK_REST_MAX_MS - WALK_REST_MIN_MS);
}

export const SHEEP_WANDER = Object.freeze({
  centerLeft: 50,
  rangeLeft: 34,     // ★16〜84。★左右の端まで（★羊が140なので、少し内側）
  centerTop: 80,
  rangeTop: 10       // ★70〜90。★床の帯の中
});

/** ★羊の重ね順。★床のものと、同じ帯・同じ物差しです。 */
export function sheepZIndex(feetPct) {
  return zIndexOf(SHEEP_Z_BAND, feetPct);
}

/**
 * ★置く高さ（★部屋の上からの％）。
 *
 *   ★★天井のものは 上、★壁のものは 壁の帯、★床のものは 床の線。
 *   ★★これまで、★どれも 床に置いていました。
 */
export const CEILING_TOP_PCT = 2;      // ★天井から下げるものの、上端
export const WALL_CENTER_PCT = 30;     // ★壁に掛けるものの、まん中
export const TABLETOP_FEET_PCT = 74;   // ★台の上に置くものの、足もと
export const OUTSIDE_FEET_PCT = 66;    // ★外のものの、足もと（★床の線）

/**
 * ★置いてよい高さの帯（★部屋の上からの％）。
 *
 *   ★★2026-09-08、★一度でも動かすと、★家具が宙に浮きました。
 *     ★置き場所は「top が入っていないときだけ、床の線から出す」形でした。
 *     ★動かすと top が入るので、★床の線を見なくなっていました。
 *     ★実機で「置きかたが むずかしい」とご報告をいただきました。
 *
 *   ★★だから、★落とした先を、★帯の中に収めます。
 *     ★床のものは、★足もとが「床の帯」に入るところまで。
 *     ★壁のものは、★「壁の帯」の中まで。
 *   ★★これで、★もう浮きません。★床から離れることが、ありません。
 *
 *   ★床は、部屋の下から42％（★CharacterHome の床の高さと同じ）。
 */
/**
 * ★床の 高さ（★部屋の 下から 何％か）。
 *
 *   ★★2026-09-13、★42 → **48** に しました（★坂本さんの お決め）。
 *     ★「壁の 高さは そのまま、★床を 実用の 範囲で 最大限 広げる」。
 *   ★★境目（★壁と 床の 線）は、★上から 100 − 48 ＝ **52％** に なります。
 *
 *   ★★これ 1つで 決めます。★2026-09-13 まで、★3か所に 書いて ありました ──
 *     ★InteriorLayer の ROOM_FLOOR_BOTTOM_PCT
 *     ★CharacterHome の `height: "42%"`（★床の 色）
 *     ★CharacterHome の `bottom:"22%", height:"58%"`（★庭の 包み）
 *   ★★片方だけ 直す 日が 来ます。★1か所に します。
 */
/**
 * ★掴める ところを、★見た目より どれだけ 広げるか（★px・2026-09-14）。
 *
 *   ★★坂本さん（実機）──「★家具の 当たり判定が、★見た目より 狭い」。
 *
 *   ★★読んで 分かった こと。
 *     ★掴める ところ ＝ `DraggableItem` の 箱 そのもの です。
 *     ★★ところが、★「うごかす」の あいだ 出る 点線は `inset: -4`。
 *       ★★点線は 箱より ★4px 外側に あります。
 *       ★★つまり「見えて いる 枠の 内側を 押した のに 掴めない」帯が、
 *         ★まわり 4px、★幅に して 8px あります。
 *     ★★指の 腹は 8〜10mm あります。★4px の ずれは、★外し続けます。
 *
 *   ★★だから、★掴める ところを 点線の **外** まで 広げます。
 *     ★★見えて いる 枠の 中は、★どこを 押しても 掴めます。
 *   ★★絵は 1つも 動きません。★置き場所も 変わりません。
 *     ★広げるのは、★透明な 当たり だけ です。
 *
 *   ★★点線と 当たりは、★同じ ここから 出します。
 *     ★別々に 書くと、★片方だけ 変わって また ずれます。
 */
export const GRAB_PAD_PX = 12;

/** ★「うごかす」の あいだ 出る 点線の、★外への はみ出し（★px）。 */
export const EDIT_OUTLINE_INSET_PX = 4;

// ★★★2026-09-17、★48 → 56 に しました（★坂本さんの お決め ㋓・小さく 動かす）。
//   ★★はじめの お指図は「壁 1.5倍・床 2倍」でした。
//     ★★52×1.5 ＝ 78 ／ 48×2 ＝ 96 ── ★合計 174。★舞台は 100 です。
//     ★★成り立たない ので、★お決めが ㋓（★小さく 動かして 見る）に なりました。
//   ★★★床を 広げれば、★壁は **必ず** 低く なります。★合計は 100 です。
//     ★壁 52 → **44**。★「壁は 変えない」は、★数の うえで できません。
// ★★★2026-09-18、★56 → 48 に **戻しました**。
//   ★★床を 広げる 話は、★坂本さんの お指図で 保留に なりました。
//   ★★本当の お困りごとは 別 でした ── ★壁の ものが 縦に 動ける 範囲 です。
//     ★★`WALL_BAND` を 広げる だけ で 済みます。★部屋は 触りません。
//   ★★一緒に 入れた もの（数どうしの 決まり・庭の 切り離し）は 残します。
export const FLOOR_BOTTOM_PCT = 48;

/** ★壁と 床の 境目（★上から 何％か）。 */
export const WALL_HEIGHT_PCT = 100 - FLOOR_BOTTOM_PCT;

/**
 * ★床の 線（★舞台の 上から 数えた ％）。★壁と 床の 境目 です。
 *
 *   ★★`WALL_HEIGHT_PCT` と 同じ 数 です。★けれど 意味が ちがいます ──
 *     ★`WALL_HEIGHT_PCT` … ★壁の **高さ**
 *     ★`FLOOR_TOP_PCT`   … ★上から 数えた **線の 位置**
 *   ★★帯の 上限に 使う ときは、★こちらの 名で 書きます。★読んで 分かる ため。
 */
export const FLOOR_TOP_PCT = 100 - FLOOR_BOTTOM_PCT;

// ★★床の 線が 上がった ぶん、★置ける 帯も 広げます（★2026-09-17）。
//   ★★床は 上から 44％ から 下 です。★60 から では、★上の 16％ が 遊びます。
//   ★★広げる だけ です。★いま 置いて ある もの（60〜97）は 1つも 動きません。
// ★★上端は 60 の まま です（★2026-09-18・床を 戻した ため）。
// ★★★下端を 97 → 98 に しました（★2026-09-17）。
//   ★★見張り（`room-numbers`）が 見つけました ──
//     ★家具の 既定の 足もとは **98** です。★帯の 外に 出て いました。
//   ★★きょう 動かす 前 から そう でした（★60〜97 と 98）。
//     ★★別の 仕組み（★いまの101点 と 内装249点）の 数 だから、
//       ★誰も 突き合わせて いません でした。
//   ★★1 広げます。★いま 置いて ある ものは 1つも 動きません。
export const FLOOR_BAND = Object.freeze([60, 98]);   // ★足もとの高さ（％）
/**
 * ★壁の ものの 上端（％）。
 *
 *   ★★2026-09-13、★[5, 50] → **[5, 40]** に しました。
 *     ★★床を 広げた ぶん、★境目が 58％ → 52％ に 上がります。
 *     ★★上端 50％の ままだと、★壁の ものが 床に かかります。
 *   ★★境目までの ゆとりは 12％。★前は 8％ でした。★狭く なって いません。
 *
 *   ★★すでに 置いて ある ものは、★**書き換えません**。
 *     ★`clampToBand` は 描く ときだけ 効きます。★しまった 値は そのまま。
 *     ★★ただし、★上端が 40％より 下に あった ものは、★見た目が 上に 動きます。
 */
// ★★★2026-09-18、[5, 40] → [0, 床の 線] に 広げました（★坂本さんの お決め ㋐）。
//
//   ★★お困りごとは「★壁の ものが 縦に 動ける 範囲が 狭い」でした。
//     ★★壁は 0〜52％ です。★けれど 動かせるのは 5〜40％ ── ★35 ぶん だけ。
//       ★★上の 5 と、★下の 12 が 使えません でした。
//       ★★下の 12 は、★床の すぐ 上。★目の 高さの あたり です。
//   ★★壁ぜんぶ に すると 52 ぶん。★★35 の **1.49倍** です。
//     ★★坂本さんの「★壁を 1.5倍」は、★これ だった と 思われます。
//
//   ★★★数を 書き写しません。★床の 線から 引きます。
//     ★★いつか 床を 動かす 日が 来ても、★ひとりでに ついて きます。
//
//   ★★いま 掛けて ある ものは 1つも 動きません。
//     ★★`clampToBand` は **挟む だけ** です。★広げれば 挟まれなく なる だけ。
//     ★★坂本さんの 9点は すべて 5〜40 の 中に あります。
//
//   ★★★床に かかる ことを 禁じません（★お決め）。
//     ★★壁の ものは `top` が **上の 端** です。★絵は 320×320。
//       ★`top = 40`（★これまでの 下限）でも、★下の 端は 約 60％ ──
//       ★★**もとから 床に かかって いました**。
//     ★★裁定 その71 の 追補と 同じ 考え方 です ──
//       ★「置けなく する」のでは なく、★「置ける ように して、★お客さまが 決める」。
export const WALL_BAND = Object.freeze([0, FLOOR_TOP_PCT]);

export function clampToBand(v, band) {
  const [lo, hi] = band;
  if (typeof v !== "number" || !Number.isFinite(v)) return null;
  return Math.max(lo, Math.min(hi, v));
}

/** ★左右も、部屋の外へ出しません。★出ると、二度と掴めません。 */
export const LEFT_BAND = Object.freeze([6, 94]);

/** ★その絵の、部屋に対する幅（％）。★画面で決め打ちしないこと。 */
/**
 * ★床に置くものだけ、★少し 大きくします（★2026-09-09・実機のご報告）。
 *
 *   ★★「家具が、羊にくらべて 小さく見える」。★測ったら、そのとおりでした。
 *
 *   ★★見た目の幅は「絵の枠」では なく「中身」で 決まります。
 *     ★羊の体　　枠の 56.4％ しか 使っていません（★実測）
 *     ★大ソファ　枠の 79.7％　／　チェア 45.6％　／　座卓 100％
 *   ★★だから、★枠の幅（22％）を くらべても 意味が ありません。
 *
 *   ★★室の幅に対する「中身」の幅（★直す前）
 *     ★羊の体　　16.9％（★26％ × 1.15 × 0.564）
 *     ★大ソファ　17.5％　→ ★羊と ほぼ 同じ
 *     ★チェア　　10.0％　→ ★羊の 6割。★すわる椅子が 羊より 小さい
 *
 *   ★★実物の 目安では、★2人がけのソファは 羊の 1.5倍ほどです。
 *     ★だから 1.35倍に します。★大ソファ 23.6％（羊の 1.40倍）。
 *     ★★1.5倍ぴったりには しません。★部屋が 狭く見えます。
 *
 *   ★★窓・扉・壁・天井・床壁のタイルは、★1つも 変えません。
 *     ★★窓は 枠の穴と 景色が 合っています。★動かすと ずれます。
 *     ★タイルは 敷きつめるものです。★大きさに 意味が ありません。
 *   ★★変えるのは、★床に置くもの・机の上・柱だけです（★39点）。
 */
export const FLOOR_ITEM_SCALE = 1.35;

/** ★大きくしてよい 置き場所。★ここに 無いものは、1つも 変えません。 */
export const SCALED_PLACEMENTS = Object.freeze(["floor", "tabletop", "structure"]);

export function widthPctOf(item) {
  const w = item && item.size ? item.size[0] : FURNITURE_CANVAS;
  const base = w * WIDTH_PER_CANVAS_PX;
  const place = item && item.placement;
  return SCALED_PLACEMENTS.includes(place) ? base * FLOOR_ITEM_SCALE : base;
}

/**
 * ★右の壁に、ぴったり寄せる left（％）。
 *
 *   ★★2026-09-08、★扉が「壁に接していない」とご指摘をいただきました。
 *     ★★縦は合っていました（★実測 0.00％のずれ）。★横でした。
 *     ★扉の絵は 320 幅で、★中身は x14〜306。★右に13画素の余白があります。
 *     ★★left=89％ だと、★中身の右端は 99.11％。★0.89％ 内側に浮きます。
 *   ★★だから、★絵の端ではなく、★中身の端を、壁に合わせます。
 *     ★中身の範囲は、★名簿に入れました（contentX）。★1点ずつ実測した値です。
 */
/**
 * ★掴みどころ（★2026-09-09・実機「当たり判定が 広すぎる」）。
 *
 *   ★★絵の枠のうち、★中身が どこに あるかを 割合で 返します。
 *     ★{ left, right, top, bottom }。★どれも「枠の 何％を 空けるか」です。
 *
 *   ★★床に置くものは、★枠の 横75〜82％・縦24〜60％ しか 使っていません。
 *     ★★のこりは 透けています。★そこまで 掴めると、となりと 重なります。
 *   ★★中身の範囲は 名簿が 持ちます（contentX / contentY）。
 *     ★1点ずつ 実測した 値です（★2026-09-09）。
 *   ★★無い品は、★枠ぜんたいを 返します。★掴めなくなるよりは ましです。
 */
export function hitInsetOf(item) {
  const none = { left: 0, right: 0, top: 0, bottom: 0 };
  if (!item) return none;
  const w = item.size ? item.size[0] : FURNITURE_CANVAS;
  const h = item.size ? (item.size[1] || item.size[0]) : FURNITURE_CANVAS;
  const cx = Array.isArray(item.contentX) ? item.contentX : null;
  const cy = Array.isArray(item.contentY) ? item.contentY : null;
  if (!cx && !cy) return none;
  return {
    left: cx ? (cx[0] / w) * 100 : 0,
    right: cx ? ((w - cx[1]) / w) * 100 : 0,
    top: cy ? (cy[0] / h) * 100 : 0,
    bottom: cy ? ((h - cy[1]) / h) * 100 : 0
  };
}

export function flushRightLeftPct(item) {
  const w = widthPctOf(item);
  const cw = item && item.size ? item.size[0] : FURNITURE_CANVAS;
  const cx = item && Array.isArray(item.contentX) ? item.contentX : [0, cw];
  // ★中身の右端が 100％ に来る left。★中心で置くので、幅の半分を足します。
  return 100 - (cx[1] / cw) * w + w / 2;
}

// ---------------------------------------------------------------------------
// ★窓（★枠 ＋ 景色 ＝ 169通り）
// ---------------------------------------------------------------------------

/**
 * ★窓の重ね順。
 *
 *   ★★景色を先に描き、★枠を上に重ねます。
 *     ★枠の中は抜いてあるので、★景色が透けます。
 *   ★★逆にすると、★枠が景色に隠れます。
 */
export const WINDOW_LAYER_ORDER = Object.freeze(["view", "window"]);

export function windowFrames() {
  return itemsByCategory("window");
}

export function windowViews() {
  return itemsByCategory("view");
}

/**
 * ★いま選んでいる窓の、2枚。
 *
 *   ★★2026-09-08、★決めが変わりました（坂本さんの決め）。
 *     ★もとは「片方だけでも出す」でした。
 *     ★★ですが、★枠は 384×384、★景色は 480×320 で、★縦横比が違います。
 *       ★片方だけ置くと、★大きさが合わず、★おかしく見えます。
 *     ★★だから、★2枚そろって、はじめて1つの窓とします。
 *       ★片方だけのときは、★1枚も出しません。
 *
 * @param {object} chosen  { window: 鍵, view: 鍵 }
 */
/**
 * ★窓の既定（★2026-09-08・坂本さんの決め）。
 *
 *   ★★もとは「2枚そろわなければ、1枚も出さない」でした。
 *     ★★片方だけ選ぶと、★窓が消えます。★壁に穴が空いたように見えます。
 *   ★★だから、★いつも1組（枠1つ・景色1つ）にします。
 *     ★選んでいないほうは、★既定で埋めます。
 *     ★★「何も無い」を作らないこと。★おうちには、窓が要ります。
 */
export const DEFAULT_WINDOW = "window_01";
export const DEFAULT_VIEW = "view_01";

export function windowLayers(chosen) {
  const c = chosen || {};
  // ★★選んでいないほうは、★既定で埋めます。★片方だけにしません。
  const frame = interiorItemByKey(c.window) || interiorItemByKey(DEFAULT_WINDOW);
  const view = interiorItemByKey(c.view) || interiorItemByKey(DEFAULT_VIEW);
  if (!frame || !view) return [];
  return WINDOW_LAYER_ORDER.map((k) => (k === "window" ? frame : view));
}

/**
 * ★窓が、そろっているか。
 *   ★★画面が「あと片方です」と伝えるために使います。★黙らないためです。
 */
export function windowReady(chosen) {
  // ★★いつも1組です（★既定で埋めます）。★そろっていない状態は、ありません。
  return windowLayers(chosen).length === 2;
}

/**
 * ★いま出ている窓の、枠と景色。
 *   ★★選んでいなくても、★既定が返ります。★画面の印に使います。
 */
export function currentWindow(chosen) {
  const c = chosen || {};
  return {
    window: c.window || DEFAULT_WINDOW,
    view: c.view || DEFAULT_VIEW
  };
}

/**
 * ★枠の「抜けているところ」（★穴）。★1枚ずつ実測しました。
 *
 *   ★★2026-09-08、★どの枠も 74% だとして景色を置いていました。
 *     ★実測すると、★13枚とも違いました。
 *
 *       ふつうの枠　　0.805〜0.867（★ほぼ正方形・まん中）
 *       window_09 　　0.617（★小さい丸窓）
 *       window_11 　　幅0.867 × ★高0.302（★上から0.633 の、低くて横長）
 *       window_10 　　★穴ではなく、すりガラス（半透明）でした
 *
 *   ★★だから「割合を1つ」では、★合うはずがありませんでした。
 *     ★実機で「枠と景色が、まるで合っていない」とご指摘をいただきました。
 *
 *   ★★いまは、★穴の四角に、★景色を切り抜いてはめます。
 *     ★[左, 上, 幅, 高] を、★絵の大きさに対する割合で持ちます。
 *     ★景色は 480×320、★穴は正方形のこともあるので、
 *       ★★引き伸ばさず、★はみ出しを切ります（★cover）。
 *       ★引き伸ばすと、★空も山も ゆがみます。
 *
 *   ★★名簿に入っています。★画面で数を書かないこと。
 */
/**
 * ★枠の「抜けているところ」だけを、白にした型（★2026-09-08 の直し）。
 *
 *   ★★もとは「穴の四角」で切っていました。
 *     ★丸い枠の穴を測ると、★四角い範囲が返ります。
 *     ★その四角で切ると、★四隅に景色が残り、★丸の外へ はみ出しました。
 *   ★★枠の絵そのものを型にするのも、★誤りでした。
 *     ★枠の外側も 透けているので、★そこにも 景色が出ます。
 *   ★★だから、★「外から届かない、透けているところ」だけを 白にした
 *     ★型を、★13枚 作りました。★これで、どんな形でも はみ出しません。
 */
export function windowHoleMask(item) {
  return item && item.holeMask ? INTERIOR_BASE + item.holeMask : null;
}

export function windowHole(item) {
  const h = item && Array.isArray(item.hole) ? item.hole : null;
  // ★★穴が分からない枠は、★まん中に控えめに置きます。★消しません。
  return h && h.length === 4 ? h : [0.09, 0.09, 0.82, 0.82];
}

/** ★すりガラスか（★穴ではなく、半透明で透ける枠）。 */
export function isFrostedWindow(item) {
  return !!(item && item.frosted);
}

/** ★組み合わせの数。★画面には出しません（★数を見せない決めのため）。 */
export function windowCombinationCount() {
  return windowFrames().length * windowViews().length;
}

// ---------------------------------------------------------------------------
// ★置き場所（★1つの分類につき、1つだけ置けるもの）
// ---------------------------------------------------------------------------

/**
 * ★1つだけ置ける分類。
 *
 *   ★窓枠・窓の外・扉・壁と床は、★部屋に1つです。
 *   ★家具・庭・壁かけは、★いくつでも置けます。
 */
export const SINGLE_SLOT_CATEGORIES = Object.freeze(["window", "view", "door", "wallTile", "floorTile"]);

/**
 * ★★壁のタイルと、床のタイルは、★別のものです（★2026-09-08 の直し）。
 *
 *   ★9点のうち、★壁が5点、★床が4点です（★manifest の style で分かれています）。
 *   ★★私は、これを1つの「tile」として扱い、★部屋ぜんぶに敷いていました。
 *     ★だから、★床の柄を選ぶと、★壁まで変わっていました。
 *   ★ここで分けます。★画面で判じないこと。
 */
export function tileSurface(item) {
  if (!item || item.category !== "tile") return null;
  if (item.style === "壁") return "wallTile";
  if (item.style === "床") return "floorTile";
  return null;
}

export function wallTiles() {
  return itemsByCategory("tile").filter((i) => tileSurface(i) === "wallTile");
}

export function floorTiles() {
  return itemsByCategory("tile").filter((i) => tileSurface(i) === "floorTile");
}

export function isSingleSlot(category) {
  return SINGLE_SLOT_CATEGORIES.includes(category);
}

/**
 * ★置いているものの形。
 *
 *   ★★character_equipped の中に持ちます（★列を、足しません）。
 *     ★着せかえの印と、同じ入れ物です。★保存の道も、もう在ります。
 *
 *     interior: {
 *       window: "window_03",        ← 1つだけ
 *       view:   "view_07",          ← 1つだけ
 *       door:   "door_02",          ← 1つだけ
 *       tile:   "tile_04",          ← 1つだけ
 *       furniture: ["furniture_01", …],   ← いくつでも
 *       garden:    ["garden_02", …],
 *       wallart:   ["wallart_05", …]
 *     }
 */
export function interiorOf(equipped) {
  const v = equipped && equipped.interior;
  return v && typeof v === "object" ? v : {};
}

/**
 * ★1つ置く・外す。
 *
 *   ★★1つだけの分類は、★置き換えます。★同じものを押したら、外します。
 *   ★いくつでも置ける分類は、★足す・引くです。
 *   ★★元の入れ物を書き替えません。★新しいものを返します。
 */
/**
 * ★いま 置いている 鍵を、★1本の 並びに します。
 *
 *   ★★interiorOf は「入れ物（object）」を 返します。★並び（array）では ありません。
 *     ★★2026-09-08 夜、★私は これを for...of で まわしました。
 *       ★★「is not iterable」で 落ちました。★実機で ご報告をいただきました。
 *     ★★入れ物の 形は、★1つだけ置くもの（wallTile／floorTile／window／view／door）と、
 *       ★いくつでも置くもの（furniture／showa／garden／wallart の 並び）が 混ざります。
 *   ★★だから、★平らにする所を、★ここ1つに します。★呼ぶ側で ほどきません。
 */
export const MANY_SLOTS = Object.freeze(["furniture", "showa", "garden", "wallart"]);
export const ONE_SLOTS = Object.freeze(["wallTile", "floorTile", "window", "view", "door"]);

export function placedKeys(equipped) {
  const placed = interiorOf(equipped);
  const out = [];
  for (const k of ONE_SLOTS) {
    if (typeof placed[k] === "string" && placed[k]) out.push(placed[k]);
  }
  for (const k of MANY_SLOTS) {
    const v = placed[k];
    if (Array.isArray(v)) for (const x of v) if (typeof x === "string" && x) out.push(x);
  }
  return [...new Set(out)];
}

export function toggleInterior(equipped, item) {
  if (!item || !item.category) return equipped || {};
  const cur = interiorOf(equipped);
  const c = item.category;
  // ★★タイルは、★壁と床を、★別の置き場所にします（★2026-09-08 の直し）。
  const c2 = c === "tile" ? tileSurface(item) : c;
  if (c2 && c2 !== c) {
    const next = { ...cur };
    if (next[c2] === item.key) delete next[c2];
    else next[c2] = item.key;
    return { ...(equipped || {}), interior: next };
  }
  if (isSingleSlot(c)) {
    const next = { ...cur };
    if (next[c] === item.key) delete next[c];
    else next[c] = item.key;
    return { ...(equipped || {}), interior: next };
  }
  const list = Array.isArray(cur[c]) ? cur[c] : [];
  const has = list.includes(item.key);
  return {
    ...(equipped || {}),
    interior: {
      ...cur,
      [c]: has ? list.filter((k) => k !== item.key) : [...list, item.key]
    }
  };
}

/**
 * ★動かせるものを、1つでも置いているか。
 *
 *   ★★2026-09-08、★「並べかえる」の押しどころが、
 *     ★旧い家具があるときだけ出ていました。
 *     ★★門の中では旧い家具が0点なので、★押しどころが出ず、
 *       ★新しい家具を、★1つも動かせませんでした。
 *     ★実機でご指摘をいただきました。★そのとおりでした。
 *   ★★動かせるのは、★床と壁に置くものです（★家具・昭和・庭・壁かけ）。
 *     ★窓・扉・タイルは、★置き場所が決まっています。
 */
export const MOVABLE_CATEGORIES = Object.freeze(["furniture", "showa", "garden", "wallart"]);

export function movableInteriorKeys(equipped) {
  const cur = interiorOf(equipped);
  return MOVABLE_CATEGORIES.flatMap((c) => (Array.isArray(cur[c]) ? cur[c] : []));
}

export function hasMovableInterior(equipped) {
  return movableInteriorKeys(equipped).length > 0;
}

/** ★いま置いているか。 */
export function isPlaced(equipped, item) {
  if (!item) return false;
  const cur = interiorOf(equipped);
  const surf = tileSurface(item);
  if (surf) return cur[surf] === item.key;
  if (isSingleSlot(item.category)) return cur[item.category] === item.key;
  const list = cur[item.category];
  return Array.isArray(list) && list.includes(item.key);
}

// ============================================================================
// ★㋕ ── ★「ながめる で 見える ところ」の 枠（★裁定 その71・追補・2026-09-17）
//
//   ★★字は ここが 持ちます。★画面に 書き写しません。
//   ★★裁定の 字 ── ★1文字も 変えて いません。
// ============================================================================

/** ★枠に 添える 1行（★裁定の とおり・2行 で 書かれて います）。 */
export const VIEW_BAND_NOTE = Object.freeze([
  "この 枠の 中が、ながめる で 見える ところです。",
  "外にも 置けます。"
]);

// ============================================================================
// ★部屋の 数どうしの 関わり（★2026-09-17・★「6段目」）
//
//   ★★きょうまで、★部屋の 数は **別々に** 置かれて いました ──
//     `FLOOR_BOTTOM_PCT` / `FLOOR_BAND` / `WALL_BAND` /
//     `SHEEP_WANDER` / `FURNITURE_FLOOR_TOP`
//   ★★「どれと どれが 揃って いる べきか」が、★どこにも 書かれて いません でした。
//   ★★だから、★1つだけ 動かすと **静かに** ずれます。
//     ★★2026-09-17、★床を 動かす とき、★まさに それが 起きかけました。
//
//   ★★ここに 書いて、★見張り（`components/tests/room-numbers.test.js`）に
//     ★守らせます。★次に 誰かが 1つだけ 動かしたら、★落ちます。
// ============================================================================

/**
 * ★数どうしの 決まり。★`true` で なければ なりません。
 *
 *   ★★見張りが これを 1つずつ 通します。★ここに 書いた ものだけ が 守られます。
 */
export const ROOM_NUMBER_RULES = Object.freeze([
  {
    name: "壁と 床を 足すと 100",
    ok: () => FLOOR_BOTTOM_PCT + WALL_HEIGHT_PCT === 100,
    why: "舞台は 100 です。片方を 広げれば、もう 片方は 必ず 狭く なります。"
  },
  {
    name: "足もとの 帯は、床の 上に ある",
    ok: () => FLOOR_BAND[0] >= FLOOR_TOP_PCT,
    why: "床の 線より 上に 足もとを 置くと、家具が 壁に 立ちます。"
  },
  {
    name: "足もとの 帯は、部屋の 中に ある",
    ok: () => FLOOR_BAND[1] <= 100 && FLOOR_BAND[0] < FLOOR_BAND[1],
    why: "部屋の 外に 置けては いけません。"
  },
  {
    name: "家具の 既定の 足もとは、帯の 中",
    ok: () => FURNITURE_FLOOR_TOP_PCT >= FLOOR_BAND[0]
      && FURNITURE_FLOOR_TOP_PCT <= FLOOR_BAND[1],
    why: "外れると、新しく 置いた ものだけ が 別の 高さに 出ます。"
  },
  {
    name: "羊は 床の 上を 歩く",
    ok: () => (SHEEP_WANDER.centerTop - SHEEP_WANDER.rangeTop) >= FLOOR_TOP_PCT
      && (SHEEP_WANDER.centerTop + SHEEP_WANDER.rangeTop) <= 100,
    why: "羊が 壁を 歩いたり、部屋の 外へ 出たり します。"
  },
  {
    name: "壁の ものの 帯は、壁の 中",
    ok: () => WALL_BAND[0] >= 0 && WALL_BAND[1] <= FLOOR_TOP_PCT,
    why: "壁の 線より 下に 掛けると、絵が 床に 埋まります。"
  },
  {
    name: "壁の ものの 帯は、壁ぜんぶ",
    ok: () => WALL_BAND[0] === 0 && WALL_BAND[1] === FLOOR_TOP_PCT,
    why: "上や 下が 使えないと、掛けたい ところに 掛けられません。"
      + " 2026-09-18 の お決めで、壁ぜんぶ に しました。"
  }
]);

/**
 * ★家具の 既定の 足もと。
 *
 *   ★★`components/CharacterHome.jsx` に `FURNITURE_FLOOR_TOP = 98` と
 *     ★直書きされて いました（★2026-09-17 に こちらへ 移しました）。
 *   ★★床の 数と 並べて 置かない と、★片方だけ 動きます。
 */
export const FURNITURE_FLOOR_TOP_PCT = 98;

/**
 * ★庭の 背景の 下端（★舞台の ％）。
 *
 *   ★★★床の 線とは **別の 数** です（★2026-09-17）。
 *     ★★`CharacterHome.jsx` に `FLOOR_BOTTOM_PCT - 26` と 書かれて いました。
 *     ★★`26` に 意味は ありません ── ★もとの 直書き `22%` に 戻る ように
 *       ★逆算して 置かれた 数 でした（★`c12441af`・2026-09-14）。
 *       ★★その とき 床は 42％ で、★42 − 26 は 16。★22 では ありません。
 *     ★★そのため、★床を 動かすと 庭の 背景も 一緒に 動いて いました。
 *       ★★結ぶ 理由が ありません でした。★ここで 切り離します。
 */
export const GARDEN_BACKDROP_BOTTOM_PCT = 22;
