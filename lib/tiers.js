...
/**
 * ★買い切りが、★いま 生きているか（★2026-09-09）。
 *
 *   ★★status が active で、★終わる日が まだ 来ていないこと。
 *   ★★status だけを 見ないこと。★期限が 過ぎても、
 *     ★誰かが 書き換えるまで active の ままです。
 *     ★★時計を 見るのは、★読むときです。
 *   ★★行は 消しません。★終わっても 残します。
 */
export function purchaseAlive(purchase, nowISO) {
  const p = purchase || {};
  if (p.status !== "active") return false;
  if (!p.ends_at) return false;
  const now = nowISO || new Date().toISOString();
  return now < p.ends_at;
}

/**
 * ★いま 効いている 段（★契約と 買い切りの、強いほう）。
 *
 *   ★★両方 お持ちの方が いらっしゃいます。
 *     ★★弱いほうで 上書きしないこと。★取り上げることに なります。
 *   ★★並びで 渡します。★1つでも 生きていれば、そちらを 見ます。
 */
export function effectiveTierWith(subscription, purchases, nowISO) {
  const rank = { [FREE]: 0, [BASIC]: 1, [FULL]: 2 };
  let best = effectiveTier(subscription);
  for (const p of (purchases || [])) {
    if (!purchaseAlive(p, nowISO)) continue;
    const t = TIERS.includes(p.tier) ? p.tier : FREE;
