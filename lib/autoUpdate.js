 ...
 *   ★★自動の ほうとは、★守りが ちがいます。
 *     ★自動　★迷ったら しません（★守り 4つ）
 *     ★手動　★ご本人が 選ばれました。★止めません
 *   ★★けれど、★消えるものが あるなら、★先に 言います。
 *     ★黙って 消さないこと。★それは 押した 方の 落ち度では ありません。
 *
 *   ★★Service Worker にも、★ついでに 聞きます。
 *     ★待っている 版が あれば、★それも 入れ替わります。
 *   ★★聞けなくても 進みます。★読み込み直すのが 目的です。
 */
export async function reloadNow(deps) {
  const d = deps || {};
  const nav = d.navigator || (typeof navigator !== "undefined" ? navigator : null);
  try {
    if (nav && nav.serviceWorker && nav.serviceWorker.getRegistration) {
      const reg = await nav.serviceWorker.getRegistration();
      if (reg && reg.update) await reg.update();
    }
  } catch (e) {
    // ★★聞けなくても かまいません。★読み込み直すのが 目的です。
  }
  const loc = d.location || (typeof window !== "undefined" ? window.location : null);
