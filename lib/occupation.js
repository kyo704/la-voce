...
// 登録画面のための、9言語の呼び名
//
//   ★OCCUPATION_LABELS（日本語）と、ここの ja は必ず同じ文字にします。
//     ずれると、登録画面とアプリの中で職業の呼び名が食い違います。
//     components/tests/signup-occupation.test.js が、その一致を見ています。
//
//   ★データの持ち主はこのファイルです。登録画面の側に11個を書き写さないこと。
//     書き写すと、職業を足したときに片方だけが古くなります。
// ============================================================================
export const OCCUPATION_LABELS_I18N = {
  classical:   { ja: "声楽・オペラ", en: "Classical / Opera", zh: "声乐・歌剧", it: "Canto lirico / Opera", de: "Klassischer Gesang / Oper", fr: "Chant classique / Opéra", es: "Canto lírico / Ópera", ko: "성악・오페라", ru: "Академический вокал / Опера" },
  musical:     { ja: "ミュージカル", en: "Musical Theatre", zh: "音乐剧", it: "Musical", de: "Musical", fr: "Comédie musicale", es: "Teatro musical", ko: "뮤지컬", ru: "Мюзикл" },
  pops:        { ja: "ポップス／ロック", en: "Pop / Rock", zh: "流行／摇滚", it: "Pop / Rock", de: "Pop / Rock", fr: "Pop / Rock", es: "Pop / Rock", ko: "팝／록", ru: "Поп / Рок" },
  voiceActor:  { ja: "声優", en: "Voice Actor", zh: "配音演员", it: "Doppiatore/Doppiatrice", de: "Synchronsprecher/in", fr: "Comédien(ne) de doublage", es: "Actor/Actriz de voz", ko: "성우", ru: "Актёр озвучивания" },
  narrator:    { ja: "ナレーター", en: "Narrator", zh: "旁白配音", it: "Narratore/Narratrice", de: "Sprecher/in", fr: "Narrateur/Narratrice", es: "Narrador(a)", ko: "내레이터", ru: "Диктор" },
  announcer:   { ja: "アナウンサー", en: "Announcer", zh: "播音员", it: "Annunciatore/Annunciatrice", de: "Moderator/in", fr: "Présentateur/Présentatrice", es: "Locutor(a)", ko: "아나운서", ru: "Ведущий(ая) эфира" },
  actorStage:  { ja: "俳優（舞台）", en: "Actor (Stage)", zh: "演员（舞台）", it: "Attore/Attrice (teatro)", de: "Schauspieler/in (Bühne)", fr: "Acteur/Actrice (théâtre)", es: "Actor/Actriz (teatro)", ko: "배우(무대)", ru: "Актёр (театр)" },
  actorScreen: { ja: "俳優（映像）", en: "Actor (Screen)", zh: "演员（影视）", it: "Attore/Attrice (schermo)", de: "Schauspieler/in (Film)", fr: "Acteur/Actrice (écran)", es: "Actor/Actriz (pantalla)", ko: "배우(영상)", ru: "Актёр (кино и ТВ)" },
  rakugo:      { ja: "落語・講談", en: "Rakugo / Kōdan", zh: "落语・讲谈", it: "Rakugo / Kōdan", de: "Rakugo / Kōdan", fr: "Rakugo / Kōdan", es: "Rakugo / Kōdan", ko: "라쿠고・고단", ru: "Ракуго / Кодан" },
  mc:          { ja: "司会・MC", en: "Host / MC", zh: "主持・MC", it: "Presentatore / MC", de: "Moderation / MC", fr: "Animateur / MC", es: "Presentador / MC", ko: "사회・MC", ru: "Ведущий / MC" },
  other:       { ja: "その他", en: "Other", zh: "其他", it: "Altro", de: "Sonstiges", fr: "Autre", es: "Otro", ko: "기타", ru: "Другое" }
};

/**
 * 職業の呼び名を、その言語で返す。
 * ★知らない言語や知らない職業でも、必ず何かを返します（日本語に落ちます）。
 *   登録画面で空の選択肢が出ると、選べなくなるためです。
 */
export function occupationLabelIn(key, lang) {
  const row = OCCUPATION_LABELS_I18N[isOccupation(key) ? key : OTHER_OCCUPATION];
  return row[lang] || row.ja;
}

/**
 * 登録画面で選んだ職業を、初回ログインのときに profiles へ移すための差分。
 *
 *   ★lib/ageGate.js の adoptSignupAnswer と同じ考え方です。
 *     登録の時点ではまだログインしていないため、profiles には書けません。
 *     いったん auth の user_metadata に預け、ここで移します。
 *
 *   ★すでに本人が選んでいれば null を返し、何もしません。
 *     あとから設定で選び直した値を、登録時の答えで上書きしないためです。
 */
export function adoptSignupOccupation(profile, signupOccupation) {
  if (!profile) return null;
  if (isOccupation(profile.voice_occupation)) return null;
  if (!isOccupation(signupOccupation)) return null;
  return { voice_occupation: signupOccupation };
}
