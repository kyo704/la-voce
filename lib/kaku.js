...
//
//   ★★★どの 種が ある かは `public.my_page_fields()` が 出します（★裁定207）。
//     ★★ここで 並べません。★`enabledAt` の 無い 種は 呼ばれません。
// ============================================================================
import { labelOf as fieldLabelOf } from "@/lib/pageFields";
 ...
 * ★その 種の 札。
 *
 *   ★★★2026-09-26 ── ★出どころが 台帳の 関数に なりました（★裁定207）。
 *     ★呼ぶ 側が `my_page_fields()` の 行を 渡します。★ここで 並べません。
 */
export function labelOf(fields, kind) {
  return fieldLabelOf(fields, kind);
}
