...
export const TITLE = tx("日を 選ぶ");

/** ★曜日（★見本の `日月火水木金土`）。 */
export const WEEK = Object.freeze([tx("日"), tx("月"), tx("火"), tx("水"), tx("木"), tx("金"), tx("土")]);

/** ★月の 名（★見本の 一覧）。 */
export const MONTHS = Object.freeze([
  tx("1月"), tx("2月"), tx("3月"), tx("4月"), tx("5月"), tx("6月"), tx("7月"), tx("8月"), tx("9月"), tx("10月"), tx("11月"), tx("12月")
]);
...
/** ★前の月・次の月（★見本の `‹ 前の月` ／ `次の月 ›`）。 */
export const PREV = tx("‹ 前の月");
export const NEXT = tx("次の月 ›");

/** ★上の 1行（★いま 何を 選んで いるか）。 */
export const ASK_RANGE_A = tx("はじめの日を 選んでください");
export const ASK_ONE = tx("日を 選んでください");
export const ASK_MULTI = tx("無理な日を 選んでください（いくつでも）");
export const ASK_RANGE_B = tx("（終わりの日を 選んでください）");

/** ★早く 選ぶ 札（★`range` の ときだけ）。 */
export const QUICK = Object.freeze([tx("先月"), tx("今月"), tx("この3か月")]);

/** ★下の 札。 */
export const BTN_ONE = tx("この日に する");
export const BTN_MULTI = tx("これで いい");
export function btnOf(mode) {
  return normalizeMode(mode) === "multi" ? BTN_MULTI : BTN_ONE;
}

/** ★下の 断り（★使い方で 変わります）。 */
export const NOTE_RANGE = tx("はじめの日 → 終わりの日 の 順に 押します。もう一度 押すと やり直せます。");
export const NOTE_OTHER = tx("押すと 選べます。もう一度 押すと 外れます。");
