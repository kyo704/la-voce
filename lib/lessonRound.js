...
export const PREFS_NOTE = Object.freeze([
  ...
  // ★★「入れられます」── ★学生が **ご自分で** 入れます（★1件ずつ）。
  //   ★★この 字は その ままで 正しい です。★「入ります」に しません。
  "決まったら、きょうと 日程に 出ます。カレンダーには、ご自分で 1件ずつ 入れられます。"
]);
...
/** ★地図に 出す 字（★見本の .sub）。 */
/**
 * ★レッスン割の 入口（★見本 `P_wari`・★2026-09-24）。
 *
 *   ★★★この 一枚が ありません でした。★開くと いきなり 地図 でした。
 *     ★★何を する 画面なのか、★いま どこまで 進んで いるのか、
 *       ★次に 何を 押せば よいのかが、★どこにも ありません でした。
 */
// ★★★2026-09-24、★2行目を 直しました。★きょう 見本から 写した ばかり です。
//   ★前 …「確定すると 全員の カレンダーに 届きます。」
//   ★★届きません。★この 品に 購読の 道は ありません。★お知らせも 送りません。
//     ★★すぐ 下の `DONE_NOTE` が、★同じ 束の 中で そう 書いて います ──
//       「お知らせは 送りません。学生が 開いたときに 見えます。」
//   ★★見本を 写す ときは、★仕組みを 先に 見る こと。★写した ばかりで 嘘に なりました。
export const HUB_WARN = Object.freeze([
  "年度初めの レッスンを、割り振るための 画面です。",
  "学生の 希望を 見ながら 置き、確定すると 学生の 画面に 出ます。"
]);
/** ★確定した あと（★見本 …「確定しました。学生の カレンダーに 届いています」）。 */
export const HUB_WARN_DONE = Object.freeze([
  // ★★同じ わけ で 直しました（★2026-09-24）。★「届いて います」も
  //   ★「そのまま 反映されます」も、★この 品では 起きません。
  "確定しました。学生の 画面に 出て います。",
  "動かすと、学生の 画面も 変わります。"
]);

export const HUB_NOW_HEAD = "いまの ようす";
export const HUB_START = "回を 始める";
export const HUB_START_SUB = "新しい 学期・前期後期";
export const HUB_ANSWERED = "希望を 出した方";
export const HUB_NOT_YET = "まだの方";
export const HUB_PLACED = "置いた方";
export const HUB_DUE = "しめきり";

export const HUB_STEPS_HEAD = "すすめかた";
export const HUB_STEPS = Object.freeze([
  { key: "map", label: "① 希望の 地図を 見る", sub: "どの コマに 何人 来られるか" },
  { key: "put", label: "② 地図から 置く", sub: "コマを 押すと、来られる方の 名前が 出ます" },
  // ★★「全員の カレンダーへ」も 直しました（★2026-09-24）。★配る 先は 画面 です。
  { key: "done", label: "③ 確定して 配る", sub: "学生の 画面へ" }
]);
export const HUB_STEP_DONE = "済み";

/**
 * ★入口の 下の 註（★見本 `P_wari` の note）。
 *
 *   ★★4行 とも 確かめました。★本当 です ──
 *
 *   ★①「希望は、担当の 先生と、日程を 組む 方だけが 見られます。」
 *     ★★`lesson_prefs` の 決まりは 3つ です ──
 *       ★ご本人（`user_id = auth.uid()`）
 *       ★`has_can(org,'sched_all')`（★日程を 組む 方）
 *       ★`assignments` で その 学生の 担当 先生
 *     ★★ほかの 道は ありません。★見本の 字と 1つずつ 合って います。
 *
 *   ★②「理由は うかがっていません。授業の コマは「×」とだけ 伝わり、
 *       授業名は 出ません。」
 *     ★★`isClassSlot` は 真偽 しか 返しません。★名を 受け取る 引数も ありません。
 *     ★★「来られない」の 欄も わけを 聞きません（★`TT_COPY.ngSub`）。
 *
 *   ★③「まだの方に、こちらから 催促は しません。」
 *     ★★この 画面に 知らせを 送る 道は ありません。★札も ありません。
 *     ★★★`nudge_timetable` は 時間割の 話 です。★こちらでは 使いません。
 *
 *   ★④「細かく 動かすときは「レッスンの 日程を 組む」も 使えます。」
 *     ★★`components/OpsKumu.jsx` です。★前から あります。
 */
export const HUB_NOTES = Object.freeze([
  "希望は、担当の 先生と、日程を 組む 方だけが 見られます。",
  "理由は うかがって いません。授業の コマは「×」とだけ 伝わり、授業名は 出ません。",
  "まだの方に、こちらから 催促は しません。",
  "細かく 動かす ときは「レッスンの 日程を 組む」も 使えます。"
]);

/**
 * ★入口の 数（★見本 …「希望を 出した方 12 / 18人」）。
 *
 *   ★★★数えるのは ここ だけ です。★画面で 数えません。
 *   ★★0の ときも 数で 出します ── ★ここは ご自分の 責めでは なく、
 *     ★★「まだ 始まって いない」という 事実 です。
 */
export function hubCounts({ monkaCount, prefs, placed } = {}) {
  const 全 = typeof monkaCount === "number" ? monkaCount : 0;
  const 出 = prefs && typeof prefs === "object"
    ? new Set(Object.keys(prefs).map((k) => String(k).split(":")[0])).size : 0;
  const 置 = placed && typeof placed === "object" ? Object.keys(placed).length : 0;
  return { total: 全, answered: 出, notYet: Math.max(0, 全 - 出), placed: 置 };
}
...
/**
 * ★回が 1つも 無い ときの 字（★2026-09-24・Opus の ご指摘）。
 *
 *   ★★前は 画面が `return null` でした。★白い ままに なります。
 *     ★★「読み込み中」と「無い」と「壊れた」が、★同じ 顔に なります。
 *   ★★見本の `SC['コマの中身']` と 同じ 形に します ──
 *     ★何が 起きて いるかを 1行、★次に 何を すれば よいかを 1行。
 *   ★★責めません。★「まだ 始めて いません」と 書きます。
 */
export const MAP_NO_ROUND_HEAD = "まだ 回が ありません。";
export const MAP_NO_ROUND_SUB =
  "「回を 始める」から、新しい 学期の 回を 作れます。";
 ...
 * ★届く ところ。
 *
 *   ★★★2026-09-24 に 直しました。★前は 見本の まま「学生の きょう・日程・カレンダー」。
 *     ★★カレンダーには **届きません**。★入れるのは 学生 です（★1件ずつ）。
 *     ★★「届く ところ」に 並べると、★こちらから 入れる ように 読めます。
 */
export const DONE_REACH = "学生の きょう・日程";
...
export function confirmLines(placed) {
  ...
  return Object.freeze([
    "置いた " + n + "人に、レッスンが 届きます。",
    "置けていない方は、あとから 置けます。",
    // ★★2026-09-24 ── ★カレンダーは 追いかけません（★購読の 道が ありません）。
    //   ★★動かした ぶんが 変わるのは、★学生の 画面 です。
    "確定したあと 動かしても、学生の 画面に そのまま 出ます。"
  ]);
}
 ...
 * ★確定した あとの 2行目。
 *
 *   ★★★2026-09-24 に 直しました。★前は こう 書いて いました ──
 *     「学生は、それぞれ 自分の カレンダーの 住所を 持っています。」
 *   ★★★そんな 住所は ありません。★この 品に 購読の 道は 1本も ありません。
 *     ★あるのは、★1件ずつ の Google カレンダーへの 道 だけ です
 *       （`components/VocalTracker.jsx` の `buildGoogleCalendarUrl`）。
 *     ★★押した その 1件 だけ が 入ります。★あとで 変わっても 追いかけません。
 *   ★★★すぐ 上の `DONE_NOTE` は「お知らせは 送りません」と 書いて います。
 *     ★★同じ 束の 中で、★2つが 食いちがって いました。
 */
export const DONE_AFTER_HOW =
  "学生の 画面に 出ます。カレンダーへは、学生が 1件ずつ 入れます。";
