...
export const BACK_TO = tx("もっと");

/** ★題（★見本の `h2`）。★「お仕事を選ぶ」では ありません。 */
export const TITLE = tx("どんな お仕事ですか");
...
export const LEAD_LINES = Object.freeze([
  tx("えらぶと、画面の ことばが 変わります。"),
  tx("あとから いつでも 変えられます。書いたものは 消えません。")
]);
...
export const FIELDS = Object.freeze([
  Object.freeze({ key: "music", name: tx("音楽（歌・楽器）"), sub: tx("曲目・レパートリー・演奏会") }),
  Object.freeze({ key: "voice", name: tx("声の お仕事"), sub: tx("声優・ナレーター・アナウンサー") }),
  Object.freeze({ key: "stage", name: tx("舞台"), sub: tx("演目・公演") })
]);
...
export const TOAST = tx("ことばを 変えました");
...
export const NOTE_LINES = Object.freeze([
  tx("書いた 記録は、ひとつも 変わりません。"),
  tx("ことばの 見え方だけが 変わります。"),
  tx("声の お仕事・舞台を えらぶと、「さがす」は 出ません。"),
  tx("伴奏の 相手を さがす ことが ないからです。"),
  tx("あとで 音楽に 戻せば、また 出ます。")
]);
