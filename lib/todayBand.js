...
/**
 * ★その日づけ（★"2026-09-18T21:00:00+00:00" → ★端末の 時計で "2026-09-19"）。
 *
 *   ★★★台帳は UTC で 持って います。★字を そのまま 切ると ずれます。
 *     ★★日本では 9時間 先 です。★夜の レッスンが、★前の 日に 見えます。
 *   ★★★`slice(0,10)` を 書かない で ください。★ここを 呼んで ください。
 */
export function dayOf(iso, tz) {
  ...
  if (tz) opt.timeZone = tz;
  return new Intl.DateTimeFormat("en-CA", opt).format(d);
}

function sameDay(iso, todayISO, tz) {
  const p = dayOf(iso, tz);
  return !!p && p === todayISO;
}
...
export const ATTENDANCE = Object.freeze([
  // ★★★2026-09-18、★学校の 言葉に そろえました（★坂本さんの お決め）。
  //   ★★もと … 来た ／ 休み ／ 中止
  //   ★★いま … ★出席 ／ 休み ／ ★休講
  //   ★★見本（4本 とも）が `['出席','休み','休講']` です。
  //   ★★★台帳は 変えません。★`came` / `absent` / `canceled` の まま です。
  //     ★★字だけの 話 です。★入れ直しも 移し替えも 要りません。
  //   ★★★38人の 画面の 字が 変わります。★個人の 画面も 同じ 言葉に なります。
  //
  //   ★★「欠席」は 作りません（★見本も 作って いません）。
  //     ★★見本の 注 ──「休講＝その回を しなかった とき。★生徒の 欠席に なりません」
  //     ★★休み は 生徒の 話、★休講 は その回の 話 です。★別の こと です。
  { key: "came", label: "出席" },
  { key: "absent", label: "休み" },
  { key: "canceled", label: "休講" }
]);
