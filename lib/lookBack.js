...
/**
 * ★下の 3行（★見本 `sakanobo()` の 最後の .note ／ ★2026-09-14）。
 *
 *   ★★見本の 字に そろえました。★1文字も 変えて いません。
 *     ★★前は 別の 3行でした ──
 *       「書いたものを そのまま 出しています。」
 *       「文章を 添えません。…「これが 原因です」と 言いません。」
 *       「見て、ご自分で 気づくための 画面です。」
 *     ★★どれも 悪い 文では ありません。★けれど 見本の 言って いる ことと、
 *       ★**別の こと**を 言って いました。
 *
 *   ★★見本が ここで 言いたいのは、★1つ です ──
 *     ★「この 一覧は、★**2つの 出どころ**を 合わせて います」。
 *     ★★そして「本番だけに すると 0件に なる」が、★その わけ です。
 *     ★★行ごとの 小見出し（LOOK_BACK_SOURCE）と、★同じ 話 です。
 *       ★片方だけ 直すと、★説明と 一覧が 食い違います。
 */
export const LOOK_BACK_NOTE = Object.freeze([
  "2つとも 使います ── △出づらいと 書いた日 と 本番で 出なかった日。",
  "本番だけに すると、0件に なります。",
  // ★★2026-09-25、★後ろ半分を 足しました（★見本 `SC['前3日']` の note）。
  //   ★★「確率も 割合も 出しません」で 止めると、★何を しない かは 分かりますが、
  //     ★★**いちばん 大事な こと** が 抜けます ── ★原因を 決めつけない、です。
  //   ★★並んで いる 日を 見て、★「これが 原因だ」と こちらが 言わない こと。
  //     ★見て 考えるのは ご本人 です。
  "文章を 添えません。確率も 割合も 出しません。「これが 原因です」と 言いません。",
  // ★★2026-09-25、★2行 足しました（★見本 `SC['前3日']` の note）。
  //   ★★★この 2行が、★この 画面の **芯** です ──
  //     ★こちらは 何も 足して いません。★書いた ものを そのまま 並べて います。
  //     ★見て、★気づくのは ご本人 です。★こちらが 気づかせません。
  //   ★★上の「原因です と 言いません」と 対に なって います。
  //     ★★何を しないか（上）と、★何を するか（下）です。
  "書いたものを そのまま 出しています。",
  "見て、ご自分で 気づくための 画面です。"
]);

/**
 * ★一覧の 行の 小見出し（★見本 `sakanobo()` の `.usu` ／ ★2026-09-14）。
 *
 *   ★★見本は、★行ごとに「なぜ この日が 並んで いるか」を 言って います。
 *     ★★見本の 中は `i%3===0` で 出し分けて います。★作り物の 数だから です。
 *     ★★こちらは **本当の 出どころ**を 知って います ──
 *       `notOut` … 本番で 出なかった日（★行事の 記録から）
 *       `hard`   … △出づらい と書いた日（★喉の 調子 2以下）
 *
 *   ★★両方に 当てはまる 日が あります。★そのときは `notOut` を 出します。
 *     ★★本番で 出なかった、の ほうが 強い 事実だから です。
 *     ★★決め方を ここに 1つ 置きます。★画面で 決めません。
 */
export const LOOK_BACK_SOURCE = Object.freeze({
  notOut: "本番で 出なかった日",
  hard: "△出づらい と書いた日"
});
 ...
 * ★さかのぼれる 日と、★その 出どころ。
 *
 *   ★★日付だけが 要る ときは `.map((x) => x.date)` を 通します。
 *     ★★合わせる ところを 2つに しません。★片方だけ 変わると ずれます。
 *   ★★2026-09-14、★`lookBackableDays` を 消しました。
 *     ★出どころの 札が 要るように なり、★呼び手が いなく なった ためです。
 *     ★★「作った 関数は、どこかから 呼ばれて いるか」（★N-1 の 決まり）。
 */
export function lookBackDays(notOut, hard) {
  const seen = new Set();
  const out = [];
  (notOut || []).forEach((d) => {
    if (!d || seen.has(d)) return;
    seen.add(d);
    out.push({ date: d, source: "notOut", label: LOOK_BACK_SOURCE.notOut });
  });
  (hard || []).forEach((d) => {
    if (!d || seen.has(d)) return;
    seen.add(d);
    out.push({ date: d, source: "hard", label: LOOK_BACK_SOURCE.hard });
  });
