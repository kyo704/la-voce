 ...
 *
 *   ★★別の欄を作りません。★持ち物から数えます。
 *     ★欄を作ると、★そこが「残高」になります。★残高は作らない決めです。
 *
 *   ★★描き直しの4点は、★数えません（lib/legacyWearables.js の REDRAWN_AS）。
 *     ★古いお店で買った麦わら帽子が、★新しい鍵でも持ち物に入ります。
 *     ★それを「箱2で受け取った」と数えると、★1回ぶん損をさせます。
 *
 * @param {string[]} ownedKeys  持っている鍵
 * @param {object} redrawnAs    {古い鍵: 新しい鍵}
 */
export function box2ReceivedCount(ownedKeys, redrawnAs) {
  const owned = new Set(Array.isArray(ownedKeys) ? ownedKeys : []);
  const notCounted = new Set();
  for (const [oldKey, newKey] of Object.entries(redrawnAs || {})) {
    if (owned.has(oldKey)) notCounted.add(newKey);
  }
  return BOX2_KEYS.filter((k) => owned.has(k) && !notCounted.has(k)).length;
}

/**
 * ★その回が、いつ受け取れるようになったか。
 *
 *   ★記録した日を、★古い順に並べた★30日目・60日目…の日付です。
 *   ★★別の欄に書きません。★記録そのものから出します。
 *
 * @param {string[]} sortedDates  記録した日（YYYY-MM-DD・★古い順）
 * @param {number} round          1回目なら 1
 * @returns {string|null}
 */
export function roundAvailableDate(sortedDates, round) {
  const d = Array.isArray(sortedDates) ? sortedDates : [];
  const i = round * BOX2_EVERY_DAYS - 1;
  return i >= 0 && i < d.length ? d[i] : null;
}

/**
 * ★お任せで届ける日か。
 *
 *   ★受け取れるようになった月が、★今より前の月なら、★お届けします。
 *   ★★選ばないまま溜めないためです。★残高にしない、という決めです。
 *
 *   ★日付は、呼ぶ側が渡します。★ここで new Date() を呼びません
 *     （★呼ぶと、検査で日を差し替えられません）。
 *
 * @param {string} availableAt  受け取れるようになった日（YYYY-MM-DD）
 * @param {string} todayISO     今日（YYYY-MM-DD）
 */
export function shouldAutoDeliver(availableAt, todayISO) {
  if (!availableAt || !todayISO) return false;
  return String(availableAt).slice(0, 7) < String(todayISO).slice(0, 7);
}
 ...
 *   ★★毎回ちがうものが出ては、★選べません。
 *     ★同じ回なら、★いつ開いても同じ3点です。
 *   ★★Math.random を使いません。★回の番号から決めます。
 *
 *   ★すでにお持ちのものは、★出しません。
 *   ★残りが3点より少なければ、★その分だけ出します。
 *
 * @param {string[]} ownedKeys
 * @param {number} round
 * @returns {string[]}
 */
export function pickBox2Choices(ownedKeys, round) {
  const owned = new Set(Array.isArray(ownedKeys) ? ownedKeys : []);
  const rest = BOX2_KEYS.filter((k) => !owned.has(k));
  if (rest.length <= BOX2_CHOICES) return rest;
  // ★回の番号から、★決まった位置を取ります。★同じ回なら、同じ3点です。
  //   ★★ばらけるように、★互いに素に近い歩幅で進みます。
  const step = 17;
  const out = [];
  let i = (round * step) % rest.length;
  while (out.length < BOX2_CHOICES) {
    if (!out.includes(rest[i])) out.push(rest[i]);
    i = (i + step) % rest.length;
  }
  return out;
}
