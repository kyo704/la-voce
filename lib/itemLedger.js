...
import { UNLOCK_CONDITIONS } from "@/lib/character";
...
/**
 * ★品の 鍵 → ★開いた 条件の 鍵（★UNLOCKS の 逆引き）。
 *
 *   ★★UNLOCKS は「条件 → 品」の 向きです。
 *     ★台帳が 持って いるのは 品の ほう なので、★逆に 引きます。
 *   ★★1度だけ 作ります。★行ごとに 作り直しません。
 */
const CONDITION_OF_ITEM = Object.freeze((() => {
  const m = {};
  Object.keys(UNLOCKS || {}).forEach((cond) => {
    (UNLOCKS[cond] || []).forEach((itemKey) => { m[itemKey] = cond; });
  });
  return m;
})());
 ...
 *     「記録が 50日に なった日」「はじめての 本番の 翌日」「はじめの 贈りもの」
 *   ★出どころ 見本の 入口の 札 ──「台帳（★いつ・**何で** 手に入ったか）」
 *   ★裁定 2026-09-15・坂本さん
 *     「買ったもの＝『買いました』、贈りもの＝『贈りもの』の 文言で 進める」
 *
 *   ★★3つの 道が あります（★ACQUIRED_BY）。
 *     shop   … 点で 買った   → ★条件が ありません。★道の 名前を 言います。
 *     gift   … 贈りもの      → ★同上。
 *     unlock … 条件が 開いた → ★どの 条件かを 言います。
 *
 *   ★★unlock の どの 条件かは、★台帳に **残って いません**。
 *     ★`item_acquisitions` が 持つのは shop／gift／unlock の 3つ だけ です。
 *     ★★だから 品の 鍵から `UNLOCKS` を 逆に 引きます。
 *     ★★引けない ときは、★何も 返しません。★作り話を しません。
 *
 *   ★★言葉 そのものは ここに 書きません。
 *     ★`t()` が 9言語を 持って います（★呼ぶ側から 渡します）。
 *     ★★ここが 決めるのは「どの 鍵を 使うか」だけ です。
 *
 * @param {object} row       ★台帳の 1行
 * @param {function} t       ★鍵 → その言語の 言葉
 * @returns {string}         ★出す 言葉。★分からなければ 空文字
 */
export function acquisitionReason(row, t) {
  if (!row || typeof t !== "function") return "";
  const by = row.acquired_by;
  if (by === ACQUIRED_BY.SHOP) return t("acquiredByShop");
  if (by === ACQUIRED_BY.GIFT) return t("acquiredByGift");
  if (by !== ACQUIRED_BY.UNLOCK) return "";
  const cond = CONDITION_OF_ITEM[row.item_key];
  if (!cond) return "";
  const def = UNLOCK_CONDITIONS[cond];
  // ★★言葉が 無い 鍵は、★出しません。★鍵の 文字が 画面に 出るのを 防ぎます。
  //   ★★2026-09-15 まで、★5つ とも 言葉が ありませんでした。
  if (!def || !def.labelKey) return "";
  const word = t(def.labelKey);
  return word === def.labelKey ? "" : word;
}
...
/**
 * ★台帳の 下の 注記（★見本 `SC['台帳']` の `.note`・2行）。
 *
 *   ★★2026-09-15 まで、★2行目の 後半が ありませんでした。
 *     ★★「記録を 消しても 減りません」は 別の 言い方で 出て いましたが、
 *       ★★「**連続日数を 出しません。**」は どこにも ありませんでした。
 *     ★★これは 禁止事項（★進捗バーの 一種）の 約束 です。
 *       ★`lib/character.js:173` に コメントでは ありました。
 *       ★★コメントは 画面では ありません。★読む人には 届きません。
 */
export const LEDGER_NOTE = Object.freeze([
  "いつ・何で 手に入ったかを 残します。",
