...
export const SETTING_SECTIONS = Object.freeze([
  ...
  { key: "export", label: "書き出す（校務システムへ）", any: ["meibo", "sched_all"],
    ready: false, needs: "書き出した 記録を しまう 表（誰が・いつ・何を）" },
  { key: "import", label: "読み込む（校務システムから）", any: ["meibo", "master"],
    ready: false, needs: "読み込みの 道と、取り違えを 直す 画面" },
  { key: "koma", label: "時間の 割り方（コマ）", any: ["koma", "koma_mine"],
    ready: false, needs: "コマの 表（曜日・時刻・長さ）" },
  { key: "place", label: "場所", any: ["koma"],
    ready: false, needs: "場所の 表（部屋の 名・使える 時間）" },
  { key: "mine", label: "自分の 予定（レッスン いがい）", any: ["sched_mine"],
    ready: false, needs: "先生ご自身の 予定の 表" },
  { key: "jugyo", label: "授業の 型", any: ["meibo"], ready: true },
  { key: "saiten", label: "評価の 型", any: ["master"],
    ready: false, needs: "評価の 項目の 表と、点を 入れる 画面" },
  { key: "org", label: "学校の 形", any: ["master"],
    ready: false, needs: "学部・学科・分野の 表（台帳 08-10）" },
