...
export function computeUnlocked(entries, profile) {
  ...
  const p = profile || {};
  const hasGoal = typeof p.practice_goal === "string" && p.practice_goal.trim() !== "";
  const hasReview = Array.isArray(p.practice_reviews) && p.practice_reviews.length > 0;
  if (hasGoal && hasReview) unlocked.add("practiceGoalDone");
  return unlocked;
}
