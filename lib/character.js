 ...
 *
 * ★★practiceGoalDone を、★実際に 配れるように しました（★2026-09-10）。
 *   ★出どころ 訂正 §7-2「★消したあとに、同じ形の 穴が 空いていないか 確かめる」
 *   ★★条件も 札も 品（hatCamellia）も 揃っているのに、
 *     ★この関数が、★1度も 配っていませんでした。
 *     ★★つまり hatCamellia は、★誰の手にも 渡りませんでした。
 *     ★notOutDates と 同じ形の 穴です。
 *   ★★配れなかった 理由は、★見どころが entries に 無いからです。
 *     ★練習の 目標と 振り返りは profiles に あります。
 *     ★だから profile を 受け取るように しました。
 *   ★★渡されなければ、★これまでどおり 配りません。★減りません。
 *     ★訂正 §8-1「★移行では『増える方向』にしか 動かさない」。
 */
export function computeUnlocked(entries, profile) {
  return unlockedFromSummary(unlockSummaryFromEntries(entries, profile));
}

// ---------------------------------------------------------------------------
// ★開く 条件を、★summary（見どころの まとめ）から 判じる 形に しました
//   （★2026-09-11・J05 の 台帳の ため）。
//
//   ★★なぜ 分けたか。
//     台帳に「いつ 開いたか」を 残すには、★サーバでも 同じ 判じが 要ります。
//     ところが 画面の entries は rowToEntry() を 通した 形で、
//     ★その 関数は VocalTracker.jsx の 中に あり、サーバから 呼べません。
//     ★★そのまま サーバに 書き写すと、★同じ 決めが 2か所に なります。
//       ★このリポジトリが 何度も 踏んだ 形です。
//     ★★そこで「★しきい値の 決め」だけを 1か所に 残し、
//       ★見どころの 取り出し方を、画面用と 表用に 分けました。
//       ★数（1回・10回・10種類）は、★下の 1か所にしか ありません。
// ---------------------------------------------------------------------------

/**
 * ★開く 条件の 本体。★数は ここにしか 書きません。
 *
 * @param {object} summary
 *   @param {number}  summary.performances  本番を した 日の 数
 *   @param {boolean} summary.hasPianissimo 弱音の 高音を 1度でも 書いたか
 *   @param {number}  summary.fieldKinds    触れた 記録項目の 種類（累計）
 *   @param {boolean} summary.hasGoal       稽古の 目標を 書いてあるか
 *   @param {boolean} summary.hasReview     振り返りを 1度でも 書いたか
 * @returns {Set<string>}
 */
export function unlockedFromSummary(summary) {
  const s = summary || {};
  const unlocked = new Set();
  const performances = Number(s.performances) || 0;
  if (performances >= 1) unlocked.add("firstPerformance");
  if (performances >= 10) unlocked.add("performances10");
  if (s.hasPianissimo) unlocked.add("firstPianissimo");
  // 記録項目を10種類以上使ったか（その日ではなく、累計で何種類に触れたか）
  if ((Number(s.fieldKinds) || 0) >= 10) unlocked.add("tenFieldKinds");
  // ★★練習の 目標を 立てて、★1度でも 振り返った方に 開きます。
  //   ★★累計です。★振り返りは 増えるだけで、★減りません。
  //   ★「よい 振り返り」かどうかを 見ません。★書いた、という 行為だけです。
  if (s.hasGoal && s.hasReview) unlocked.add("practiceGoalDone");
  return unlocked;
}

/** ★画面の 側（rowToEntry を 通した 形）から、見どころを 取り出します。 */
export function unlockSummaryFromEntries(entries, profile) {
  ...
  return {
    performances: days.filter((e) =>
      (e.activities || []).some((a) => a && a.kind === "本番")).length,
    hasPianissimo: days.some((e) => e && e.pianissimoHighNote),
    fieldKinds: kinds.size,
    ...goalPartOf(profile)
  };
}

/**
 * ★表の 行（snake_case・そのまま）から、見どころを 取り出します。
 *
 *   ★★サーバ用です。★rowToEntry を 通しません。
 *   ★★本番の 数え方は migrateLegacyToActivities と 同じに します。
 *     activities が 空の 古い 行は、activity_type を 見ます。
 *   ★★種類の 数は、★画面の 数え方より 少なめに 出ます。
 *     画面の entry には、★組み立て直した 名前（activities・voiceEntries など）が
 *     混じるからです。★少なめに 倒します。
 *     ★早く 台帳に 載るより、★遅く 載る ほうが よいです。
 */
export function unlockSummaryFromRows(rows, profile) {
  const list = Array.isArray(rows) ? rows : [];
  const kinds = new Set();
  let performances = 0;
  let hasPianissimo = false;
  list.forEach((row) => {
    const acts = Array.isArray(row && row.activities) ? row.activities : [];
    const isPerformance = acts.length > 0
      ? acts.some((a) => a && a.kind === "本番")
      : (row && row.activity_type === "本番");
    if (isPerformance) performances += 1;
    if (row && row.pianissimo_high_note) hasPianissimo = true;
    Object.entries(row || {}).forEach(([k, v]) => {
      if (v == null || v === "" || (Array.isArray(v) && v.length === 0)) return;
      if (k === "date" || k === "user_id" || k === "id") return;
      kinds.add(k);
    });
  });
  return { performances, hasPianissimo, fieldKinds: kinds.size, ...goalPartOf(profile) };
}

/** ★稽古の 目標と 振り返りは、★どちらの 道でも profiles から 取ります。 */
// ★★外からも 使います（★2026-09-14・No.019.5）。
//   ★台帳が entries を 数えた ときも、★目標の 判定は ★この 1つを 通します。
//   ★★書き写すと、★片方だけ 直った 形に なります。
export function goalPartOf(profile) {
  const p = profile || {};
  return {
    hasGoal: typeof p.practice_goal === "string" && p.practice_goal.trim() !== "",
    hasReview: Array.isArray(p.practice_reviews) && p.practice_reviews.length > 0
  };
}
...
export const SHOP_ITEMS = [
  ...
  // ★★インテリア第1便（2026-09-06）。★写真のような絵を敷きます。
  //   ★これまでの床は、★そのまま残します。★取り上げません。
  { key: "floor_natural", category: "floor", cost: 90, origCost: 90, nameKey: "itemFloorNatural" },
  { key: "floor_modern", category: "floor", cost: 90, origCost: 90, nameKey: "itemFloorModern" },
  { key: "floor_nordic", category: "floor", cost: 90, origCost: 90, nameKey: "itemFloorNordic" },
  { key: "floor_asian", category: "floor", cost: 90, origCost: 90, nameKey: "itemFloorAsian" },
  { key: "floor_industrial", category: "floor", cost: 90, origCost: 90, nameKey: "itemFloorIndustrial" },
  { key: "floor_country", category: "floor", cost: 90, origCost: 90, nameKey: "itemFloorCountry" },
  { key: "floor_hotel", category: "floor", cost: 90, origCost: 90, nameKey: "itemFloorHotel" },
  { key: "floor_mix", category: "floor", cost: 90, origCost: 90, nameKey: "itemFloorMix" },
  ...
  // ★★インテリア第1便（2026-09-06）。
  //   ★wall_american と floor_american は、★すでに在ります。
  //     ★絵だけが増えました。★もう1つ作らないこと。
  { key: "wall_natural", category: "wall", cost: 90, origCost: 90, nameKey: "itemWallNatural" },
  { key: "wall_modern", category: "wall", cost: 90, origCost: 90, nameKey: "itemWallModern" },
  { key: "wall_nordic", category: "wall", cost: 90, origCost: 90, nameKey: "itemWallNordic" },
  { key: "wall_asian", category: "wall", cost: 90, origCost: 90, nameKey: "itemWallAsian" },
  { key: "wall_industrial", category: "wall", cost: 90, origCost: 90, nameKey: "itemWallIndustrial" },
  { key: "wall_country", category: "wall", cost: 90, origCost: 90, nameKey: "itemWallCountry" },
  { key: "wall_hotel", category: "wall", cost: 90, origCost: 90, nameKey: "itemWallHotel" },
  { key: "wall_mix", category: "wall", cost: 90, origCost: 90, nameKey: "itemWallMix" },
  { key: "wall_industrialBrick", category: "wall", cost: 90, origCost: 90, nameKey: "itemWallIndustrialBrick" },
