// ============================================================================
// 教室を閉じる／オーナーが退会するときの扱い
//
//   出典 docs/lavoce-教室プラン仕様-複数教師と複数生徒.md §46（owner＝契約者。
//        課金・席の増減・★教室の削除）
//        判断（2026-09-01、A＋B・Cは却下）
//
//   ★この1ファイルが持つ決めごと
//     ① 教室に「ほかの方」が何人いるか（＝誰を数えるか）
//     ② オーナーが退会するとき、教室をどうするか（消す／止める）
//     ③ 教室を閉じると、何が消えて何が残るか
//     ④ 画面に出す文言
//
//   ★なぜ1つにまとめるか
//     この判定は「退会の処理」と「教室の画面」の両方から要ります。
//     2か所に書くと、片方だけ直したときに
//     ★「画面では止まるのに、APIは消してしまう」が起こります。
//     この repo で繰り返し起きてきた壊れ方そのものです。
//
//   ★自動の引き継ぎは作りません（判断C＝却下）。
//     オーナーは契約者です。自動で引き継ぐと、承諾していない人に
//     ★支払いの義務を負わせることになります。
//     引き継ぎは、知らせる→承諾を得る→移す、の順で、公開後に作ります。
//     それまでは運営者が手で入れ替えます。
// ============================================================================

import { isMissingTable } from "./supabaseErrors.js";
// ★契約者か どうかの 決めは、★1か所 だけ（★裁定 その116）。
import { isContractOwner } from "./orgContract.js";

/**
 * ★「ほかの方」として数える相手。
 *
 *   講師・オーナー   memberships（役割を問わない）
 *   生徒            enrollments（★status が active の人だけ）
 *
 * ★生徒も数えます。「ほかに◯人の方がいます」の「方」に、生徒さんは
 *   当然ふくまれます。数えないと、生徒が在籍したままの教室が
 *   ★黙って消えます。消えたことは、生徒側の画面にしか現れません。
 *
 * ★status が 'left' の人は数えません。すでに自分で抜けた人を数えると、
 *   もう誰もいない教室を永久に閉じられなくなります。
 */
export const OTHER_PEOPLE_SOURCES = [
  { table: "memberships", userColumn: "user_id", activeFilter: null },
  { table: "enrollments", userColumn: "student_id", activeFilter: { column: "status", value: "active" } }
];

/**
 * 教室を閉じるとき、消す順番。
 *
 * ★外部キーの実際の向きに合わせた順です。子から先に消します。
 *   ★ここを cascade 任せにしないこと。2026-09-01 の退会の失敗は、
 *     「たぶん cascade するはず」で書いた一覧が原因でした。
 *     実際に確かめた順を、明示して並べます。
 */
//   ★★★2026-09-23、★足りて いません でした。
//     ★行事・役職・場所・ご請求・学科・コマ・連絡・下書き・名簿の下書き・
//       授業の型・先生への 招きが、★この 一覧に ありません でした。
//     ★★消えて いなかった わけでは ありません。★最後の `organizations` で
//       ★連鎖して 消えて いました。★**見えない ところで 消えて いた** の です。
//   ★★★それが 危ない わけ ──
//     ★① 連鎖で 落ちると、★誤りは `organizations` の 1行 だけ に 見えます。
//        ★どの 表で 止まったかが 分かりません（★2026-09-23 の 事故が まさに これ）。
//     ★② 画面の「消えるもの」の 字が、★本当より 少なく 書いて ありました。
//   ★★上から 順に 消します。★最後が `organizations` です。
export const CLOSE_ORG_DELETE_ORDER = [
  { table: "lessons", column: "org_id" },
  { table: "assignments", column: "org_id" },
  { table: "enrollments", column: "org_id" },
  { table: "org_invitations", column: "org_id" },
  // ★★2026-09-23 に 足した ぶん（★連鎖 任せを やめます）。
  { table: "org_message_drafts", column: "org_id" },
  { table: "org_messages", column: "org_id" },
  { table: "org_events", column: "org_id" },
  { table: "org_places", column: "org_id" },
  { table: "org_periods", column: "org_id" },
  { table: "org_divisions", column: "org_id" },
  { table: "roster_drafts", column: "org_id" },
  { table: "lesson_presets", column: "org_id" },
  { table: "teacher_invitations", column: "org_id" },
  { table: "org_billing", column: "org_id" },
  // ★★`memberships` は 役職より 先 です（`post_id` が 役職を 指します）。
  { table: "memberships", column: "org_id" },
  { table: "org_posts", column: "org_id" },
  { table: "organizations", column: "id" }
];

// ★★★記録の 表は、★**わざと 入れて いません**（2026-09-23）。
//   ★`monka_read_log` ／ `org_billing_log` ／ `score_log` ／ `export_log`。
//   ★★いま、★学校を 閉じると これらも 連鎖で 消えます（`ON DELETE CASCADE`）。
//   ★★★これは 約束と ぶつかって います ──
//     ★「見た 記録は 消せません」（★約束 P-0386）
//     ★「ご本人から 求められたら、30日以内に お見せします」
//   ★★★けれど、★ここで 勝手に 直しません。★決めの こと です。
//     ★学校が 無く なった あとに、★誰が その 記録を 読むのか。
//     ★読む 人が 居ないなら、★残す 意味も 変わります。
//   ★★台帳の 側（外部キー）を `SET NULL` に するかどうかは、★裁定を お待ちします。

// ---------------------------------------------------------------------------
// 文言（★画面に直接書かないこと。ここが唯一の正）
// ---------------------------------------------------------------------------

/** 教室を閉じる前の確認。★残るものを先に言うこと。 */
export const CLOSE_ORG_KEEP_LINE =
  "教室を閉じても、生徒さんが書いた記録は、消えません。ご本人のものとして残ります。";

/**
 * ★「消えるもの」。★増えたら ここを 直す こと。
 *
 *   ★★★2026-09-23、★3つ しか 書いて いません でした。
 *     ★本当は、★行事・役職・場所・ご請求・学科・コマ・連絡も 消えて いました。
 *   ★★少なく 書く のは、★書かない のと 同じ くらい 悪い です。
 *     ★★「それも 消えるとは 思わなかった」と 言われます。
 */
export const CLOSE_ORG_DELETE_LINE =
  "消えるもの：教室そのもの／先生と生徒の紐付け／レッスンの予定／"
  + "行事／役職／場所／ご請求の設定／学科・コース／コマ／連絡と書きかけ";

/**
 * オーナーの退会を止めるときの知らせ。
 *
 * ★2つとも、押せるボタンにすること（Opus の訂正、2026-09-01）。
 *   最初の案は「教室を閉じる」だけがボタンで、引き継ぎは押せない文字でした。
 *   ★それでは、戻せないほうだけが押せる形になります。
 *     人は押せるものを押します。文言が中立でも、選択肢は中立ではありません。
 *
 * ★「教室を残す」は mailto: を開きます。件名に★教室のIDを入れること。
 *   入れないと、最初の返信が「どの教室ですか」から始まります。
 *   引き継ぎは運営者が手で入れ替えるので、IDが分からないと何もできません。
 */
export function departingOwnerNotice(otherCount) {
  return {
    lines: [
      `この教室には、ほかに ${otherCount}人 の方がいます。`,
      "アカウントを削除すると、この教室の契約者がいなくなります。"
    ],
    choices: [
      {
        key: "close",
        label: "教室を閉じる",
        action: "button",
        lines: [
          "このまま終わりにします。元に戻せません",
          "生徒さんが書いた記録は消えません。ご本人のものとして残ります"
        ]
      },
      {
        key: "keep",
        label: "教室を残す",
        action: "mailto",
        lines: [
          "別の先生に引き継ぎます。迷っている場合も、こちらへ",
          "運営者にご連絡ください。3日以内にお返事します"
        ]
      }
    ]
  };
}

/**
 * 「教室を残す」を押したときに開くメールの下書き。
 *
 * ★件名に教室のIDを必ず入れます。名前だけでは足りません。
 *   「マイ教室」は ensureOwnOrg がつける既定の名前なので、
 *   ★同じ名前の教室が、人数ぶん存在します。名前では特定できません。
 */
/**
 * ★オーナーが抜けるが、教室は続く場合の知らせ（2026-09-02・Opus の裁定）。
 *
 *   ★「教室を動かせるか」と「誰が払うか」は、別の問いです。
 *     責任者が残っていれば、教室は動きます。★止めません。
 *     ですが契約者は居なくなります。そこは黙って通せません。
 *
 *   ★止めないので、選択肢は出しません。1行の知らせと、連絡先だけです。
 *     「教室を閉じる」を並べると、閉じなくてよい教室を閉じさせてしまいます。
 *
 *   ★生徒の退会では出しません。生徒は契約者ではありません。
 */
export const DEPARTING_PAYER_LINE =
  "あなたが退会すると、この教室の契約者がいなくなります。運営を続けるには、運営者にご連絡ください。";

export function departingPayerNotice(org, email) {
  return {
    line: DEPARTING_PAYER_LINE,
    mailtoLabel: "運営者に連絡する",
    mailto: payerMailto(org, email)
  };
}

/**
 * 契約者がいなくなることの連絡メール。
 * ★件名に教室のIDを入れます（transferMailto と同じ理由。名前では特定できません）。
 */
export function payerMailto(org, email) {
  const orgId = (org && org.orgId) || "";
  const name = (org && org.name) || "教室";
  const subject = `教室の契約者について（教室ID: ${orgId}）`;
  const body = [
    "Woolsong 運営者さま",
    "",
    `教室「${name}」の契約者が、退会によっていなくなります。`,
    "教室の運営は続いています。今後の扱いについて、ご相談させてください。",
    "",
    "----------------------------------------",
    "★この行より下は、消さないでください",
    `教室ID: ${orgId}`,
    `教室名: ${name}`,
    "----------------------------------------",
    ""
  ].join("\n");
  return `mailto:${email}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
}

export function transferMailto(org, email) {
  const orgId = (org && org.orgId) || "";
  const name = (org && org.name) || "教室";
  const subject = `教室の引き継ぎについて（教室ID: ${orgId}）`;
  const body = [
    "Woolsong 運営者さま",
    "",
    `教室「${name}」を、別の先生に引き継ぎたいです。`,
    "（迷っている場合は、その旨をお書きください）",
    "",
    "----------------------------------------",
    "★この行より下は、消さないでください",
    `教室ID: ${orgId}`,
    `教室名: ${name}`,
    "----------------------------------------",
    ""
  ].join("\n");
  return `mailto:${email}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
}

// ---------------------------------------------------------------------------
// 判定
// ---------------------------------------------------------------------------

/**
 * その人がオーナーである教室の一覧。
 *
 * ★memberships の owner だけでなく、created_by も見ます。
 *   membership の作成だけ失敗した教室（オーナーの居ない教室）が
 *   実際に生まれうるためです（VocalTracker の ensureOwnOrg 参照）。
 *   ここで拾わないと、その教室は誰にも消せなくなります。
 */
export async function listOwnedOrgs(client, userId) {
  const ids = new Set();

  const { data: owned, error: mErr } = await client
    .from("memberships").select("org_id").eq("user_id", userId).eq("role", "owner");
  if (mErr) return { error: mErr };
  (owned || []).forEach((r) => r.org_id && ids.add(r.org_id));

  const { data: created, error: oErr } = await client
    .from("organizations").select("id").eq("created_by", userId);
  if (oErr) return { error: oErr };
  (created || []).forEach((r) => r.id && ids.add(r.id));

  return { orgIds: [...ids] };
}

/**
 * その教室にいる「ほかの方」の人数。
 *
 * ★同じ人が講師としても生徒としても入っている場合を、二重に数えません。
 *   「ほかに2人」と出たのに1人しかいない、が起きます。
 */
export async function countOtherPeople(client, orgId, userId) {
  const people = new Set();
  for (const src of OTHER_PEOPLE_SOURCES) {
    let q = client.from(src.table).select(src.userColumn).eq("org_id", orgId);
    if (src.activeFilter) q = q.eq(src.activeFilter.column, src.activeFilter.value);
    const { data, error } = await q;
    if (error) return { error };
    (data || []).forEach((row) => {
      const id = row[src.userColumn];
      if (id && id !== userId) people.add(id);
    });
  }
  return { count: people.size };
}

/**
 * 退会しようとしている人の教室を、2つに仕分ける。
 *
 *   solo    ほかに誰もいない  → 行ごと消す（判断B）
 *   blocked ほかに人がいる    → ★退会を止める（判断の追加要件）
 *
 * ★止めるほうが1つでもあれば、退会の処理は何も始めません。
 *   途中まで消してから止めると、記録だけ消えてアカウントが残ります。
 */
/**
 * ★退会が危うくする教室（2026-09-02）。
 *
 *   listOwnedOrgs とは、答えている問いが★違います。混ぜないこと。
 *
 *     listOwnedOrgs      「この教室を★閉じてよいのは誰か」
 *                        → オーナーだけ。閉じるのは取り返しがつきません。
 *                          /api/org/close の権限の判定に使っています。
 *                          ★ここに admin を足すと、責任者が教室を消せます。
 *
 *     listOperatedOrgs   「この人が抜けると、★困る教室はどれか」
 *                        → オーナーと責任者。どちらも運営している人です。
 *
 *   ★2026-09-02 の事故は、後者が前者で代用されていたためです。
 *     +g4t3（責任者）が退会しようとしたとき、listOwnedOrgs は
 *     role='owner' で絞っていたので★0件を返し、
 *     止める対象が1つも無いまま、退会が進みました。
 *     ほかに3人いる教室から、責任者の membership だけが消えました。
 *
 *   ★役割の名前で場合分けしません（憲章 §9）。
 *     「運営できる人か」で見ます。ORG_OPERATOR_ROLES の1か所だけです。
 */
// ★★★2026-09-20（★裁定 その115 Q2）── ★名を 変えました。
//   ★★もと `ORG_OPERATOR_ROLES`。★`ROLES` の 名が、★できことと 同じ 層に
//     ★★見せて いました。★ここで 見て いるのは「運営を 担う 立場か」です。
//   ★★★契約者 その ものは `organizations.contract_owner_user_id` です
//     （★裁定 その116）。★決めは `lib/orgContract.js` が 持ちます。
//   ★★ここは「退会で 学校を 放り出さない ため の 網」です。★広めに 取ります。
export const OPERATOR_ROLES = ["owner", "admin"];
/** ★古い 名。★呼ぶ 側が 残って いる あいだ だけ 置きます。 */
export const ORG_OPERATOR_ROLES = OPERATOR_ROLES;

export async function listOperatedOrgs(client, userId) {
  const ids = new Set();

  const { data: operated, error: mErr } = await client
    .from("memberships").select("org_id").eq("user_id", userId)
    .in("role", ORG_OPERATOR_ROLES);
  if (mErr) return { error: mErr };
  (operated || []).forEach((r) => r.org_id && ids.add(r.org_id));

  // ★作った教室も入れます。membership の作成だけ失敗している場合
  //   （作りかけの教室）でも、放り出さないためです。
  const { data: created, error: oErr } = await client
    .from("organizations").select("id").eq("created_by", userId);
  if (oErr) return { error: oErr };
  (created || []).forEach((r) => r.id && ids.add(r.id));

  return { orgIds: [...ids] };
}

/**
 * その教室に、この人が抜けたあとも★運営できる人が何人残るか。
 *
 * ★「ほかに人が居るか」とは別の問いです（2026-09-02）。
 *   責任者が1人抜けても、オーナーが残っていれば教室は続きます。
 *   そのときに止めるのは、行きすぎです。
 */
export async function countRemainingOperators(client, orgId, userId) {
  const { data, error } = await client
    .from("memberships").select("user_id, role").eq("org_id", orgId)
    .in("role", ORG_OPERATOR_ROLES);
  if (error) return { error };
  const others = (data || []).filter((r) => r.user_id && r.user_id !== userId);
  return { count: others.length };
}

/**
 * 退会しようとしている人の教室を、仕分ける。
 *
 *   solo    ほかに誰もいない            → 教室ごと消す
 *   blocked ほかに人がいて、★運営できる人が0になる → 止める
 *   （どちらでもない）ほかに人がいるが、運営できる人が残る → ★そのまま抜ける
 *
 * ★2026-09-02、3つ目を足しました。
 *   それまでは「ほかに人が居れば止める」でした。そのため、
 *   ★責任者が1人抜けるだけでも、オーナーが残っているのに止まりました。
 *   しかも止めたときの文は
 *     「アカウントを削除すると、この教室の契約者がいなくなります。」
 *   です。オーナーが残っている場合、★この文は事実ではありません。
 *   止まったことだけを見て「直った」と報告しましたが、
 *   ★出ている文が正しいかを見ていませんでした。
 *
 * ★止めるのは「教室から運営できる人が居なくなる」ときだけです。
 *   人数ではなく、★残る運営者の数で決めます（憲章 §9）。
 */
export async function classifyOwnedOrgs(client, userId) {
  // ★閉じてよいか（listOwnedOrgs）ではなく、抜けると困るか（listOperatedOrgs）。
  const owned = await listOperatedOrgs(client, userId);
  if (owned.error) return { error: owned.error };

  const solo = [];
  const blocked = [];
  const payer = [];   // ★止めないが、契約者が居なくなる教室
  for (const orgId of owned.orgIds) {
    const counted = await countOtherPeople(client, orgId, userId);
    if (counted.error) return { error: counted.error };
    if (counted.count === 0) {
      solo.push(orgId);
    } else {
      // ★ほかに人が居ても、運営できる人が残るなら、止めません。
      const ops = await countRemainingOperators(client, orgId, userId);
      if (ops.error) return { error: ops.error };
      if (ops.count > 0) {
        // ★ただし、抜けるのが★契約者本人なら、契約者が居なくなります。
        //   止めませんが、黙っても通しません（1行の知らせを出します）。
        // ★★★`memberships.role === "owner"` で 見て いました（★2026-09-21 に 直しました）。
        //   ★★契約者は 役割の 名では ありません。
        //     ★`organizations.contract_owner_user_id` です（★裁定 その116）。
        //   ★★★ずれると、★2つとも 誤ります ──
        //     ★① 契約を 譲った あとの 元オーナーに、★要らない 知らせが 出ます。
        //     ★② 役割が owner で ない 本当の 契約者は、★**知らせを 受けません**。
        //       ★★学校が 黙って 支払う 人を 失います。★こちらが 重い です。
        //   ★★決めは `lib/orgContract.js` が 1つ 持ちます。★ここで 判じません。
        const { data } = await client.from("organizations")
          .select("id, name, contract_owner_user_id").eq("id", orgId).maybeSingle();
        if (isContractOwner(data, userId)) {
          payer.push({ orgId, name: (data && data.name) || "教室" });
        }
        continue;
      }
      const { data } = await client.from("organizations").select("id, name").eq("id", orgId).maybeSingle();
      blocked.push({ orgId, name: (data && data.name) || "教室", otherCount: counted.count });
    }
  }
  return { solo, blocked, payer };
}

/**
 * 教室を1つ閉じる。★呼ぶ前に、閉じてよいかを確かめること。
 * この関数は権限を見ません（管理者クライアントで動かすため）。
 */
export async function closeOrg(admin, orgId) {
  const failures = [];
  for (const { table, column } of CLOSE_ORG_DELETE_ORDER) {
    const { error } = await admin.from(table).delete().eq(column, orgId);
    // ★表が無いだけなら無視します。列が無いのは無視しません（判定は1か所）。
    if (error && !isMissingTable(error)) failures.push({ table, message: error.message });
  }
  return failures;
}
