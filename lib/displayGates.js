...
//   ★出どころ docs/design/pack-final/裁定-ふりかえる・とだな・もっと（9月10日 その7）.md §2-2
//     「★② 散らばりに くらべた ひらき ≧ 0.50
//       ★③ ★選んだ 数だけ 厳しくします（0.50 ＋ 0.12 ×（選んだ数−1））」
//   ＋ 坂本さんの お決め（★2026-09-11・②）
//   ★★緩めた のでは ありません。★厳しく しました。
export const NARRATIVE_MIN_EFFECT_SIZE = 0.50;  // ② |Hedges' g| ≥ 0.50

/**
 * ★たくさん 調べた ぶんの 上のせ（★裁定 その7 §2-2 ③）。
 *
 *   ★★1つだけ 調べるなら 0.50。★5つ 調べるなら 0.50 ＋ 0.12 × 4 ＝ 0.98。
 *   ★★これが 営業資料 2ページ目の「たくさん 調べたぶんの 補正」の 半分です
 *     （★もう半分は BH-FDR）。
 *   ★★「数を 撃てば 当たる」を なくす ため、★2つの 道で 締めます。
 *
 *   @param chosen ★選んだ 数（★1 以上）
 */
export const EFFECT_SIZE_STEP = 0.12;

export function minEffectSizeFor(chosen) {
  const n = Number.isFinite(chosen) && chosen >= 1 ? Math.floor(chosen) : 1;
  return NARRATIVE_MIN_EFFECT_SIZE + EFFECT_SIZE_STEP * (n - 1);
}
...
export const GATES = [
  ...
  //   ★声の予報そのものをやめたので、★このゲートを呼ぶ人がいなくなりました。
  //   ★★規則：作ったものは、必ずどこかから呼ばれているか。
  //     ★呼ばれないゲートを残すと、★あとで「ある」と誤解されます。
  // ACWR: 28日。★パネルのロックと「今日の一言」の警告は、必ずこの同一フラグを見ること。
  { key: "acwr", labelKey: "gateLabelAcwr", minDays: UNLOCK_DAYS.LV5 },
  // ★★env.comfortZone を外しました（★2026-09-07・10番）。
  //   ★件数5だけの関門でした。★効果量も多重比較も見ていません。
  //   ★カードを外したので、★この関門を通る絵も、もうありません。
...
// 効果量の判定。|g| ≥ 0.50 または |ρ| ≥ 0.3 のどちらかを満たせばよい（§6-1 ②）。
 ...
 * @returns {{ key, passed, reason, message, label }}
...
export function evaluateGate(key, ctx = {}, t) {
  ...
  if (!gate) {
    ...
    return { key, passed: false, reason: "unknownGate", message: null, label: key };
  }
  const label = tr(gate.labelKey);
  // ★★2026-09-11、★remaining（★あと何日・あと何件）を 返すのを やめました。
  //   ★出どころ 坂本さんの お決め（★2026-09-11）／裁定 その7 §2-4
  //     「✕ 門①を『あと○日』と 書く ／ ◯ 進んだ側から 書く」
  //   ★★返り値に 残しておくと、★いつか 誰かが 帯を 描きます。
  //     ★実際、★描いていました（VocalTracker の ProgressDots）。
  //   ★★消したのは 数えることです。★記録は 1件も 触っていません。
  const fail = (reason, message) => ({ key, passed: false, reason, message, label });

  if (gate.minDays != null) {
    const days = num(ctx.days);
    if (days == null) return fail("days", tr("gateNoClearTrend").replace("{label}", label));
    if (days < gate.minDays) {
      // ★「あと◯日」と言わないこと（2026-09-01）。「あと」は人を急かします。
      //   目標と、いまの日数を並べるだけにします。
      return fail("days",
        tr("gateNeedDays")
          .replace("{total}", gate.minDays)
          .replace("{done}", days)
          .replace("{label}", label));
    }
  }
  ...
  if (gate.minNPerGroup != null) {
    ...
    if (n == null) return fail("count", tr("gateNoClearTrend").replace("{label}", label));
    if (n < gate.minNPerGroup) {
      // ★★「あと{n}件」ではなく、★たまった 側を 書きます（★裁定 その7 §2-4）。
      return fail("count", tr("gateNeedRecords")
        .replace("{total}", gate.minNPerGroup).replace("{done}", n).replace("{label}", label));
    }
  }
  ...
  if (effect !== true) {
    ...
    return fail("effect", tr("gateNoClearTrend").replace("{label}", label));
  }

  if (gate.requiresFdrPass && ctx.fdrPass !== true) {
    return fail("fdr", tr("gateNoClearTrend").replace("{label}", label));
  }

  return { key, passed: true, reason: null, message: null, label };
}
...
// ============================================================================
// 効果量の3つの表示状態（設計憲章 §3-4）
//
//   ★4段階の星（★☆☆☆〜★★★★）は、もう使いません。
//     憲章 §3-4 は表示状態を3つだけ定めています。星は「だんだん確からしくなる」
//     という4段階目盛りで、①待機／②通過／③不通過 のどれとも一致しません。
//
//       ① waiting        件数が足りない → 「あと◯で判定を始められます」
//       ② shown          3ゲート全通過   → 図 ＋ 解釈の文章
//       ③ inconclusive   件数は足りたが通らない
//                        → 「はっきりした関係は見えませんでした」
//                        → ★数字（効果量・q値）も出さない
//
//   ★③を「関係なし」と書かないこと。見えなかったことと、無いことは違います。
//
//   ★2026-08-30 まで、効いた習慣ランキングは n≥3 で星と効果量を出していました。
//     古い lavoce-指標設計図.md のしきい値が、そこだけ残っていたためです。
//     n=3 で「効果量 g=0.62」と断定する文が出ていました。
// ============================================================================

export const EFFECT_WAITING = "waiting";
export const EFFECT_SHOWN = "shown";
export const EFFECT_INCONCLUSIVE = "inconclusive";

/**
 * その比較を、いまどう見せるか。
 * ★呼ぶ側で n や |g| を直に比べないでください。しきい値がそこだけ古くなります。
 *
 * @param res { n1, n0, g, fdrPass } 群ごとの件数・効果量・FDRを通ったか
 */
export function effectStateOf(res) {
  if (!res) return EFFECT_WAITING;
  const { n1, n0, g, fdrPass } = res;
  if (typeof n1 !== "number" || typeof n0 !== "number") return EFFECT_WAITING;
  // ① 件数が足りない
  if (n1 < NARRATIVE_MIN_N_PER_GROUP || n0 < NARRATIVE_MIN_N_PER_GROUP) return EFFECT_WAITING;
  // ③ 件数は足りたが、効果量か多重比較で通らない
  if (typeof g !== "number" || !isFinite(g)) return EFFECT_INCONCLUSIVE;
  // ★★選んだ 数だけ 厳しくします（★裁定 その7 §2-2 ③）。
  //   ★渡されなければ 1つ ぶん（★0.50）です。
  if (Math.abs(g) < minEffectSizeFor(res.chosen)) return EFFECT_INCONCLUSIVE;
  if (!fdrPass) return EFFECT_INCONCLUSIVE;
  // ② 全部通った
  return EFFECT_SHOWN;
}

/** 数字（効果量・件数・信頼区間）を出してよいか。★通ったときだけ。 */
export function mayShowEffectNumbers(res) {
  return effectStateOf(res) === EFFECT_SHOWN;
}
