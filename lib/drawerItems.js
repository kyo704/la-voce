...
export function itemsFor(category, tab, ctx) {
  ...
  // ★★「外の けしき」を、まど から かべ・ゆか へ 引っ越しました
  //   （★2026-09-11・見本 J03 ／ 坂本さんの お決め）。
  //   ★★消していません。★居どころが 変わっただけです。
  //   ★★理由は 見本です。★J03 の 札は「かべ／ゆか／外の けしき」の 3つで、
  //     ★まど は 別の 札に なっています。
  if (category === "surface") {
    const tiles = (c.interiorItems || []).filter(
      (i) => i.category === "tile" || i.category === "view");
    const of = c.tileSurfaceOf || (() => null);
    // ★★外の けしき は、かべ でも ゆか でも ありません。
    //   ★tileSurfaceOf は 壁材の ための ものなので、ここで 分けます。
    return pick(tiles, tab, c, (i) => (i.category === "view" ? "view" : of(i)));
  }
  if (category === "window") {
    return pick((c.interiorItems || []).filter(
      (i) => ["window", "door"].includes(i.category)), tab, c, (i) => i.category);
  }
