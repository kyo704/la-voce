 ...
 *     ★★一般の方には、★§9 の「試着」のままです（★2026-09-08・お決め）。
 *       ★鍵の付いた品も、★押して見られます。★持てないだけです。
 *
 *   ★★空のときは、★誰にも出しません。★出さない側が安全です。
 */
export function wardrobeAllItemsUserIds(env) {
  const raw = (env || {}).NEXT_PUBLIC_WARDROBE_ALL_ITEMS_USER_IDS;
  if (typeof raw !== "string") return [];
  return raw.split(",").map((x) => x.trim()).filter(Boolean);
}

export function mayWearEverything(userId, env) {
  const ids = wardrobeAllItemsUserIds(env);
  if (ids.length === 0) return false;
  return ids.includes(String(userId || ""));
}
 ...
 *
 *   ★★2026-09-08 まで、★下の並び（LAYER_ORDER）で描いていました。
 *     ★そこでは 下 → 上 の順で、★上が 下の前に来ていました。
 *     ★★Opus の決めは 逆です（上10 → 下20）。
 *     ★実機で「うわぎが したぎの前に出る」とご報告をいただきました。
 *
 *   ★★1点ずつの z は、★名簿が持ちます（★ながい上は 35）。
 *     ★ここは、★z を持たない品（★記念のもの219点）のための 既定です。
 */
export const SLOT_Z = Object.freeze({
  body: 0, top: 10, bottom: 20, shoes: 30, outer: 40,
  head: 45, neck: 50, eyes: 55, hat: 60,
  // ★★全身ものは、★上下のかわりです。★羽織りの手前に置きます。
  garment: 42,
  // ★★持ちものは、★いちばん手前です。★手に持つものだからです。
  prop: 70
});

export function slotZ(slot) {
  return Object.prototype.hasOwnProperty.call(SLOT_Z, slot) ? SLOT_Z[slot] : 50;
}
...
export const MOTIONS = Object.freeze({
  ...
  // ★★足の運びを 0.62 → 0.82 に（★2026-09-09・坂本さんの ご承認）。
  //   ★★体が 3.2秒で 進むのに、★足が 0.62秒で 動くと、★滑って 見えます。
  //     ★2026-09-06 に、★同じ形の ご指摘を いただいています。
  //   ★歩みと 足を、★同じ比で 遅くします。
  walk:      { bobY: 0,   tilt: 0,   sec: 0.82, travelX: 34,
...
export const LEGS = Object.freeze({
  ...
  // ★★振れる角度（★2026-09-08 夜・坂本さんのご指摘「振れすぎ」）。
  //   ★★14 → 7。★およそ 半分にしました。
  //     ★ご指示は「18 なら 9 に」でしたが、★いまの値は 14 でした。
  //     ★「およそ半分に」というご指図に合わせ、★7 にしています。
  //   ★★ここだけを 変えれば、★脚の振れが 変わります。
  swing:   7,     // ★振れる角度
  sec:     0.82,  // ★足の運び（★2026-09-09・0.62 から。★歩みに合わせます）
