...
/**
 * ★押しどころの言葉。★何をする品かで、変わります。
 *
 *   ★★「身につける」を、★家具にも出さないこと。★置くものは「おく」です。
 */
export function actionLabel(categoryKey, alreadyOn) {
  if (categoryKey === "store") return COPY.putAway;
  if (categoryKey === "wear") return alreadyOn ? COPY.takeOff : COPY.wear;
  return alreadyOn ? COPY.takeOff : COPY.put;
}
