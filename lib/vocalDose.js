...
export function dayVocalDose(entry) {
  ...
  if (!hasSpeechSession && entry) {
    if (typeof entry.nonPerformanceSpeechMinutes === "number" && entry.nonPerformanceSpeechMinutes > 0) {
      ...
      // ★読み取り専用の旧項目です。消さないでください（2026-08-28 調査）。
      //   いまの記録画面に speakingLevel の入力はありません。コミット 1b38007 の
      //   時点では3択の入力が出ていて、febd5f9 で外されました。
      //   「入力が無い＝誰も使っていない」と考えて消しかけましたが、
      //   実データを数えたところ1件ありました。しかもその日には
      //   non_performance_speech_minutes が入っていません。つまりここを消すと、
      //   その人のその日の発声量が 0 になります。1件でも、記録した事実は事実です。
      //   書き込みは増えません（入力が無いため）。読むだけ残します。
      // 旧3択からの概算。これは推定。
