...
export async function purgeAccount(admin, userId) {
  ...
  const { error: countError } = await admin.from("account_deletions").insert({});
  if (countError && !isMissingTable(countError)) {
    console.error("削除の件数を記録できませんでした。supabase/migration_account_deletions_count.sql を実行してください。", countError);
  }
