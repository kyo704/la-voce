...
// ★お支払いの 記録を、★人と 切り離して 残す 決め（★第3段・2026-09-16）。
//   ★★どの 列を 残すか・切るかは、★あちらが 1か所で 決めます。
import {
  KEEP_FROM_SUBSCRIPTION, KEEP_FROM_PURCHASE, RETENTION_TABLE,
  toRetained, severedProblems
} from "./paymentRetention.js";
...
export const USER_OWNED_TABLES = [
  "article_notes",
  // ★★ノート（★見本⑥・2026-09-09）。★user_id を 持つ 表です。
  //   ★★画面は まだですが、★台帳には 先に 入れます。
  //     ★退会の 消し込みは、★入れ忘れが いちばん 気づかれません。
  //     ★誰も 書いていないうちに 入れておきます。
  //   ★deleted_at で 隠した行も、★退会のときは 消します。
  //     ★「消さない」は 画面の話で、★退会は 別のことです。
  "notes",
  "article_progress",
  "chapter_state",
  "character_inventory",
  ...
  //   ★退会したら、この方の 台帳も 消します。
  //   ★★「いちど あなたのものに なったものは、あなたのものです」は
  //     ★使っておられる あいだの 約束です。★退会は 別のことです。
  "item_acquisitions",
  ...
  //   ★本人のものです。★退会のときは一緒に消します。
  //   ★★consent_records と同じ扱いです。
  //     ★「行を消さない」は、★同意を撤回したときの話です。
  //     ★アカウントごと消すときは、★消去権のほうが優先します。
  ...
  "memberships",
  "subscriptions",
  // ★★買い切り（★2026-09-09）。★退会のときは、★ここも 消します。
  //   ★★「行を 消さない」のは、★使っているあいだの 話です。
  //     ★退会は 別です。★その方の ものは、★すべて 消します。
  "purchases"
];
...
export const NULLED_REFERENCES = [
  ...
  //   ★★型は 学校の もの です。★作った 方が 抜けても、★型は 続きます。
  //   ★★名前だけ 外します（★`org_events.created_by` と 同じ 扱い）。
  { table: "lesson_presets", column: "created_by" },
  // ★★★時間割の お知らせを 出した 方（★お決め D78・2026-09-19）。
  //   ★★台帳の 制約は `on delete set null` です。★行は 残ります。
  //   ★★★「1回だけ」の 記録が 消えると、★もう一度 知らせが 行きます。
  //     ★★だから 行は 残し、★名前だけ 外します。
  { table: "timetable_nudges", column: "sent_by" },
  // ★★★連絡の 下書きの 宛て先（★お決め D82・2026-09-19）。
  //   ★★先生が 消えても、★書いた 方の 字は 残します。★宛て先だけ 外します。
  { table: "org_message_drafts", column: "teacher_id" },
  // ★★保護者の 同意の 先生（★裁定 その107）。★行は 残します。
  { table: "guardian_consents", column: "teacher_id" },
  // ★★評価の 型を 作った 方（★裁定 その105）。★型は 学校の もの です。★残します。
  { table: "evaluation_items", column: "created_by" },
 ...
 */
/**
 * ★お支払いの 記録を、★人と 切り離して `payment_records` へ 写します。
 *
 *   ★★`subscriptions`（毎月）と `purchases`（買い切り）の 両方。
 *   ★★同じ 契約を 2度 入れません（★表の 一意の 縛りが はじきます）。
 *     ★★はじかれた ことを 誤りに しません。★やり直しでも 二重に ならない ため です。
 *   ★★表が まだ 無い ときは、★止めます。
 *     ★★「表が 無いから 素通り」に すると、★帳簿を 残さずに 消えます。
 *     ★★`supabase/migration_payment_records.sql` を 先に 走らせて ください。
 */
export async function retainPaymentRecords(admin, userId) {
  const failures = [];
  const jobs = [
    { table: "subscriptions", keep: KEEP_FROM_SUBSCRIPTION, kind: "subscription" },
    { table: "purchases", keep: KEEP_FROM_PURCHASE, kind: "purchase" }
  ];
  for (const job of jobs) {
    // ★★★ご本人の 書き出し です。★**ぜんぶ** を お渡しする ため の 道 です。
    //   ★★列を 名ざしに すると、★足した 列が 書き出しから 漏れます。
    //   ★★漏れると、★ご本人が 取り出せない ものが できます。
    // eslint-disable-next-line no-restricted-syntax -- ★上の 註の とおり。
    const { data, error } = await admin
      .from(job.table).select("*").eq("user_id", userId);
    if (error) {
      // ★★その 表が 無い ことは あります（★買い切りを まだ 作って いない 環境）。
      //   ★★それは 素通りして よい です。★残す ものが ありません。
      if (isMissingTable(error)) continue;
      failures.push({ table: job.table, message: error.message });
      continue;
    }
    for (const row of data || []) {
      const retained = toRetained(row, job.keep, job.kind);
      // ★★切り忘れが 無いか、★入れる 前に 見ます。
      const problems = severedProblems(retained);
      if (problems.length > 0) {
        failures.push({ table: RETENTION_TABLE, message: problems.join("／") });
        continue;
      }
      const { error: insErr } = await admin.from(RETENTION_TABLE).insert(retained);
      if (!insErr) continue;
      // ★23505 は 一意の 縛り。★2度目 です。★正しい 形 です。
      if (String(insErr.code) === "23505") continue;
      failures.push({ table: RETENTION_TABLE, message: insErr.message });
    }
  }
  return failures;
}
...
export async function purgeAccount(admin, userId) {
  ...
  // ★★★お支払いの 記録だけ、★先に 切り離して 残します（★第3段・2026-09-16）。
  //
  //   ★★出どころ　坂本さん ──「★subscriptions の行を 全部 消しては いけません。
  //     ★法人税法・所得税法 ── 取引の帳簿は 7年 保存。
  //     ★GDPR 17条3項(b)・個情法 ── 法令に基づく保存は 削除の例外。
  //     ★★個人が 分からない形で（user_id を 切り離す）」
  //
  //   ★★**消す 前**に します。★消してから では、★もう 読めません。
  //   ★★`payment_records` は `user_id` の 列を **持ちません**。
  //     ★入れない ように 気を つけるのでは なく、★入れられません。
  //   ★★残す 列は `lib/paymentRetention.js` が 決めます。★ここでは 決めません。
  //
  //   ★★ここで しくじったら、★**退会を 止めます**。
  //     ★★帳簿を 残せないのに 消すのは、★法律の 側で まずい ことです。
  //     ★★`failures` に 入れると、★呼び手が 止めます。
  const retainFailures = await retainPaymentRecords(admin, userId);
  if (retainFailures.length > 0) {
    return { ok: false, failures: retainFailures };
  }
 ...
 *
 *   ★★なぜ ここに 置くか。
 *     ★★この 一枚は、★消す 表の 一覧（`USER_OWNED_TABLES`）を 持って います。
 *       ★★画面の 言葉を 別の 場所に 置くと、★表を 足した 人が
 *         ★画面の 一覧を 直し忘れます。★それが 2026-09-15 に 起きた ことです。
 *     ★★見つかった 欠け ── ★レパートリーは **消えるのに**、
 *       ★画面の 一覧に 名前が ありません でした。
 *         ★`repertoire_tessitura`／`role_master`／`project_master` の 3表。
 *     ★★もう1つ ──「羊のおうちの 持ち物」は、
 *       ★`item_acquisitions`（★手に入れた日の 台帳）も 消えると 読めません。
 *
 *   ★★`count` … その 行に 数を 添えるか。★画面が 数えて 入れます。
 *   ★★`cycle` … 周期を 記録して いる 方にだけ 足す 行（★lib/cycleCopy.js）。
 *     ★★記録して いない 方に、★話題だけが 目に 入る わけが ありません。
 */
export const LOST_ON_DELETE = Object.freeze([
  Object.freeze({ key: "entries", label: "日々の記録", count: "days" }),
  Object.freeze({ key: "questionnaires", label: "質問票の回答", count: "responses" }),
  // ★★㋕（★2026-09-15）。★見本が 名指しして います。
  //   ★★3つの 表が 消えます ── 曲・役・公演。
  Object.freeze({ key: "repertoire", label: "レパートリー（曲・役どころ・公演）" }),
  Object.freeze({ key: "notes", label: "稽古ノート・目標" }),
  // ★★㋖（★2026-09-15）。★前は「羊のおうちの持ち物」だけ でした。
  //   ★★台帳（`item_acquisitions`）も 消えます。★そう 読める 言葉に します。
  Object.freeze({ key: "items", label: "手に入れたものの 台帳と、羊のおうちの 持ち物" }),
  Object.freeze({ key: "health", label: "既往症・アレルギー・常用薬", cycle: true })
]);

/**
 * ★引き止めの 約束（★見本 `SC['退会']` の `.note`・2行）。
 *
 *   ★裁定 2026-09-15・坂本さん「㋘ 追加」
 *
 *   ★★これは 飾りでは ありません。★守れて いる ことを 数えて あります ──
 *     ★退会の 3枚から コメントを 外した 本文に、
 *     ★「もったいない」「本当に」「よろしい」「理由」「残念」「惜しい」
 *       「考え直」が **1件も** ありません（★2026-09-15 に 数えました）。
 *   ★★見張り `withdraw-promise.test.js` が、★同じ ことを 毎回 数えます。
 *     ★★書いた あとで 破れる ことを 防ぎます。
 *
 *   ★★「この画面の 1回だけ」── ★アプリは 3枚 ですが、
 *     ★引き止めらしき ものは 1枚目の［先にデータを書き出す］だけ です。
 *     ★★だから この 2行は **1枚目に** 置きます。★2枚目・3枚目では ありません。
 *     ★★3枚の ままで よい、★という 裁定です（★㋗・2026-09-15）。
 */
export const NO_RETENTION_NOTE = Object.freeze([
  "退会を 隠しません。引き止めを 2回以上 出しません（この画面の 1回だけ）。",
  "「もったいない」と 書きません。理由を 聞きません。"
]);

/** ★見本が 太字に して いる ところ。 */
export const NO_RETENTION_BOLD = Object.freeze(["2回以上"]);
