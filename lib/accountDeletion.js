...
export const NULLED_REFERENCES = [
  ...
  // ★担当（2026-09-02 に追加）。レッスンと同じ扱いです。
  //   ★誰が誰を担当したかは、★生徒側の履歴でもあります。
  //     先生が抜けたからといって、生徒の履歴を消してはいけません。
  //     link_consents.teacher_id を SPECIAL_DELETES に入れかけて
  //     止めたのと、同じ理由です（2026-09-01）。
  //
  // ★ここに入れると、外部キーがどちらでも正しく働きます。
  //   NO ACTION なら … 先に null にするので、auth.users を消せます
  //   CASCADE なら  … 先に null にした行は、もう auth.users を指していません。
  //                   だから★連鎖に巻き込まれず、行が残ります。
  //   ★つまり confdeltype を知らなくても、この登録は安全で、
  //     どちらの場合も「行は残し、名前だけ外す」に揃います。
  { table: "assignments", column: "teacher_id" },
