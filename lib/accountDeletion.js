...
export const PENDING_FK_DECISIONS = [
  { table: "lessons", column: "teacher_id",
    note: "先生の退会時は null にして行を残す（Opus 決定）。実装は 30日の猶予との兼ね合いを整理してから" },
  { table: "lessons", column: "created_by",
    note: "先生の退会時は null にして行を残す（Opus 決定）。★null にすると「誰が作ったか」が辿れなくなるので、監査は created_at で見る形に切り替える必要がある" },
  { table: "entry_comments", column: "created_by",
    note: "★機能そのものの是非を確認中。自由記述の1対1連絡は憲章 §10・レッスン連絡パッチ §4-1 が禁じています" }
];
