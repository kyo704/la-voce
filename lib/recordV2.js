...
export function sectionIsOpen(sectionKey, { layoutV2, openSheet }) {
  ...
  //   ★★訂正の きっかけは、★私の 誤りです ── ★上の ④で
  //     「見本に 畳む しくみは ない」と 申しましたが、★ありました。
  //     ★見本は <details> を 使わず、★JavaScript で 畳んでいます
  //     （★foldNotes()／★シートの「詳しく 書く（分で）」）。
  //   ★★隠すと、★パッサッジョの通りにくさ・高音の出しやすさ の
  //     ★入口が 1つも 無くなります。★書けなく なります。
  //   ★★だから、★出します。★ただし 畳んで 出します。
  //     ★畳み方は components/VocalTracker.jsx の SectionCard が 持ちます。
  //
  // ★★1枚が 開いている あいだは、★どの節も 出しません。
  //   ★★かぶさる 1枚の 下に 節が 並ぶと、★閉じた ときに 古い 画面に 見えます。
 ...
 * ★その節を、★畳んで 出すか。
 *
 *   ★★見本の 1枚に 入る はずだった 節を、★畳んで 出します。
 *     ★題だけが 見えていて、★「詳しく 書く」を 押すと 開きます。
 *   ★★札の 字は、★見本の 中の 字です（★SH['koe'] の「詳しく 書く（分で）」）。
 *     ★私が 作った 言葉では ありません。
 *   ★★門の外（38人）は、★畳みません。★これまでどおり 開いています。
 */
export function sectionIsFolded(sectionKey, { layoutV2 }) {
  if (!layoutV2) return false;
  return sheetOfSection(sectionKey) != null;
}

/** ★畳んだ 節を 開く 札の 字（★見本 SH['koe'] より。★1文字も 変えない）。 */
export const FOLD_OPEN = "詳しく 書く";
export const FOLD_CLOSE = "閉じる";
