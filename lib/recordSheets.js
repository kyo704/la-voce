// ============================================================================
// 記録の 入力シート ── 決めごと 1か所（★見本 SH['ねむり'] ほか）
//
//   ★出どころ docs/design/pack-final/00-動く見本（さわれる・全画面）.html
//     SH['ねむり'] ／ SH['こえ'] ／ SH['karada'] ／ SH['tabe'] ／
//     SH['honban'] ／ SH['hito']
//
//   ★★見本の 記録の 画面は、★「＋の 行を 押すと 下から シートが 出る」形です。
//     ★いままで、★押しても 何も 起きませんでした。
//
//   ★★この一枚が 持つのは、★何を 聞くか と、★但し書きの 言葉だけです。
//     ★どの 欄に しまうかは、★これまでどおり entryToRow が 決めます。
//     ★★新しい 欄を 作りません。
// ============================================================================

/**
 * ★本番以外で 声を使った 時間（★SH['こえ']）。
 *
 *   ★★見本の 4つと、★いまの SPEECH_MINUTE_CHOICES は 同じです。
 *     15分 ／ 30分 ／ 1時間 ／ 2時間以上
 *   ★★だから 選択肢を ここに 書き写しません。
 *     ★VocalTracker.jsx の SPEECH_MINUTE_CHOICES を そのまま 使います。
 *     ★2か所に 書くと、★片方だけ 変わります。
 */
export const KOE = Object.freeze({
  title: "本番以外で 声を使った時間",
  lead: "歌ったぶんも、話したぶんも 合わせて",
  note: "4つから 選ぶだけです。数字で 入れたい方は、詳しく 書く で 分単位に できます。"
    + "\n毎日 数字を 打たせません。",
  detail: "詳しく 書く（分で）",
  minuteLabel: "分で 書く",
  done: "これでいい"
});

/**
 * ★昨夜の 睡眠（★SH['ねむり']）。
 *
 *   ★★見本は「寝た 時刻」と「起きた 時刻」を 選ばせ、★長さを 出します。
 *   ★★しまうのは、★寝た 時刻（bedtime）と 長さ（sleepHours）です。
 *     ★起きた 時刻の 欄は ありません。★足しません。
 *     ★★寝た 時刻と 長さが あれば、★起きた 時刻は 出せます。
 *       ★同じ ことを 2つの 欄に しまいません。
 */
export const NEMURI = Object.freeze({
  title: "昨夜の 睡眠",
  lead: "寝た時刻と 起きた時刻を 選ぶと、長さが 出ます",
  note: "きのうの値を 初めから 入れています。変わっていたら 直してください。",
  done: "これでいい",
  bedLabel: "寝た 時刻",
  wakeLabel: "起きた 時刻",
  BED: Object.freeze(["22:00", "22:30", "23:00", "23:30", "0:00", "0:30", "1:00", "1:30"]),
  WAKE: Object.freeze(["5:30", "6:00", "6:30", "7:00", "7:30", "8:00", "8:30", "9:00"]),
  // ★★見本の 初期値。★きのうが 無いときに 使います。
  BED_DEFAULT: "23:30",
  WAKE_DEFAULT: "7:00"
});

/** ★「23:30」を、★0時からの 時間に します。 */
export function hourOf(hhmm) {
  const m = /^(\d{1,2}):(\d{2})$/.exec(String(hhmm || ""));
  if (!m) return null;
  return Number(m[1]) + Number(m[2]) / 60;
}

/**
 * ★寝た 時刻と 起きた 時刻から、★眠った 長さ（時間）。
 *
 *   ★★日を またぎます。★23:30 → 7:00 は 7.5 時間です。
 *   ★★引き算が 負に なったら、★24 を 足します。
 */
export function sleepLength(bed, wake) {
  const b = hourOf(bed), w = hourOf(wake);
  if (b == null || w == null) return null;
  let d = w - b;
  if (d < 0) d += 24;
  return d;
}

/** ★「6時間20分」の 形。★見本の カードの 中の 字です。 */
export function sleepWord(hours) {
  if (typeof hours !== "number" || !Number.isFinite(hours) || hours < 0) return "";
  const h = Math.floor(hours);
  const m = Math.round((hours % 1) * 60);
  // ★★60分に なったら、★1時間に 繰り上げます（★「6時間60分」を 出さない）。
  if (m === 60) return `${h + 1}時間0分`;
  return `${h}時間${m}分`;
}

/**
 * ★長さと 寝た 時刻から、★起きた 時刻を 出します。
 *
 *   ★★しまってある のは 寝た 時刻と 長さだけです。
 *     ★もう一度 開いたときに、★見本と 同じ 2つの 札を 出すために 使います。
 */
export function wakeFromBed(bed, hours) {
  const b = hourOf(bed);
  if (b == null || typeof hours !== "number" || !Number.isFinite(hours)) return null;
  const w = (b + hours) % 24;
  const h = Math.floor(w);
  const m = Math.round((w % 1) * 60);
  const hh = m === 60 ? (h + 1) % 24 : h;
  const mm = m === 60 ? 0 : m;
  return `${hh}:${String(mm).padStart(2, "0")}`;
}

/**
 * ★からだのこと（★SH['karada']）。
 *
 *   ★★選択肢は ここに 書き写しません。
 *     ★SYMPTOM_OPTIONS（★VocalTracker.jsx）を そのまま 使います。
 *     ★2か所に 書くと、★片方だけ 変わります。
 *   ★★但し書きは 見本の まま。★1文字も 変えないこと。
 *     ★「病気の名前を 使いません」は、★この家の 決めそのものです。
 */
export const KARADA = Object.freeze({
  title: "からだのこと",
  lead: "気になったものに 印を（いくつでも）",
  note: "病気の名前を 使いません。ここで 選んだものが、"
    + "ふりかえる → 並べる の「気になったこと」に 1行ずつ 出ます。",
  done: "これでいい"
});

/**
 * ★食べたもの（★SH['tabe']）。
 *
 *   ★★選択肢は DINNER_TAGS を そのまま 使います。
 *   ★★食べ終えた 時刻は、★もう ある 欄（dinner_time）に しまいます。
 */
export const TABE = Object.freeze({
  title: "食べたもの",
  lead: "当てはまるものに 印を（いくつでも）",
  timeLabel: "食べ終えた 時刻",
  TIMES: Object.freeze(["19:00", "20:00", "21:00", "22:00", "23:00"]),
  note: "良い・悪いを 決めません。食べたことを、そのまま 残すだけです。",
  done: "これでいい"
});

/**
 * ★もっと ▸ アカウント（★SH['account']）。
 *
 *   ★★2026-09-15、★4行 すべてを 押せる ように しました（★坂本さんの お決め）。
 *     ★前は 4行 中 **2行**が 押せません でした ── ★押せない 札（§8⑤）。
 *
 *   ★★「端末を 見る」は **外しました**。★消したのでは ありません。
 *     ★★台帳（API）が 無い から です。★`app/api/` に ありません。
 *     ★★見本の 字は「端末を 見る（2台）」── ★数まで 出します。
 *       ★★数える 先が 無い ものを、★出しません。
 *     ★★戻す 引き金は 台帳 ㉚ に あります ──
 *       ★「端末の 一覧の API が できた とき」。
 *     ★★字を ここに 残しません。★どこからも 読まれない 字を 置くと、
 *       ★見本と 突き合わせる 道具が「★在る」と 数えます（★2026-09-15 に 踏みました）。
 */
export const ACCOUNT_ROWS = Object.freeze([
  "メールアドレス", "パスワードを 変える", "ログアウト"
]);

/**
 * ★設定 ▸ お知らせ（★SH['tsuchi']）。
 *
 *   ★★但し書きは 見本の まま。★1文字も 変えないこと。
 *     ★「記録の 催促は 設けていません」は、★この家の 決めそのものです。
 */
export const TSUCHI_ROWS = Object.freeze([
  "学校からの 連絡", "門下の 連絡", "行事の 前の日"
]);
export const TSUCHI_NOTE = "記録の 催促は 設けていません。出しません。";

/**
 * ★曲の ようす（★SH['yousu']）。★出来ばえでは ありません。
 *
 *   ★★★2026-09-25 ── ★ここに 4つを **書き写して** いました。
 *     ★★4つ目が「しばらく 置く」（★間に 空白）に なって いました。
 *     ★★★台帳の CHECK（`repertoire_status_check`）が 許すのは
 *       ★空白の 無い ほう **だけ** です。
 *     ★★この 一覧で 書いたら、★台帳に 弾かれます（★23514）。
 *       ★★幸い どこからも 使われて いませんでした（★読み込まれて いる だけ）。
 *     ★★★だから **この 紙から 消しました**。★4つの もとは
 *       ★`lib/practiceNote.js` の `REPERTOIRE_STATUS` **だけ** です。
 *       ★★ここから 送り直す（`export … from`）ことも しません ──
 *         ★見張りが この 紙を `data:` で 読み込みます。★`@/` を 解けません。
 */
export const YOUSU_NOTE = "出来ばえでは ありません。いま どの段階かを、ご自分で 選ぶだけです。";

/**
 * ★本番・レッスン（★SH['honban']）。
 *
 *   ★★中身は、★これまでの「練習・公演」の 節を そのまま 入れます。
 *     ★★見本の 5つの 札を ここに 作り直しません。
 *       ★同じ 列（activities）への 入口が 2つに なるからです。
 *       ★2つに なると、★片方で 書いて、★もう片方で 消えます。
 *     ★★これが「③引っ越す ── 機能は そのまま。置き場所だけ」の 意味です。
 *   ★★但し書きは 見本の まま。★1文字も 変えないこと。
 */
export const HONBAN = Object.freeze({
  title: "本番・レッスン",
  note: "出来ばえは 聞きません。「何点だったか」の 欄は ありません。"
    + "\nここに 印を つけた日が、あとで 本番でそろえる の 0日目に なります。",
  done: "これでいい"
});

/**
 * ★ひとこと（★SH['hito']）。
 *
 *   ★★但し書きは 見本の まま。★1文字も 変えないこと。
 *     ★「誰にも 送られません」は、★この家の 決めそのものです。
 */
export const HITOKOTO = Object.freeze({
  title: "ひとこと",
  // ★★見本の 案内文。★1文字も 変えないこと。
  //   ★★2026-09-11 まで「自由に記録を残せます」でした（★節の 側の 言葉）。
  placeholder: "思ったことを、そのまま",
  note: "誰にも 送られません。先生にも、学校にも、運営にも 見えません。",
  done: "これでいい"
});

/**
 * ★お仕事に合わせた記録（★足す の 7つめ）。
 *
 *   ★★見本に この行は ありません。
 *     ★坂本さんの お決め（★2026-09-11・仕分けの §4 の 2 ㋐）で 足しました。
 *     ★★「見本は 全員 同じ 3択＋6行。★型ごとの 項目を どこに 置くか」
 *       → ★㋐「足す」に、★7つめの 行を 作る。
 *   ★★だから 但し書きは 見本の 引き写しでは なく、★この家の 言葉です。
 */
export const SHIGOTO = Object.freeze({
  title: "お仕事に合わせた記録",
  note: "お仕事の かたちに 合わせた 項目だけを 出しています。"
    + "\n書かなくても、その日の 記録は 成り立ちます。",
  done: "これでいい"
});

/**
 * ★本番・レッスン の 5つの 札（★見本 SH['honban']）。
 *
 *   ★出どころ 動く見本 SH['honban']
 *     本番（ソロ）／本番（合唱・アンサンブル）／レッスン／リハーサル／なし
 *
 *   ★★2026-09-11、★見本の 札を 作りました。
 *     ★★それまでは「練習・公演」の 節を そのまま 1枚に 入れていました。
 *       ★★その 節を 画面から 外した（★お決め ㋐）ので、
 *         ★1枚が 空に なります。★見本の 札が 要ります。
 *
 *   ★★「なし」は、★活動の かたまりを 1つも 作らない、という 意味です。
 *     ★★「なし」という 名前の 本番を 作りません。
 *   ★★新しい 列を 作りません。★activities に しまいます。
 *     ★ソロ／合唱の 別は detail.performanceForm（★lib/performanceForm.js）。
 */
export const HONBAN_CHOICES = Object.freeze([
  Object.freeze({ label: "本番（ソロ）", kind: "本番", form: "ソロ" }),
  Object.freeze({ label: "本番（合唱・アンサンブル）", kind: "本番", form: "合唱・アンサンブル" }),
  Object.freeze({ label: "レッスン", kind: "レッスン", form: null }),
  Object.freeze({ label: "リハーサル", kind: "リハーサル", form: null }),
  Object.freeze({ label: "なし", kind: null, form: null })
]);

/** ★曲の 小見出し（★本番を 選んだ ときだけ 出ます）。 */
export const HONBAN_REPERTOIRE_LABEL = "曲（レパートリーから）";

/** ★いまの 活動から、★どの 札が 選ばれているか。 */
export function honbanChoiceOf(entry) {
  const acts = (entry && entry.activities) || [];
  const a = acts.find((x) => x && x.kind);
  if (!a) return null;
  const form = (a.detail || {}).performanceForm || null;
  const hit = HONBAN_CHOICES.find((c) => c.kind === a.kind
    && (c.form === form || (c.form == null && a.kind !== "本番")));
  return hit ? hit.label : null;
}
