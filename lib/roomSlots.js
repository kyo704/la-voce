...
/**
 * ★1か所につき 最低2点（★裁定 §2-2）を 満たしているか。
 *
 *   @returns [{ key, label, count, enough }]
 *
 *   ★★1点だと「選ぶ」ことに なりません。
...
export function slotCounts(items) {
  return SLOTS.map((s) => {
    const n = itemsForSlot(s.key, items).length;
    return { key: s.key, label: s.label, count: n, enough: n >= MIN_PER_SLOT };
  });
}
