 ...
 */
export function refluxCareOn(profile) {
  return !!(profile && profile.reflux_care_consent_at);
}

/**
 * ★同意を、いつ受け取ったか。★撤回されていれば null です。
 *
 *   ★★周期の記録と、同じ形にしてあります（profile の日時で見ます）。
 *   ★consents の台帳にも1行 残しますが、★門はこちらです。
 *     ★台帳は「いつ・どの文面に同意したか」の記録で、★毎回引きません。
 */
export function refluxCareConsentAt(profile) {
  return (profile && profile.reflux_care_consent_at) || null;
}
 ...
 * ★行に書く形。★列の名前で返します。
 *
 *   ★★同意が無ければ、★3つとも null です。
 *     ★「consentAt が null のまま記録を作らせない」（§13-1）。
 *   ★★列の名前を、★呼ぶ側で書かないこと。★2か所になります。
 */
export function toRow(input, allowed) {
  const v = sanitize(input, allowed);
  return {
    sleep_side: v.sleepSide,
    head_raised: v.headRaised,
    belly_tight: v.bellyTight
  };
}
