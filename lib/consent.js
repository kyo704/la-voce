...
// ★★★2026-09-15、★同意の 文から「★」を 外しかけて、★戻しました。
//
//   ★★SEV2（★画面に 出る「★」を 消す）で、★ここも 直しました。
//     ★★そして 気づきました ── ★**ハッシュが 変わります**。
//       health.cycle        fnv1a-88086752 → fnv1a-a34ee161
//       health.reflux_care  fnv1a-094f46d4 → fnv1a-35972d00
//     ★★`consent_records` は 文面を 持たず、★このハッシュだけを 持ちます。
//       ★★文を 変えると、★過去の 行が **どの 文面か 分からなく なります**。
//
//   ★★では 版を 上げればよいか ── ★**いいえ**。★上の 規則が 禁じて います。
//     「★CONSENT_POLICY_VERSION を上げてよいのは、B-3 と B-4 の本文が
//       確定し、★その2つへのリンクが実際に開ける日だけです。
//       ★誤字の直しでは上げません。」
//     ★★「★」を 外すのは、★誤字の 直しの ほうです。
//
//   ★★だから 戻しました。★画面に「★」が 出る ままです。★承知の うえです。
//     ★★**黙って 同意の 文を 変える ほうが、★重い** から です。
//
//   ★★★次に 版を 上げる とき、★一緒に 外して ください。
//     ★台帳 ㊳ に 書いて あります。★引き金は「★版を 上げる とき」です。
//   ★★台帳 ㊳ に 書いて あります。★引き金は「★版を 上げる とき」です。
//   ★★いま 助かって いるのは、★偶然 です ──
//     ★`consent_records` に 在るのは `health.record` の 22件 だけで、
//     ★その 文面は 変えて いません（fnv1a-8df9e04c の まま）。
//     ★★`health.cycle` や `health.reflux_care` に 行が あったら、
//       ★きょう 私は それを 読めなく して いました。
...
export const CONSENT_PURPOSES = [
  ...
  // ★★★2026-09-15、★`research.anonymized` を 消しました（★No.027・N-1違反）。
  //
  //   ★★どこからも 読まれて いませんでした。
  //     ★`consent_records` に 入るのは `health.record` と `health.reflux_care` だけ。
  //     ★`purposeByKey()` を 呼ぶのは 2か所で、★どちらも 別の 鍵 でした。
  //     ★★書かれる ことも、★読まれる ことも ありませんでした。
  //
  //   ★★実際の 同意は `profiles.consent_stats_use_at` に 入って いました。
  //     ★時刻が 1つ だけ です。★歴史を 持ちません。
  //     ★★だから「一度 入れて、あとで 切った 方」を 見分けられません。
  //
  //   ★★★この 定義が、★文言の 割れを 見逃した 原因 です。
  //     ★ここには こう 書いて ありました ──
  //       「★画面に出ている文言と、一字一句そろえること（2026-09-01）。
  //         2か所に別の文言があると、どちらに同意したのか答えられなくなります。」
  //     ★★その とおりに なりました。★3通りに 割れて いました。
  //     ★★けれど この 定義は 誰にも 使われて いなかったので、
  //       ★★割れても 誰も 気づきませんでした。★見張りも ありません でした。
  //
  //   ★★★字を 1か所に 置いても、★そこが 読まれて いなければ、
  //     ★1か所に した 意味が ありません。
  //     ★★「1か所に まとめた」と 書く ときは、★**読まれて いるか**まで 見ること。
  //
  //   ★★消した 字は `docs/records/修正の記録-No.027-…md` §1 に そのまま あります。
  //     ★29名の 方が ご同意くださって いました（★2026-08-26〜09-15）。
  //     ★★消した 紙を 読めば、★何に ご同意 いただいたのかが 分かります。
];
