...
export async function sendEmailChangedNotice({ fetchImpl, apiKey, from, oldEmail, newEmail, changedVia }) {
  ...
  // ★★2026-09-05 夜、★お知らせが1通も届きませんでした。
  //   ★私は「送れなくても止めない」だけ書いて、★理由を残していませんでした。
  //   ★★止めないことと、★黙ることは、★別です。
  //     ★黙ると、★なぜ届かないのかを、★誰も追えません。
  //   ★だから、★理由を残します。
  if (!apiKey) {
    console.error("★変更のお知らせ：RESEND_API_KEY が設定されていません");
    return 0;
  }
  if (!send) {
    console.error("★変更のお知らせ：fetch が使えません");
    return 0;
  }
  ...
  for (const to of [oldEmail, newEmail]) {
    ...
    try {
      ...
      if (res && res.ok) { sent += 1; continue; }
      // ★★断られた理由を、残します。
      //   ★よくあるのは、★from のドメインが Resend で確かめられていないことです。
      //   ★★理由が分からないと、★同じ夜をもう一度過ごすことになります。
      let why = "";
      try { why = await res.text(); } catch (e2) { why = "（本文を読めません）"; }
      console.error(`★変更のお知らせを断られました status=${res && res.status} ${String(why).slice(0, 300)}`);
    } catch (e) {
      // ★★止めないこと。★片方に届けば、気づいていただけます。
      //   ★どちらにも届かなくても、★付け替えは終わっています。
      //   ★ですが、★理由は残します。
      console.error("★変更のお知らせを送れませんでした:", e && e.message);
    }
  }
  return sent;
}
