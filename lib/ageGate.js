...
// ----------------------------------------------------------------------------
// ★登録の とき（★裁定 その106 の あと・お決め 6㋐・2026-09-20）
// ----------------------------------------------------------------------------
//   ★★★坂本さんの お決め ── ★15歳未満は 受け入れません（★登録の とき 弾きます）。
//     ★★もう 1つの 道（㋑ 保護者の 同意を 取る 仕組み）は 作りません。
//       ★★同意の 取り方・しまい方・撤回の 道が 要ります。★いま その 余裕が ありません。
//     ★★★これは **事業の 範囲の 決め** です。★法律の 線では ありません。
//       ★★だから「法律で 決まって います」と 書きません。★断言しません。
//
//   ★★★答えない 方は 弾きません。★分からない のは 15歳未満とは ちがいます。
//     ★★弾くのは「15歳未満です」と ご自分で 選んだ 方 だけ です。
//
//   ★見張り components/tests/age-gate.test.js

/** ★登録の ときに 選んで いただく 帯（★3つ）。 */
export const SIGNUP_BANDS = [
  { key: AGE_BANDS.UNDER_15, label: "15歳未満" },
  { key: AGE_BANDS.TEEN, label: "15〜17歳" },
  { key: AGE_BANDS.ADULT, label: "18歳以上" }
];

/** ★登録できるか。★答えて いない ときは 止めません。 */
export function maySignUp(band) {
  return band !== AGE_BANDS.UNDER_15;
}

/**
 * ★弾く ときの 1行。
 *
 *   ★★理由を 書きます。★責めません。★断言しません。
 *   ★★★「いま」と 書きます ── ★変わる 見込みが ある ため です。
 */
/**
 * ★登録の ときの 帯を、★`profiles` へ 移します（★初回の 1回 だけ）。
 *
 *   ★★すでに 帯が 決まって いれば `null` を 返します。★上書きしません。
 *   ★★知らない 字は 受け取りません。★勝手な 値を しまいません。
 */
export function adoptSignupBand(profile, signupBand, now) {
  if (!profile) return null;
  if (hasAgeBand(profile)) return null;
  if (!STORABLE_AGE_BANDS.includes(signupBand)) return null;
  return ageBandToProfilePatch(signupBand, now);
}

export const SIGNUP_REFUSE_LINE =
  "15歳未満の 方は、いま ご登録いただけません。"
  + "保護者の 方の 同意を いただく 仕組みが、まだ ないからです。"
  + "私たちの 決まりとして、そうして います。";
