...
/**
 * ★まとめて つける 画面の、★いちばん 上の 断り（★見本 `SC['出欠つけ']` の warn）。
 *
 *   ★★2026-09-24 に 足しました。★1行も ありません でした。
 *   ★★★これは **なぜ この 形なのか** を 申し上げる 字 です。
 *     ★★はじめから ぜんぶ「出席」です（`DEFAULT_MARK`）。
 *       ★★知らない 方には、★「勝手に 付いて いる」と 見えます。
 *     ★★わけを 書くと、★そこが「手間を 省く ため」に 変わります。
 */
export const BULK_WARN = Object.freeze([
  "まず 全部を 一度に つけて、違う方だけ 直します。",
  "普段は ほとんどが 出席です。1人ずつ 押すのは、時間の 無駄です。"
]);

/**
 * ★まとめて つける 画面の 下の 1行（★見本 `SC['出欠つけ']` の note）。
 *
 *   ★★お名前が 押せる ことが、★見た目で 分かりません（★`›` だけ です）。
 *   ★★本当 です ── ★`onOpenOne(l)` が 1人の 画面へ 移します。
 */
export const BULK_TAP_LINE =
  "お名前を 押すと、その方 ひとりの 画面に なります。（1人ずつ つけたい とき）";

/** ★いちばん 下の 注（★見本の `.note`。★減らしません）。 */
export const NOTES = Object.freeze([
  "出席は「担当して いる 人」が つけます。",
  "事務は、先生に 代わって つけられます（つけた人として 事務の 名前が 残ります）。",
  // ★★2026-09-24、★後ろ半分を 足しました（★見本 `SC['出欠つけ']`）。
  //   ★★「出ません」は **見せない** こと。★「欄も ありません」は **集めて いない** こと。
  //     ★★2つは ちがいます。★集めて いない ほうが 強い 約束 です。
  //   ★★本当 です ── ★`ATTENDANCE` の 印は 3つ だけ で、★わけを 書く 欄は
  //     ★この 束にも、★画面にも、★`lessons` の 列にも ありません。
  "生徒の 体調・記録の 中身は、どの 立場でも 出ません。「なぜ 休んだか」の 欄も ありません。",
