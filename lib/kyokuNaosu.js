...
/** ★題。 */
export const TITLE = tx("曲を 直す");

/** ★書ける 4つ（★見本の `f(...)` と 札）。 */
export const FIELDS = Object.freeze([
  Object.freeze({ key: "repertoire_name", label: tx("曲名"), placeholder: tx("曲名") }),
  Object.freeze({ key: "composer", label: tx("作曲家・役"), placeholder: tx("れい：プッチーニ　ミミ") })
]);

/** ★ようすの 節。 */
export const STATUS_LABEL = tx("様子");
 ...
 */
export const FIRST_LABEL = tx("はじめて 記録した日");
...
export const BTN_DELETE = tx("この曲を 消す");

/** ★下の 断り（★約束）。 */
export const NOTE =
  tx("打った そばから 残ります。記録した日数・本番の 回数は、記録から 数えているので 直せません。");
