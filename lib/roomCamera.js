...
export function cameraOf(o) {
  ...
  const lo = 100 - 100 * z;
  // ★★下を 向きすぎない ように します（★2026-09-13）。
  //   ★★床を 42％ → 48％ に 広げた とき、★ながめる で 窓が 画面の 外に 出ました。
  //     ★★カメラは 羊を 追います。★床が 広がると 羊も 下に 居ます。
  //     ★★だから さらに 下を 向き、★壁の ものが 上に はみ出しました。
  //   ★★keepTopPct … ★これより 上を 切らない、という 線（★場面の ％）。
  //     ★★見えて いる 上端は −y ÷ z です。★それを この 線 以下に します。
  //     ★★渡されなければ、★これまでどおり 制限しません。
  const keep = Number(opt.keepTopPct);
  const yRaw = clamp(50 - clamp(aimTop, 0, 100) * z, lo, 0);
  const yMin = Number.isFinite(keep) ? -clamp(keep, 0, 100) * z : lo;
  return {
    x: clamp(50 - clamp(left, 0, 100) * z, lo, 0),
    y: clamp(yRaw, Math.max(lo, yMin), 0),
    z
  };
}
 ...
 */
/**
 * ★場面が 切り替わる ときの、★カメラの 秒数（★ミリ秒）。
 *
 *   ★★2026-09-14、★坂本さん（実機）──「★したく画面に 切り替わる ときの
 *     カメラの 動きが 遅すぎる」。
 *
 *   ★★測りました（390×844）── `transition-duration: 3.2s`、
 *     ★落ち着くまで 約3.0秒。
 *   ★★3.2秒は 羊の 歩く 秒数（`WALK_MS`）でした。
 *     ★★カメラが 羊を **追う** あいだは、★その 数で なければ なりません。
 *       ★ずれると、★着く前に 止まったり、★着いてから 動いたり します。
 *     ★★けれど 場面の 切り替えでは、★羊は 動いて いません。
 *       ★追う 相手が いないのに、★3.2秒 かけて いました。
 *
 *   ★★引き出しが 上がる 秒数（`SIZES.slideMs` ＝ 260ms）に そろえます。
 *     ★★別の 数に すると、★部屋と 引き出しが ばらばらに 動きます。
 *   ★★少しだけ 長くして あるのは、★引き出しより 先に 部屋が
 *     ★止まって しまわない ため です。
 */
export const ROOM_SWITCH_MS = 280;
...
export function cameraStyle(cam, o) {
  ...
  return {
    ...
    // ★★`walkMs` に ROOM_SWITCH_MS を 渡すのは、★切り替えの あいだ だけ です。
    //   ★羊を 追う ときは WALK_MS の ままです（★上の 決まりを 崩しません）。
...
/**
 * ★「ながめる」で 見えて いる 範囲（★舞台の ％）。
 *
 *   ★出どころ 裁定 その71・追補（★2026-09-17）── ★㋕ を 戻す。
 *     ★★「したく の画面に、★『ながめる で 見える 範囲』を 細い枠で 示す」
 *
 *   ★★新しい 式を 作りません。★`cameraOf` が 返す もの から 引き直します。
 *     ★★変形は `translate(x%, y%) scale(z)`、★原点は `0 0`。
 *       ★★舞台の 点 `p` は `x + z·p` に 写ります。
 *       ★★画面に 出るのは `0 ≤ x + z·p ≤ 100` の ところ。
 *       ★★つまり `p` は `-x/z` から `(100 - x)/z` まで。
 *   ★★確かめ ── `x = -30`, `z = 1.6` を 入れると 18.75〜81.25。
 *     ★★2026-09-17 に 実機で 測った 帯と 一致します。
 *
 *   ★★`cam` が 恒等（★寄って いない）なら、★ぜんぶ 見えて います。
 *     ★★その ときは `null` を 返します。★枠を 出す 意味が ありません。
 *
 *   @param cam ★`cameraOf` が 返した もの
 *   @returns `{ left, top, width, height }`（★％）／ ★寄って いなければ null
 */
export function visibleBandPct(cam) {
  const c = cam || {};
  const z = Number(c.z);
