...
/**
 * ★運営の「設定」から 出す ときの 1行（★見本 `stMine`・2026-09-20）。
 *
 *   ★★★ここに 入れた ところには、★ご自分で 組む とき レッスンが 入りません。
 *     ★★`lib/opsKumu.js` の `openSlots` が 見て います。
 *   ★★★事務が よその 先生の 分を 組む ときは、★効きません。
 *     ★★よその 先生の ふさがりを 読む 道が ありません（★台帳 08-28）。
 *     ★★★守れない ことを、★守れる ように 書きません。
 *   ★★中身（名前）は、★ご本人 だけ が 見られます。★決まりが そう なって います。
 */
export const MINE_HERE_LINE =
  "ここで「来られない」に した ところには、ご自分で 日程を 組む とき"
  + " レッスンが 入りません。中身は あなただけが 見られます。"
  + "（事務が 代わりに 組む ときは、まだ 効きません）";

/**
 * ★★★自分の コマは、★いま どの 学校からも 見えません（★2026-09-25）。
 *
 *   ★★見本の `自分のコマ` には、★学校ごとの 言い方が あります ──
 *     ★「◯◯高校 での コマです。ほかの 学校とは 混ぜません。」
 *     ★「事務の 側は、これを 見るだけ です。直せません。」
 *   ★★その 形は まだ 作って いません。★画面が 学校を 知りません。
 *
 *   ★★★台帳を 見て 確かめた こと（★本番・2026-09-25）──
 *     ★`my_periods` は 22行。★その **ぜんぶ** が `org_id` なし です。
 *     ★読む 道は `get_teacher_periods`（SECURITY DEFINER）1本 だけ で、
 *       ★その 中で `p.org_id = p_org_id` と しぼって います。
 *     ★★だから `org_id` が 無い 行は、★どの 学校からも 1行も 出ません。
 *     ★書く 道は `my_periods` の update／delete の 決まり だけ で、
 *       ★どちらも `auth.uid() = user_id` です。★事務に 書く 道は ありません。
 *
 *   ★★★この 2行は、★いま ほんとうの こと です。★だから 書きます。
 *     ★「事務の 側は 見るだけ」は、★学校に 入って からの 話 です。
 *     ★★いまは **見えても いません**。★見えると 書くと、
 *       ★出して いない ものを 出して いる ように 読まれます。
 */
export const NO_ORG_LINES = Object.freeze([
  "どこにも 属さない コマです。",
  "いまは どの 学校からも 見えません。"
]);
...
/**
 * ★表の 下の 1行（★見本 …「先生に 見える あき　12コマ　／　押すと 直せます」）。
 *
 *   ★★前は「空いて いる コマ」でした。★数は 同じ です。
 *     ★★★ちがうのは、★**だれに 見えるか** を 言って いる ことです。
 *       ★見本の ほうが 強い ── ★この 数は 先生に 伝わる 数 だからです。
 *   ★★「押すと 直せます」── ★押せる ことが 見た目で 分かりません。
 */
export const TT_FREE_LABEL = "先生に 見える あき";
export const TT_EDIT_HINT = "押すと 直せます";

/**
 * ★凡例（★見本の 4つの 色見本）。
 *
 *   ★★★3つ です。★見本の 4つ目「ほかの 教室」は 作って いません。
 *     ★★この 表に「ほかの 教室」の マスが 出ません（★`buildGrid` は
 *       ★`class` / `unavailable` / `free` の 3つ しか 返しません）。
 *     ★★★無い マスの 色見本を 置くと、★探させます。
 *   ★★色は 画面が 持ちます（`CELL_BG`）。★ここは 名と 順だけ です。
 */
export const TT_LEGEND = Object.freeze([
  { state: "free", label: "あき" },
  { state: "unavailable", label: "あきだが 来られない" },
  { state: "class", label: "授業" }
]);

/**
 * ★表の 下の 註（★見本 `SC['時間割']` の `.note`）。
 *
 *   ★★★見本は 4行 です。★書いたのは 3行 です。
 *
 *   ★★①②③ …… ★確かめました。★本当 です ──
 *     ★`my_timetable` の 決まりは `auth.uid() = user_id` 1つ だけ。
 *       ★先生の 決まりも、★`security definer` の 道も ありません。
 *     ★`seed_prefs_from_timetable` は ご本人として 動き、
 *       ★`slot_key` と `level` しか 書きません ── ★名も 先生も 教室も 備考も
 *       ★1文字も 出て いきません。
 *     ★「来られない」の 欄は わけを 聞きません（★`TT_COPY.ngSub`）。
 *       ★どこに いるかを 入れる 欄も ありません。
 *
 *   ★★★②は 見本と 字が ちがいます。★わざと です ──
 *     ★見本 …「あなたの 端末と、あなたの 画面にしか ありません」
 *     ★ここ …「あなたの 画面にしか 出ません」
 *     ★★★中身は 台帳に あります。★端末の 中だけ では ありません。
 *       ★★「端末の 中だけ」と 書くと、★嘘に なります。
 *       ★★守って いるのは 決まり です ── ★ご本人 以外 1行も 引けません。
 *
 *   ★★★④「時間の 割り方（コマ）は 学校が 決めます。設定 →「時間の 割り方」。」
 *     ★★書いて いません。★その「設定」が、★まだ ありません。
 *     ★★出来た 日に、★ここへ 足して ください。
 */
export const TT_GRID_NOTES = Object.freeze([
  "先生に 送るのは「空いているか どうか」だけ です。",
  "授業の 名前・担当の先生・教室・備考は、あなたの 画面にしか 出ません。",
  "理由を 聞きません。どこに いるかも 送りません。"
]);

/**
 * ★マスを 選ばずに 1つの マスの 画面へ 来た とき（★2026-09-24・Opus の ご指摘）。
 *
 *   ★★白い 画面には なって いません ── ★表へ 落ちます。
 *     ★★けれど 黙って 落ちます。★押した つもりの 方には「戻された」と 見えます。
 *   ★★1行 だけ 出します。★責めません。★次に 何を すれば よいかを 書きます。
 */
export const CELL_NOT_PICKED = "コマを 選んで ください。";
/** ★その 下の 1行（★見本 `SC['授業を入れる']` の note）。★どこを 押すかを 書きます。 */
export const CELL_NOT_PICKED_HOW = "時間割から、入れたい コマを 押して ください。";
