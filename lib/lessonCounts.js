...
//
//   ★★2026-09-08、★読む先を held から attendance へ移しました。
//     ★★lessons.held は、★一度も作られていませんでした。
//       ★migration_lesson_held.sql は、★書かれないままでした。
//       ★だから「実施した」を押しても、★42703 で失敗していました。
 ...
 * 月ごとの回数。
 *
 * @param {Array} lessons  lessons の行（scheduled_at と attendance を持つ）
...
export function countHeldLessons(lessons, { year, month }) {
  ...
  rows.forEach((l) => {
    // ★★attendance を読みます（★2026-09-08）。
    //   ★★古い held も、まだ読みます。★1行も見落とさないためです。
    //     ★とはいえ、★held の列は一度も作られていないので、
    //     ★実際に入っている行は ありません。★念のためです。
    const a = attendanceOf(l);
    if (a === "came") held += 1;
    else if (a === "absent" || a === "canceled") notHeld += 1;
 ...
 * ★その行の答え。
 *
 *   ★★attendance が正です。★held は、古い形です。
 *     ★held の列は一度も作られていないので、★入っている行は ありません。
 *     ★ですが、★読む道は残します。★取りこぼさないためです。
 */
export function attendanceOf(lesson) {
  const l = lesson || {};
  if (l.attendance === "came" || l.attendance === "absent" || l.attendance === "canceled") {
    return l.attendance;
  }
  if (l.held === true) return "came";
  if (l.held === false) return "absent";
  return null;
}
