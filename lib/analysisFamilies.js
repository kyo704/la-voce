...
export function coreFactorValues(entry, prevEntry) {
  ...
  return {
    ...
    // ⑤ 起きたときのむくみ（0=なし / 1=少し / 2=はっきり）
    //   ★2026-09-03 まで null を固定で返していました。
    //     「記録する場所がまだない」という前提でしたが、いまは有ります
    //     （EdemaSelector … components/VocalTracker.jsx:11406・11937、
    //       保存は entryToRow の morning_edema）。
    //   ★その間、本人が入れた値は★分析から黙って捨てられていました。
    //   ★?? を使うこと。|| だと 0（むくみなし）が null になります。
    morningEdema: typeof e.morningEdema === "number" ? e.morningEdema : null
  };
}
 ...
 *
 * ★2026-09-03、morningEdema の除外をやめました。
 *   入力欄ができたので、除く理由がありません（上の coreFactorValues 参照）。
 * ★件数が足りるかどうかは、ここでは見ません。
 *   それは lib/displayGates.js（各群 n ≥ 10・効果量・FDR）の仕事です。
 *   ここで二重に絞ると、同じ判断が2か所になります。
 */
