...
/**
 * ★Google カレンダーへ 送る 道（★2026-09-24 に ここへ 移しました）。
 *
 *   ★★前は `components/VocalTracker.jsx` の 中に ありました。
 *     ★★2026-09-24、★出演者の 公演からも 使う ことに なりました。
 *     ★★★写しを 作りません ── ★送って よい ものの 決めは ここ に あります
 *       （`CALENDAR_ALLOWED` ／ `CALENDAR_FORBIDDEN`）。
 *       ★★決めと 道が 離れると、★どちらかが 先に 広がります。
 *
 *   ★★題名を 受け取りません（★裁定 2026-09-03 §6）。
 *     ★★前は 呼ぶ 側が「レッスン（◯◯先生）」を 渡して いて、
 *       ★利用者の Google の 予定表に 先生の お名前が 残って いました。
 *     ★★メモも 入れません ── ★体調の ことも、ほかの 方の ことも 書けます。
 *     ★★★「入れないで ください」では なく、★**入れられない 形** に します。
 */
export function googleCalendarUrl(lesson, t) {
  const ev = buildCalendarEvent(lesson, t);
  const 刻 = (d) => d.toISOString().replace(/[-:]/g, "").split(".")[0] + "Z";
  const params = new URLSearchParams({
    action: "TEMPLATE",
    text: ev.title,
    dates: `${刻(ev.start)}/${刻(ev.end)}`
  });
