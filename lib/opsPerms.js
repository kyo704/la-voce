 ...
 */
// ★★★このうち 4つは、★台帳が 1度も 見て いません（★2026-09-19・お決め D85）。
//   ★★`sched_mine` ／ `shukketsu` ／ `koma` ／ `koma_mine`
//   ★★★画面だけの 門 です。★台帳を 直に 叩く 相手には 効きません。
//     ★★据え置きです。★採点（裁定 その105）の 権限を 作る 日に、★まとめて 直します。
//     ★★台帳 docs/ledgers/08-保留している決め.md の 08-1d を 先に お読み ください。
//     ★★数は `python3 tools/insp_perm_matrix.py` が 毎回 数え直します。
export const PERMS = Object.freeze([
  ...
  // ★★★引き金 ── ★台帳 `docs/ledgers/08-保留している決め.md` の **08-12**。
  //   ★★この できことを 1つだけ 持つ 役職を 作ると、★出席の 数が **静かに 0** に なります。
  //     ★★`lessons` を 読む 決まりは、★`teacher_id = auth.uid()` の 枝に
  //       ★`sched_mine` を 併せて 求めます（★2026-09-19 に 台帳で 確かめました）。
  //     ★★誤りは 出ません。★0 と 出る だけ です。★いちばん 見つけにくい 形 です。
  //   ★★★いま 実害が 無いのは、★備えの 役職 5つ すべてが
  //     ★`sched_mine` を 併せ持って いる からです ── ★**偶然** です。
  //     ★★学校は 役職を 自由に 作れます（★裁定 その75）。
  //   ★★★引き金（★坂本さんの お言葉・2026-09-19）──
  //     ★「`monka_write` を 持ち `sched_*` を 持たない 役職が 作られた とき」。
  //   ★★直す 日 …… ★役職を 作る 画面に「門下を 付けるなら 自分の 日程も」と 出す 日。
  //     ★★または、★この 行を 触る 日。★そのとき 08-12 を 先に お読みください。
...
/**
 * ★その方の できことを、★1本の 行に します（★2026-09-18・見本の permLine）。
 *
 *   ★出どころ docs/design/pack-final/00-動く見本-PC・iPad（運営）.html
 *     ★★`function permLine(pp)` ── ★見本と 同じ 形・同じ 並び・同じ 区切り。
 *
 *   ★★並びは `PERMS` の 順 です。★持って いる 順では ありません。
 *     ★★持って いる 順に すると、★人ごとに 並びが 変わります。
 *     ★★同じ 役職の 2人で 見比べた とき、★別物に 見えます。
 *
 *   ★★1つも 無い とき、★空の 行を 返しません。
 *     ★★空だと「まだ 読めて いない」のか
 *       ★「本当に 0つ」なのか、★見た方に 区別が つきません。
 *
 *   ★★★A2（2026-09-18）で、★役割の 名では 何も 開かなく なりました。
 *     ★★開くのは できこと だけ です。
 *     ★★ところが その できことが、★画面の どこにも 出て いません でした。
 *     ★★「押しても 何も 起きない」の わけが、★見た方に 分かりません。
 *     ★★この 行は、★その わけを その場に 置く もの です。
 */
export function permLine(perms) {
  const has = permSet(perms);
  const a = PERMS.filter((p) => has.has(p.key)).map((p) => p.label);
  // ★★★ここだけ tx() に しません（★2026-09-18）。
  //   ★★この 束の できことの 名 14 も、★役職の ひな型 10 も、
  //     ★★どれも tx() を 通して いません。★日本語の まま 持って います。
  //   ★★1行だけ tx() に すると、★同じ 束の 中で 2つの 決めが 並びます。
  //   ★★★取り込むと、★見張りが 落ちます ──
  //     ★★見張りは この 束を `data:` で 読みます。
  //     ★★`data:` の 中からは `@/…` も `./…` も 解けません。
  //     ★★逃げ道に 見張りの ほうを 5本 直すと、★書き換えの 写しが 5つ できます。
  //   ★★【後まわし・引き金は この 行】── ★opsPerms を ほかの 言葉に する 日に、
  //     ★★14の 名・10の ひな型・この 1行を **まとめて** 直します。
  //     ★★1行だけ 先に 直すと、★その日 取りこぼします。
  return a.length ? a.join(" ／ ") : "できることは 1つも ありません";
}

/**
 * ★題の 下の 1行 ── ★「役職の名 ／ できこと ／ できこと …」。
 *
 *   ★★役職が 無い 方には、★役職の ところを 出しません。
 *     ★★空の 区切り（「 ／ ご請求を…」）が 頭に 残ります。
 */
export function permHeadLine(postName, perms) {
  const line = permLine(perms);
  return postName ? postName + " ／ " + line : line;
}
 ...
 */
export function mayGrant(myPerms, key, opts) {
  if (!PERM_KEYS.includes(key)) return false;

  // ★★★`monka_read` だけ の 特例（★裁定 その77・2026-09-18）。
  //
  //   ★★★なぜ 要るか ──
  //     ★★`monka_read` は、★ひな型 10役職 の どれも 持って いません。
  //     ★★上の 決まり だけ だと ── ★自分が 持って いないと 渡せない。
  //       ★★誰も 持って いない ので、★**誰も 渡せません**。★永久に 0 です。
  //     ★★2026-09-18 に 数えて、★それが 分かりました。
  //
  //   ★★★だから、★`master`（学校の 形を 直す）を 持つ 方が 渡せます。
  //     ★★自分が 持って いなくても 渡せます。
  //     ★★★ただし ── ★**自分の 役職には 付けられません**。
  //       ★★付けられると、★`master` を 持つ 方が その場で
  //         ★★門下の やりとりを 読めて しまいます。★1人で 完結します。
  //       ★★誰かに 渡す、という 形に 残します。
  //
  //   ★★★この 特例は `monka_read` **だけ** です。★広げません。
  //     ★★ほかの できことに 同じ 道を 開くと、★決まりが 2本に なります。
  //     ★★見張り `components/tests/ops-perms.test.js` が、★1つだけ で あることを 見ます。
  if (key === "monka_read") {
    if (!can(myPerms, "master")) return false;
    // ★★`opts.toMyOwnPost` … ★いま 触って いる 役職が、★自分の 役職か。
    //   ★★渡さない ときは（★呼ぶ 側が 知らない ときは）★false と します。
    //   ★★★知らない まま 通すと、★自分の 役職にも 付けられます。
    //     ★★だから 呼ぶ 側に、★必ず 渡して もらいます（★下の 見張り）。
    return !(opts && opts.toMyOwnPost);
  }
 ...
 */
export function mayGrantPost(myPerms, post, opts) {
  const s = permSet(post && post.perms);
  return [...s].every((k) => mayGrant(myPerms, k, opts));
}
...
/**
 * ★決まりの 1行（★見本 `stPost` の warn ① 2行目）。
 *
 *   ★★見本 ──「決まりは 1つ ──『学校全部に かかる ことは、
 *     ★自分が 持っていないと 渡せません』」
 *   ★★★中の 1文は `CANNOT_GRANT_REASON` そのもの です。★書き写しません。
 *     ★★触った ときの 断りと、★上の 帯の 断りが、★1文字も ずれません。
 *   ★★きょうまで、★この 決まりは **触った ときに しか** 出て いませんでした。
 *     ★★押して みて はじめて 分かる、★という こと です。
 *     ★★先に 書いて おけば、★押さずに 済みます。
 */
export const RULE_LINE_HEAD = "決まりは 1つ ──";
export function ruleLine() {
  return `${RULE_LINE_HEAD}「${CANNOT_GRANT_REASON.replace(/。$/, "")}」`;
}

/**
 * ★その できことを 持って いる 役職の 名（★見本の `whoCan`）。
 *
 *   ★★1つも 無ければ「（いません）」。★空の 文を 返しません。
 *   ★★並びは 渡された 順の まま。★こちらで 並べ替えません。
 */
export const NOBODY_LABEL = "（いません）";
export function whoCan(posts, key) {
  const a = (posts || [])
    .filter((p) => permSet(p && p.perms).has(key))
    .map((p) => (p && p.name) || "")
    .filter(Boolean);
  return a.length ? a.join("・") : NOBODY_LABEL;
}

/**
 * ★見るだけの 方への 断り（★見本 `stPost` の warn ②）。
 *
 *   ★★★きょうまで、★`post` を 持たない 方に、★何も 断って いません でした。
 *     ★★丸を 押しても 変わりません。★けれど わけが 出ません。
 *     ★★★「押せるのに 何も 起きない」── ★いちばん 悪い 形 です（★裁定 その84）。
 *   ★★誰に お願いすれば よいかまで 書きます。★行き止まりに しません。
 */
export const VIEW_ONLY_HEAD = "あなたは 見るだけです。";
export function viewOnlyLine(posts) {
  return `ひとの 役職を 変えられるのは ${whoCan(posts, "post")}`;
}
