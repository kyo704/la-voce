 ...
 * ★夕食の あとに なる 就寝の 時刻。
 *
 *   ★★札は 22:30／23:00／23:30／0:00 の 4つです。
 *   ★★0:00 は 翌日なので、★どの 夕食よりも あとです。
 *   ★★夕食が 23:00 の 日は、★0:00 しか 選べません。
 *   ★★1時間 以上 空くものだけを 使います。★食べて すぐ 寝る 日を 作りません。
 */
function bedAfter(yushoku, i) {
  const BEDS = ["22:30", "23:00", "23:30", "0:00"];
  // ★★「0:00」は 2桁では ありません。★slice では 読めません。
  //   ★lib/timeGap.js と 同じく、★split(":") で 読みます。
  const mins = (t) => {
    const [h, m] = String(t).split(":").map(Number);
    return (h === 0 ? 24 : h) * 60 + m;
  };
  const min = mins(yushoku) + 60;
  const ok = BEDS.filter((b) => mins(b) >= min);
  return ok.length ? ok[(i * 5) % ok.length] : "0:00";
}
...
function planFor(i) {
  ...
  return {
    ...
    // ★ねむり（★札から 選びます）
    //   ★★寝る 時刻は、★夕食より あとに します。
    //     ★★2026-09-11、★比較画像の たて軸が 20時間に なっていました。
    //       ★夕食 23:00・就寝 22:30 の 日が あり、★timeGapHours が
    //       ★日を またいだ と みなして 23.5時間を 返して いました。
    //       ★★関数は 正しいです。★ありえない 組を 入れて いました。
    //     ★★夕食の 時刻（19:00〜23:00）に 応じて、★そのあとの 札から 選びます。
    bed: bedAfter(["19:00", "20:00", "21:00", "22:00", "23:00"][(i * 3) % 5], i),
