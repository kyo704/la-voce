...
select json_build_object(
 'functions',(select json_object_agg(p.proname||'('||pg_get_function_identity_arguments(p.oid)||')',
     json_build_object('h',left(md5(pg_get_functiondef(p.oid)),10),'sd',p.prosecdef,'anon',has_function_privilege('anon',p.oid,'EXECUTE'),'auth',has_function_privilege('authenticated',p.oid,'EXECUTE'),-- ★表として参照しているときだけ（文字列の一致では拾わない。2026-09-23 の誤検知）
     -- ★★註を この 行に 置きます。★閉じ括弧の 前に 置くと、★`--` が `))` を 飲みます（2026-09-23）
