...
def 見本の塊(名):
  ...
  i = s.index(頭)
  つぎ = s.find("\nfunction ", i + 1)
  本 = s[i:つぎ] if つぎ > 0 else s[i:]

  # ★★★見本の 画面が、★別の 関数を 呼んで いる ことが あります（★2026-09-19）。
  #   ★★`P_homeT` は `examBlock()` を 呼びます。★あの 中に h3 と card が あります。
  #   ★★★呼び先を 読まないと、★その 節は **無かった こと** に なります。
  #     ★★取り出しの 穴を、★「差が 無い」と 読ませない ため の 決まり です。
  #   ★★だから、★字を 作る 呼び先は **仕分けに 書いて いただきます**。
  #     ★★書かれて いなければ 止まります。★黙って 足しません。
  呼び先 = []
  for m in re.finditer(r"(\w+)\s*\(", 本):
    名前 = m.group(1)
    if 名前 == 名 or ("function %s(" % 名前) not in s:
      continue
    j = s.index("function %s(" % 名前)
    k = s.find("\nfunction ", j + 1)
    中 = s[j:k] if k > 0 else s[j:]
    # ★★字（class="…"）を 作る 関数 だけ を 見ます。★数を 数える 関数は 別 です。
    if 'class="' in 中 and 名前 not in 呼び先:
      呼び先.append(名前)
  return 本, 呼び先
...
def 節を数える(塊):
  ...
  印 = {
    ...
    # ★★★2026-09-19、★ホームを くらべようと して 気づきました。
    #   ★★見本の ホームは、★数の 札（`kpi`）が 画面の 顔 です。
    #   ★★その 印を、★この 道具は 1つも 数えて いません でした。
    #   ★★★数えて いない 印は、★申告と くらべられません。
    #     ★★書き落としても、★「差が 無い」と 読めて しまいます。
    #   ★★ホーム以外の 画面に `kpi` は ありません（★2026-09-19 に 数えました）。
    #     ★★だから、★済んだ くらべの 申告は 変わりません。
    # ★★★`kpi` は **呼び出しの 数** で 数えます。
    #   ★★字（`class="kpi"`）は `kpi()` の 中に あります。★呼ぶ 側には ありません。
    #   ★★中身を 1度 足しても「1」に しか なりません。★4枚 出て いるのに、です。
    #   ★★★だから、★出て いる 枚数＝呼んだ 回数 を 数えます。
    "kpi": len(re.findall(r"\bkpi\(", 塊))
  }
...
def 走る(名):
  ...
  塊, 呼び先 = 見本の塊(名)
  # ★★★呼び先の 申告を 突き合わせます。
  #   ★★`助け` … ★この 画面の 一部 として 読む 関数（★中身を 足します）。
  #   ★★`助けは別画面` … ★別の 画面 として 数える 関数（★足しません）。
  助け = 仕分け.get("助け") or []
  別画面 = 仕分け.get("助けは別画面") or []
  書き漏れ = [x for x in 呼び先 if x not in 助け and x not in 別画面]
  if 書き漏れ:
    止まる("見本が 呼んで いる のに、仕分けに 書かれて いません: " + " ".join(書き漏れ)
          + "（★『助け』か『助けは別画面』に 書いて ください）")
  for x in 助け:
    中, _ = 見本の塊(x)
    塊 += 中
  数 = 節を数える(塊)
