...
const HONE = `(root) => {
  ...
  const 見る = (el) => {
    ...
    // ★★★'p' を 足しました（★2026-09-26）。
    //   ★★見本は 註を 'div.note' 'div.usu' で 出します。
    //     ★★実機は class を 持ちません ── ★色は lib/tokens.js から 直に 当てます。
    //   ★★★この 註に 逆さ引用符を 書かないこと ── ★ここは 字の 型 の 中 です。
    //     ★★2026-09-26 に 1つ 書いて、★型が 切れました。
    //       ★★だから class で 探すと、★実機の 註は **1つも** 見つかりません でした。
    //   ★★実機の 註は みな '<p>' です。★'p' は この 蔵では 文 の ため だけ に 使います。
    //   ★★★台帳 ── ★「約束は 字 であって、class では ありません」（★坂本さんの お決め）。
    //     ★★下の 約 の 段で 同じ 過ちを すでに 直して います。★註も 同じ です。
    if (/\\bnote\\b|\\bwarn\\b|\\busu\\b|\\bwl\\b/.test(cls) || tag === "p") kind = "注";
    ...
    //     ★★隠すと、★その 差が 消えます。
    // ★★★'back'（戻る 札）を 足しました（★2026-09-26）。
    //   ★★見本は 'div.back'（'bk()' が 出します）、★実機は '<button>'（'Back')。
    //     ★★同じ ものが 別の 種に なって いました ── ★上の 行で 2度 戒めて いる 過ち です。
    //   ★★これが 無いと、★実機の 戻る 札だけが ②に 出ます（★2026-09-26・届いたもの）。
    //   ★★★'onclick' の ある 行を 足しました（★2026-09-26）。
    //     ★★上の 註が 守って いる「本当の 差」は ──
    //       ★見本が 行の **中に** 別の 札（「なおす ›」）を 置く 形 です。
    //       ★★その ときの 行 自体は 押せません。★だから 文 の まま です。
    //     ★★けれど 見本の 行が **それ 自体 押せる** とき（'onclick' つき）は、
    //       ★実機の '<button>' と 同じ もの です。★別の 種に しません。
    //     ★★これで 守るべき 差は 残り、★形の 違いだけ が 消えます。
    else if (tag === "button" || /\\bbtn\\b|\\bback\\b/.test(cls)
             || (/\\bli\\b/.test(cls) && el.hasAttribute("onclick"))) kind = "札";
    ...
    if (kind) {
      ...
      // ★★★札 でも 止めます（★2026-09-26）。
      //   ★★押す もの 1つ の 字は **1つの 塊** です。★中を もう一度 数えません。
      //   ★★★見本の 行は 中に 'div.usu'（添える 字）を 持ちます。
      //     ★★数え直すと、★同じ 字が 札 と 注 の 2つ に 出ます。
      //     ★★実機の 添える 字は 'span' です ── ★注 に なりません。
      //       ★★★だから 見本の 側だけ が 2つ に なり、★①に 出て いました。
      //   ★★文 は 前から 止めて います。★同じ 扱いに 揃えました。
      if (kind === "文" || kind === "札") return;
    }
