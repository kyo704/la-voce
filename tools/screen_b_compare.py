...
def 見せかけか(w):
  ...
  #   ★★★2026-09-25、★この 行が **約束の 文**を 人の 名と 見て いました ──
  #     「★出欠を つけるのは 先生です。生徒の 画面からは つけません。」
  #     ★`出欠を`（3字）＋空き＋`つけるのは`（5字）＋`先生` で 当たって いました。
  #   ★★★これは いちばん 危ない 向きの 間違い です ──
  #     ★見せかけと 見なすと、★その 文が **消えても ②に 出ません**。
  #     ★約束が 抜けた ことに 誰も 気づけません。
  #   ★★上の 行と 同じ 守りを 足します ── ★1語目が 助詞で 終わる なら 人の 名では ない。
  #     ★「高橋 のぞみ 先生」…… `高橋` は 助詞で 終わらない → いまも 当たります。
  #     ★「出欠を つけるのは 先生です」…… `出欠を` は `を` で 終わる → 外れます。
  if re.match(r"^[一-龠ぁ-んァ-ヶ]{1,4}[ 　][一-龠ぁ-んァ-ヶー]{1,5}[ 　]*(先生|さん)", t) \
     and not re.search(r"[のとにはがをへでもやからばど]$", t.split()[0] if t.split() else t):
    return True
